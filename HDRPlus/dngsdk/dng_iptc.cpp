...
bool dng_iptc::IsEmpty() const {
    ...
    if (fAuthors.Count() != 0 || fAuthorsPosition.NotEmpty()) {
        return false;
    }

    if (fCity.NotEmpty() || fState.NotEmpty() || fCountry.NotEmpty()) {
        return false;
    }
    ...
    if (fDescription.NotEmpty() || fDescriptionWriter.NotEmpty()) {
        return false;
    }
...
void dng_iptc::ParseString(dng_stream &stream, dng_string &s, CharSet charSet) {
    ...
    switch (charSet) {
        ...
        default: { s.Set_SystemEncoding(c); }
    }
...
void dng_iptc::Parse(const void *blockData, uint32 blockSize, uint64 offsetInOriginalFile) {
    dng_stream stream(blockData, blockSize, offsetInOriginalFile);
            ...
            switch (dataSet) {
                case 90: {
                    ...
                    if (dataSize == 3) {
                        ...
                        if (byte1 == 27 /* Escape */ && byte2 == 0x25 && byte3 == 0x47) {
...
void dng_iptc::SpoolString(dng_stream &stream, const dng_string &s, uint8 dataSet, uint32 maxChars, CharSet charSet) {
    if (s.IsEmpty()) {
        return;
    }
...
dng_memory_block *dng_iptc::Spool(dng_memory_allocator &allocator, bool padForTIFF) {
    uint32 j;
    ...
    SpoolString(stream, fTitle, kObjectNameSet, 64, charSet);
    ...
    SpoolString(stream, fCategory, kCategorySet, 3, charSet);

    for (j = 0; j < fSupplementalCategories.Count(); j++) {
        SpoolString(stream, fSupplementalCategories[j], kSupplementalCategoriesSet, 32, charSet);
    }

    for (j = 0; j < fKeywords.Count(); j++) {
        SpoolString(stream, fKeywords[j], kKeywordsSet, 64, charSet);
    }

    SpoolString(stream, fInstructions, kSpecialInstructionsSet, 255, charSet);
    ...
    for (j = 0; j < fAuthors.Count(); j++) {
        SpoolString(stream, fAuthors[j], kBylineSet, 32, charSet);
    }

    SpoolString(stream, fAuthorsPosition, kBylineTitleSet, 32, charSet);
    ...
    SpoolString(stream, fCountryCode, kCountryCodeSet, 3, charSet);

    SpoolString(stream, fCountry, kCountryNameSet, 64, charSet);

    SpoolString(stream, fTransmissionReference, kOriginalTransmissionReferenceSet, 32, charSet);

    SpoolString(stream, fHeadline, kHeadlineSet, 255, charSet);

    SpoolString(stream, fCredit, kCreditSet, 32, charSet);

    SpoolString(stream, fSource, kSourceSet, 32, charSet);

    SpoolString(stream, fCopyrightNotice, kCopyrightNoticeSet, 128, charSet);

    SpoolString(stream, fDescription, kCaptionSet, 2000, charSet);

    SpoolString(stream, fDescriptionWriter, kCaptionWriterSet, 32, charSet);
