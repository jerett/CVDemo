...
void dng_hue_sat_map::SetDivisions(uint32 hueDivisions, uint32 satDivisions, uint32 valDivisions) {
    ...
    if (hueDivisions == fHueDivisions && satDivisions == fSatDivisions && valDivisions == fValDivisions) {
        return;
    }
...
void dng_hue_sat_map::GetDelta(uint32 hueDiv, uint32 satDiv, uint32 valDiv, HSBModify &modify) const {
    if (hueDiv >= fHueDivisions || satDiv >= fSatDivisions || valDiv >= fValDivisions || fDeltas.Buffer() == NULL) {
        DNG_REPORT("Bad parameters to dng_hue_sat_map::GetDelta");

        ThrowProgramError();
    }

    int32 offset = valDiv * fValStep + hueDiv * fHueStep + satDiv;
...
void dng_hue_sat_map::SetDeltaKnownWriteable(uint32 hueDiv, uint32 satDiv, uint32 valDiv, const HSBModify &modify) {
    if (hueDiv >= fHueDivisions || satDiv >= fSatDivisions || valDiv >= fValDivisions || fDeltas.Buffer() == NULL) {
...
bool dng_hue_sat_map::operator==(const dng_hue_sat_map &rhs) const {
    if (fHueDivisions != rhs.fHueDivisions || fSatDivisions != rhs.fSatDivisions || fValDivisions != rhs.fValDivisions)
        return false;

    if (!IsValid())
        return true;

    return memcmp(GetConstDeltas(), rhs.GetConstDeltas(), DeltasCount() * sizeof(HSBModify)) == 0;
}

/*****************************************************************************/

dng_hue_sat_map *dng_hue_sat_map::Interpolate(const dng_hue_sat_map &map1, const dng_hue_sat_map &map2, real64 weight1) {
    if (weight1 >= 1.0) {
        if (!map1.IsValid()) {
    ...
    if (map1.fHueDivisions != map2.fHueDivisions || map1.fSatDivisions != map2.fSatDivisions || map1.fValDivisions != map2.fValDivisions) {
    ...
    result->SetDivisions(map1.fHueDivisions, map1.fSatDivisions, map1.fValDivisions);
    ...
    for (uint32 index = 0; index < count; index++) {
        data3->fHueShift = w1 * data1->fHueShift + w2 * data2->fHueShift;

        data3->fSatScale = w1 * data1->fSatScale + w2 * data2->fSatScale;

        data3->fValScale = w1 * data1->fValScale + w2 * data2->fValScale;
...

    {
        ...
        printer.Process(map1.RuntimeFingerprint().data, dng_fingerprint::kDNGFingerprintSize);

        printer.Process(map2.RuntimeFingerprint().data, dng_fingerprint::kDNGFingerprintSize);
