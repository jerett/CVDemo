...
static bool SafeUint32ToInt32Mult(uint32 arg1, uint32 arg2, int32 *result) {
...
void OptimizeOrder(const void *&sPtr,
    ...
    uint32 sRange = Abs_int32(sStep0) * (count0 - 1) + Abs_int32(sStep1) * (count1 - 1) + Abs_int32(sStep2) * (count2 - 1);

    uint32 dRange = Abs_int32(dStep0) * (count0 - 1) + Abs_int32(dStep1) * (count1 - 1) + Abs_int32(dStep2) * (count2 - 1);

    if (dRange >= sRange) {
        if (dStep0 < 0) {
            sPtr = (const void *)(((const uint8 *)sPtr) + (int32)(count0 - 1) * sStep0 * (int32)sPixelSize);

            dPtr = (void *)(((uint8 *)dPtr) + (int32)(count0 - 1) * dStep0 * (int32)dPixelSize);
        ...
        if (dStep1 < 0) {
            sPtr = (const void *)(((const uint8 *)sPtr) + (int32)(count1 - 1) * sStep1 * (int32)sPixelSize);

            dPtr = (void *)(((uint8 *)dPtr) + (int32)(count1 - 1) * dStep1 * (int32)dPixelSize);
        ...
        if (dStep2 < 0) {
            sPtr = (const void *)(((const uint8 *)sPtr) + (int32)(count2 - 1) * sStep2 * (int32)sPixelSize);

            dPtr = (void *)(((uint8 *)dPtr) + (int32)(count2 - 1) * dStep2 * (int32)dPixelSize);
    ...
    if (sStep0 == ((int32)count1) * sStep1 && dStep0 == ((int32)count1) * dStep1) {
        count1 *= count0;
        count0 = 1;
    }

    if (sStep1 == ((int32)count2) * sStep2 && dStep1 == ((int32)count2) * dStep2) {
        count2 *= count1;
        count1 = 1;
    }
}
...
void OptimizeOrder(const void *&sPtr,
    ...
    OptimizeOrder(sPtr, dPtr, sPixelSize, sPixelSize, count0, count1, count2, sStep0, sStep1, sStep2, dStep0, dStep1, dStep2);
}
...
void OptimizeOrder(void *&dPtr,
    ...
    OptimizeOrder(sPtr, dPtr, dPixelSize, dPixelSize, count0, count1, count2, sStep0, sStep1, sStep2, dStep0, dStep1, dStep2);
}
...

{
    ...
    switch (planarConfiguration) {
        case pcInterleaved: {
            ...
            if (!ConvertUint32ToInt32(fPlanes, &fColStep) || !SafeUint32ToInt32Mult(fArea.W(), fPlanes, &fRowStep)) {
        ...
        case pcPlanar: {
            ...
            if (!ConvertUint32ToInt32(fArea.W(), &fRowStep) || !SafeUint32ToInt32Mult(fArea.H(), fArea.W(), &fPlaneStep)) {
                ThrowOverflow(overflowMessage);
            }
        ...
        case pcRowInterleavedAlignSIMD: {
            ...
            if (planarConfiguration == pcRowInterleaved) {
                ...
                if (!RoundUpForPixelSize(fArea.W(), fPixelSize, &planeStepUint32)) {
                    ThrowOverflow(overflowMessage);
                }
            }

            if (!ConvertUint32ToInt32(planeStepUint32, &fPlaneStep) || !SafeUint32ToInt32Mult(planeStepUint32, fPlanes, &fRowStep)) {
                ThrowOverflow(overflowMessage);
            }
...
void dng_pixel_buffer::SetConstant(const dng_rect &area, uint32 plane, uint32 planes, uint32 value) {
    uint32 rows = area.H();
    uint32 cols = area.W();

    void *dPtr = DirtyPixel(area.t, area.l, plane);

    int32 dRowStep = fRowStep;
    int32 dColStep = fColStep;
    int32 dPlaneStep = fPlaneStep;

    OptimizeOrder(dPtr, fPixelSize, rows, cols, planes, dRowStep, dColStep, dPlaneStep);
    ...
    switch (fPixelSize) {
        case 1: {
            if (rows == 1 && cols == 1 && dPlaneStep == 1 && value == 0) {
                ...
                DoSetArea8((uint8 *)dPtr, (uint8)value, rows, cols, planes, dRowStep, dColStep, dPlaneStep);
            }
        ...
        case 2: {
            if (rows == 1 && cols == 1 && dPlaneStep == 1 && value == 0) {
                ...
                DoSetArea16((uint16 *)dPtr, (uint16)value, rows, cols, planes, dRowStep, dColStep, dPlaneStep);
            }
        ...
        case 4: {
            if (rows == 1 && cols == 1 && dPlaneStep == 1 && value == 0) {
                ...
                DoSetArea32((uint32 *)dPtr, value, rows, cols, planes, dRowStep, dColStep, dPlaneStep);
            }

            break;
        }

        default: { ThrowNotYetImplemented(); }
    }
}

/*****************************************************************************/

void dng_pixel_buffer::SetZero(const dng_rect &area, uint32 plane, uint32 planes) {
    uint32 value = 0;
...
void dng_pixel_buffer::CopyArea(const dng_pixel_buffer &src, const dng_rect &area, uint32 srcPlane, uint32 dstPlane, uint32 planes) {
    uint32 rows = area.H();
    uint32 cols = area.W();

    const void *sPtr = src.ConstPixel(area.t, area.l, srcPlane);

    void *dPtr = DirtyPixel(area.t, area.l, dstPlane);
    ...
    OptimizeOrder(
        sPtr, dPtr, src.fPixelSize, fPixelSize, rows, cols, planes, sRowStep, sColStep, sPlaneStep, dRowStep, dColStep, dPlaneStep);

    if (fPixelType == src.fPixelType) {
        if (rows == 1 && cols == 1 && sPlaneStep == 1 && dPlaneStep == 1) {
            DoCopyBytes(sPtr, dPtr, planes * fPixelSize);
        }
        ...
        switch (fPixelType) {
            ...
            case ttSShort: {
                DoCopyArea8_S16(
                    (const uint8 *)sPtr, (int16 *)dPtr, rows, cols, planes, sRowStep, sColStep, sPlaneStep, dRowStep, dColStep, dPlaneStep);
...
dng_point dng_pixel_buffer::RepeatPhase(const dng_rect &srcArea, const dng_rect &dstArea) {
...
void dng_pixel_buffer::RepeatArea(const dng_rect &srcArea, const dng_rect &dstArea) {
    dng_point repeat = srcArea.Size();

    dng_point phase = RepeatPhase(srcArea, dstArea);

    const void *sPtr = ConstPixel(srcArea.t, srcArea.l, fPlane);

    void *dPtr = DirtyPixel(dstArea.t, dstArea.l, fPlane);
...
void dng_pixel_buffer::RepeatSubArea(const dng_rect subArea, uint32 repeatV, uint32 repeatH) {
    if (fArea.t < subArea.t) {
        RepeatArea(dng_rect(subArea.t, fArea.l, subArea.t + repeatV, fArea.r), dng_rect(fArea.t, fArea.l, subArea.t, fArea.r));
    }

    if (fArea.b > subArea.b) {
        RepeatArea(dng_rect(subArea.b - repeatV, fArea.l, subArea.b, fArea.r), dng_rect(subArea.b, fArea.l, fArea.b, fArea.r));
    }

    if (fArea.l < subArea.l) {
        RepeatArea(dng_rect(fArea.t, subArea.l, fArea.b, subArea.l + repeatH), dng_rect(fArea.t, fArea.l, fArea.b, subArea.l));
    }

    if (fArea.r > subArea.r) {
        RepeatArea(dng_rect(fArea.t, subArea.r - repeatH, fArea.b, subArea.r), dng_rect(fArea.t, subArea.r, fArea.b, fArea.r));
    }
}
...
void dng_pixel_buffer::ShiftRight(uint32 shift) {
    ...
    void *dPtr = DirtyPixel(fArea.t, fArea.l, fPlane);
    ...
    OptimizeOrder(sPtr, dPtr, fPixelSize, fPixelSize, rows, cols, planes, sRowStep, sColStep, sPlaneStep, dRowStep, dColStep, dPlaneStep);

    DoShiftRight16((uint16 *)dPtr, rows, cols, planes, dRowStep, dColStep, dPlaneStep, shift);
}
...
bool dng_pixel_buffer::EqualArea(const dng_pixel_buffer &src, const dng_rect &area, uint32 plane, uint32 planes) const {
    uint32 rows = area.H();
    uint32 cols = area.W();

    const void *sPtr = src.ConstPixel(area.t, area.l, plane);

    const void *dPtr = ConstPixel(area.t, area.l, plane);
    ...
    if (fPixelType == src.fPixelType) {
        if (rows == 1 && cols == 1 && sPlaneStep == 1 && dPlaneStep == 1) {
            return DoEqualBytes(sPtr, dPtr, planes * fPixelSize);
        }
...
real64 MaxDiff(const T *src1,
               ...
               uint32 cols,
               uint32 planes) {
    if (s1ColStep == s2ColStep && s1ColStep == 1)
        return MaxDiff(src1, s1RowStep, s1PlaneStep, src2, s2RowStep, s2PlaneStep, rows, cols, planes);
...
real64 dng_pixel_buffer::MaximumDifference(const dng_pixel_buffer &rhs, const dng_rect &area, uint32 plane, uint32 planes) const {
    uint32 rows = area.H();
    uint32 cols = area.W();

    const void *s1Ptr = rhs.ConstPixel(area.t, area.l, plane);

    const void *s2Ptr = ConstPixel(area.t, area.l, plane);
