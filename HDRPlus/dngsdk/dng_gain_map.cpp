...
public:
    ...
    real32 Interpolate() const { return fValueBase + fValueStep * fValueIndex; }
...
dng_gain_map_interpolator::dng_gain_map_interpolator(const dng_gain_map &map,
    ...
    : fMap(map)
      ...
      ,
      fScale(1.0 / mapBounds.H(), 1.0 / mapBounds.W())

      ,
      fOffset(0.5 - mapBounds.t, 0.5 - mapBounds.l)
...
{
    real64 rowIndexF = (fScale.v * (row + fOffset.v) - fMap.Origin().v) / fMap.Spacing().v;
...
real32 dng_gain_map_interpolator::InterpolateEntry(uint32 colIndex) {
    return fMap.Entry(fRowIndex1, colIndex, fPlane) * (1.0f - fRowFract) + fMap.Entry(fRowIndex2, colIndex, fPlane) * (fRowFract);
}

/*****************************************************************************/

void dng_gain_map_interpolator::ResetColumn() {
    real64 colIndexF = ((fScale.h * (fColumn + fOffset.h)) - fMap.Origin().h) / fMap.Spacing().h;
    ...
    if (colIndexF <= 0.0) {
        ...
        if (colIndexF >= static_cast<real64>(lastCol)) {
            ...
            fResetColumn = (int32)ceil(((colIndex + 1) * fMap.Spacing().h + fMap.Origin().h) / fScale.h - fOffset.h);
        }
    }
...
{
    fBuffer.Reset(allocator.Allocate(ComputeBufferSize(ttFloat, fPoints, fPlanes, padSIMDBytes)));
}

/*****************************************************************************/

real32 dng_gain_map::Interpolate(int32 row, int32 col, uint32 plane, const dng_rect &bounds) const {
    dng_gain_map_interpolator interp(*this, bounds, row, col, plane);
    ...
    for (int32 rowIndex = 0; rowIndex < fPoints.v; rowIndex++) {
        for (int32 colIndex = 0; colIndex < fPoints.h; colIndex++) {
            for (uint32 plane = 0; plane < fPlanes; plane++) {
                stream.Put_real32(Entry(rowIndex, colIndex, plane));
            }
        }
    }
}
...
dng_gain_map *dng_gain_map::GetStream(dng_host &host, dng_stream &stream) {
    dng_point mapPoints;
    ...
    if (gVerbose) {
        printf("Points: v=%d, h=%d\n", (int)mapPoints.v, (int)mapPoints.h);

        printf("Spacing: v=%.6f, h=%.6f\n", mapSpacing.v, mapSpacing.h);

        printf("Origin: v=%.6f, h=%.6f\n", mapOrigin.v, mapOrigin.h);
    ...
    if (mapPoints.v < 1 || mapPoints.h < 1 || mapSpacing.v <= 0.0 || mapSpacing.h <= 0.0 || mapPlanes < 1) {
        ThrowBadFormat();
    }

    AutoPtr<dng_gain_map> map(new dng_gain_map(host.Allocator(), mapPoints, mapSpacing, mapOrigin, mapPlanes));
            ...
            for (uint32 plane = 0; plane < mapPlanes; plane++) {
                ...
                if (gVerbose) {
                    if (linesPrinted < gDumpLineLimit) {
                        printf("    Map [%3u] [%3u] [%u] = %.4f\n", (unsigned)rowIndex, (unsigned)colIndex, (unsigned)plane, x);
...
dng_opcode_GainMap::dng_opcode_GainMap(const dng_area_spec &areaSpec, AutoPtr<dng_gain_map> &gainMap)

    : dng_inplace_opcode(dngOpcode_GainMap, dngVersion_1_3_0_0, kFlag_None)
...
dng_opcode_GainMap::dng_opcode_GainMap(dng_host &host, dng_stream &stream)

    : dng_inplace_opcode(dngOpcode_GainMap, stream, "GainMap")
...
void dng_opcode_GainMap::PutData(dng_stream &stream) const {
    stream.Put_uint32(dng_area_spec::kDataSize + fGainMap->PutStreamSize());
    ...
    if (overlap.NotEmpty()) {
        ...
        for (uint32 plane = fAreaSpec.Plane(); plane < fAreaSpec.Plane() + fAreaSpec.Planes() && plane < buffer.Planes(); plane++) {
            ...
            for (int32 row = overlap.t; row < overlap.b; row += fAreaSpec.RowPitch()) {
                ...
                dng_gain_map_interpolator interp(*fGainMap, imageBounds, row, overlap.l, mapPlane);
