...
private:
    ...
    struct dictionary_entry {
        ...
        dictionary_entry(const dng_string &language, const dng_string &translation)
...
public:
    ...
    const dng_string &DefaultText() const { return fDefaultText; }

    dng_string &DefaultText() { return fDefaultText; }

    uint32 TranslationCount() const { return (uint32)fDictionary.size(); }

    const dng_string &Language(uint32 index) const { return fDictionary[index].fLanguage; }

    const dng_string &Translation(uint32 index) const { return fDictionary[index].fTranslation; }

    const dng_string &LocalText(const dng_string &locale) const;

    bool IsEmpty() const { return DefaultText().IsEmpty(); }

    bool NotEmpty() const { return !IsEmpty(); }

    bool operator==(const dng_local_string &s) const;

    bool operator!=(const dng_local_string &s) const { return !(*this == s); }
