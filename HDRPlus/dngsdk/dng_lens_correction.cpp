...
dng_warp_params::dng_warp_params(uint32 planes, const dng_point_real64 &center)
...

{
    ...
    DNG_ASSERT(fCenter.h >= 0.0 && fCenter.h <= 1.0, "Center (horizontal) out of range.");

    DNG_ASSERT(fCenter.v >= 0.0 && fCenter.v <= 1.0, "Center (vertical) out of range.");
}
...
bool dng_warp_params::IsNOPAll() const {
    return IsRadNOPAll() && IsTanNOPAll();
}

/*****************************************************************************/

bool dng_warp_params::IsNOP(uint32 plane) const {
    return IsRadNOP(plane) && IsTanNOP(plane);
}
...
bool dng_warp_params::IsValid() const {
    ...
    if (fCenter.h < 0.0 || fCenter.h > 1.0 || fCenter.v < 0.0 || fCenter.v > 1.0) {
        return false;
    }
...
bool dng_warp_params::IsValidForNegative(const dng_negative &negative) const {
    ...
    if ((fPlanes != 1) && (fPlanes != negative.ColorChannels())) {
        return false;
    }
...
real64 dng_warp_params::EvaluateInverse(uint32 plane, real64 y) const {
    const uint32 kMaxIterations = 30;
    const real64 kNearZero = 1.0e-10;

    real64 x0 = 0.0;
    real64 y0 = Evaluate(plane, x0);

    real64 x1 = 1.0;
    real64 y1 = Evaluate(plane, x1);
    ...
    for (uint32 iteration = 0; iteration < kMaxIterations; iteration++) {
        ...
        const real64 x2 = Pin_real64(0.0, x1 + (y - y1) * (x1 - x0) / (y1 - y0), 1.0);

        const real64 y2 = Evaluate(plane, x2);
...
dng_point_real64 dng_warp_params::EvaluateTangential2(uint32 plane, const dng_point_real64 &diff) const {
    const real64 dvdv = diff.v * diff.v;
    const real64 dhdh = diff.h * diff.h;

    const real64 rr = dvdv + dhdh;

    dng_point_real64 diffSqr(dvdv, dhdh);

    return EvaluateTangential(plane, rr, diff, diffSqr);
}

/*****************************************************************************/

dng_point_real64 dng_warp_params::EvaluateTangential3(uint32 plane, real64 r2, const dng_point_real64 &diff) const {
    dng_point_real64 diffSqr(diff.v * diff.v, diff.h * diff.h);

    return EvaluateTangential(plane, r2, diff, diffSqr);
}
...
real64 dng_warp_params_rectilinear::Evaluate(uint32 plane, real64 x) const {
...
real64 dng_warp_params_rectilinear::EvaluateRatio(uint32 plane, real64 r2) const {
    const dng_vector &K = fRadParams[plane];  // Coefficients.
...
real64 dng_warp_params_rectilinear::MaxSrcRadiusGap(real64 maxDstGap) const {
    ...
    for (uint32 plane = 0; plane < fPlanes; plane++) {
        ...
        if (k7 == 0.0) {
            ...
            const real64 discrim = 25.0 * k5 * k5 - 63.0 * k3 * k7 + 35.0 * d2 * k5 * k7 + 49.0 * d4 * k7 * k7;
...

        {
            ...
            const real64 gap2 = Evaluate(plane, 1.0) - Evaluate(plane, 1.0 - maxDstGap);
        ...
        for (uint32 i = 0; i < numRoots; i++) {
            ...
            if (r > 0.0 && r < 1.0 - maxDstGap) {
                const real64 gap = Evaluate(plane, r + maxDstGap) - Evaluate(plane, r);

                planeMaxSrcGap = Max_real64(planeMaxSrcGap, gap);
            }
        }

        maxSrcGap = Max_real64(maxSrcGap, planeMaxSrcGap);
    }
...
dng_point_real64 dng_warp_params_rectilinear::MaxSrcTanGap(dng_point_real64 minDst, dng_point_real64 maxDst) const {
    ...
    for (uint32 plane = 0; plane < fPlanes; plane++) {
        ...
        for (uint32 i = 0; i < 3; i++) {
            for (uint32 j = 0; j < 3; j++) {
                dng_point_real64 dstDiff(v[i], h[j]);

                dng_point_real64 srcDiff = EvaluateTangential2(plane, dstDiff);
...
void dng_warp_params_rectilinear::Dump() const {
    ...
    for (uint32 plane = 0; plane < fPlanes; plane++) {
        ...
        printf("    Tangential params: %.6lf, %.6lf\n", fTanParams[plane][0], fTanParams[plane][1]);
    }
...
dng_warp_params_fisheye::dng_warp_params_fisheye(uint32 planes, const dng_vector radParams[], const dng_point_real64 &center)
...
real64 dng_warp_params_fisheye::Evaluate(uint32 plane, real64 r) const {
...
real64 dng_warp_params_fisheye::EvaluateRatio(uint32 plane, real64 rSqr) const {
...
real64 dng_warp_params_fisheye::MaxSrcRadiusGap(real64 maxDstGap) const {
    ...
    for (uint32 plane = 0; plane < fPlanes; plane++) {
        for (uint32 i = 0; i < kSteps; i++) {
            ...
            const real64 gap = Evaluate(plane, tt + maxDstGap) - Evaluate(plane, tt);

            maxSrcGap = Max_real64(maxSrcGap, gap);
        }
    }
...
dng_point_real64 dng_warp_params_fisheye::MaxSrcTanGap(dng_point_real64 /* minDst */, dng_point_real64 /* maxDst */) const {
...
dng_filter_warp::dng_filter_warp(const dng_image &srcImage,
    ...
    : dng_filter_task("dng_filter_warp", srcImage, dstImage)
...

{
    ...
    fCenter.h = Lerp_real64((real64)bounds.l, (real64)bounds.r, fParams->fCenter.h);

    fCenter.v = Lerp_real64((real64)bounds.t, (real64)bounds.b, fParams->fCenter.v);
...

    {
        ...
        squareBounds.b = squareBounds.t + Round_int32(fPixelScaleV * (real64)squareBounds.H());

        const dng_point_real64 squareCenter(Lerp_real64((real64)squareBounds.t, (real64)squareBounds.b, fParams->fCenter.v),

                                            Lerp_real64((real64)squareBounds.l, (real64)squareBounds.r, fParams->fCenter.h));
...
void dng_filter_warp::Initialize(dng_host &host) {
    ...
    fWeights.Initialize(kernel, host.Allocator());
}
...
dng_rect dng_filter_warp::SrcArea(const dng_rect &dstArea) {
    ...
    dng_rect srcArea(yMin, xMin, yMax, xMax);
...
dng_point dng_filter_warp::SrcTileSize(const dng_point &dstTileSize) {
    ...
    const real64 maxDstGap = fInvNormRadius * hypot((real64)dstTileSize.h, (real64)dstTileSize.v);
    ...
    const dng_point_real64 minDst((bounds.t - fCenter.v) * fInvNormRadius, (bounds.l - fCenter.h) * fInvNormRadius);

    const dng_point_real64 maxDst((bounds.b - 1.0 - fCenter.v) * fInvNormRadius, (bounds.r - 1.0 - fCenter.h) * fInvNormRadius);

    const dng_point_real64 srcTanGap = fParams->MaxSrcTanGap(minDst, maxDst);
...
void dng_filter_warp::ProcessArea(uint32 /* threadIndex */, dng_pixel_buffer &srcBuffer, dng_pixel_buffer &dstBuffer) {
    // Prepare resample constants.

    const int32 wCount = fWeights.Width();

    const dng_point srcOffset(fWeights.Offset(), fWeights.Offset());
    ...
    for (uint32 plane = 0; plane < dstBuffer.fPlanes; plane++) {
        real32 *dPtr = dstBuffer.DirtyPixel_real32(dstArea.t, dstArea.l, plane);
        ...
        for (int32 dstRow = dstArea.t; dstRow < dstArea.b; dstRow++) {
            ...
            for (int32 dstCol = dstArea.l; dstCol < dstArea.r; dstCol++, dstIndex++) {
                ...
                const dng_point_real64 dPos((real64)dstRow, (real64)dstCol);

                // Warp to source (uncorrected) pixel position.

                dng_point_real64 sPos = GetSrcPixelPosition(dPos, plane);
                ...
                dng_point sInt((int32)floor(sPos.v), (int32)floor(sPos.h));

                dng_point sFct((int32)((sPos.v - (real64)sInt.v) * numSubsamples), (int32)((sPos.h - (real64)sInt.h) * numSubsamples));
                ...
                const real32 *s = srcBuffer.ConstPixel_real32(sInt.v, sInt.h, plane);
...
dng_point_real64 dng_filter_warp::GetSrcPixelPosition(const dng_point_real64 &dst, uint32 plane) {
    const dng_point_real64 diff = dst - fCenter;

    const dng_point_real64 diffNorm(diff.v * fInvNormRadius, diff.h * fInvNormRadius);

    const dng_point_real64 diffNormScaled(diffNorm.v * fPixelScaleV, diffNorm.h);

    const dng_point_real64 diffNormSqr(diffNormScaled.v * diffNormScaled.v, diffNormScaled.h * diffNormScaled.h);

    const real64 rr = Min_real64(diffNormSqr.v + diffNormSqr.h, 1.0);
    ...
    if (fIsTanNOP) {
        ...
        const real64 ratio = fParams->EvaluateRatio(plane, rr);
        ...
        const dng_point_real64 tan = fParams->EvaluateTangential(plane, rr, diffNormScaled, diffNormSqr);
...
dng_opcode_WarpRectilinear::dng_opcode_WarpRectilinear(const dng_warp_params_rectilinear &params, uint32 flags)

    : dng_opcode(dngOpcode_WarpRectilinear, dngVersion_1_3_0_0, flags)
...
dng_opcode_WarpRectilinear::dng_opcode_WarpRectilinear(dng_stream &stream)
    ...
    : dng_opcode(dngOpcode_WarpRectilinear, stream, "WarpRectilinear")
...

{
    ...
    if (fWarpParams.fPlanes == 0 || fWarpParams.fPlanes > kMaxColorPlanes) {
        ThrowBadFormat();
    }
...
void dng_opcode_WarpRectilinear::Apply(dng_host &host, dng_negative &negative, AutoPtr<dng_image> &image) {
#if qDNGValidate
    ...
    AutoPtr<dng_image> dstImage(host.Make_dng_image(image->Bounds(), image->Planes(), image->PixelType()));

    // Warp the image.

    AutoPtr<dng_warp_params> params(new dng_warp_params_rectilinear(fWarpParams));

    dng_filter_warp filter(*image, *dstImage, negative, params);

    filter.Initialize(host);

    host.PerformAreaTask(filter, image->Bounds());
...
dng_opcode_WarpFisheye::dng_opcode_WarpFisheye(const dng_warp_params_fisheye &params, uint32 flags)

    : dng_opcode(dngOpcode_WarpFisheye, dngVersion_1_3_0_0, flags)
...
dng_opcode_WarpFisheye::dng_opcode_WarpFisheye(dng_stream &stream)
    ...
    : dng_opcode(dngOpcode_WarpFisheye, stream, "WarpFisheye")
...
void dng_opcode_WarpFisheye::Apply(dng_host &host, dng_negative &negative, AutoPtr<dng_image> &image) {
#if qDNGValidate
...
dng_vignette_radial_params::dng_vignette_radial_params(const dng_std_vector<real64> &params, const dng_point_real64 &center)
...
dng_vignette_radial_params::dng_vignette_radial_params(const dng_vignette_radial_params &params) {
...
public:
    ...
    virtual real64 Evaluate(real64 x) const {
        DNG_REQUIRE(fParams.fParams.size() == dng_vignette_radial_params::kNumTerms, "Bad number of vignette opcode coefficients.");
...
dng_opcode_FixVignetteRadial::dng_opcode_FixVignetteRadial(const dng_vignette_radial_params &params, uint32 flags)

    : dng_inplace_opcode(dngOpcode_FixVignetteRadial, dngVersion_1_3_0_0, flags)
...
dng_opcode_FixVignetteRadial::dng_opcode_FixVignetteRadial(dng_stream &stream)
    ...
    : dng_inplace_opcode(dngOpcode_FixVignetteRadial, stream, "FixVignetteRadial")
...
void dng_opcode_FixVignetteRadial::Prepare(dng_negative &negative,
    ...
    const dng_point_real64 centerPixel(Lerp_real64(bounds.t, bounds.b, params.fCenter.v),

                                       Lerp_real64(bounds.l, bounds.r, params.fCenter.h));

    const real64 pixelScaleV = 1.0 / negative.PixelAspectRatio();

    const real64 maxRadius = hypot(Max_real64(Abs_real64(centerPixel.v - bounds.t), Abs_real64(centerPixel.v - bounds.b)) * pixelScaleV,

                                   Max_real64(Abs_real64(centerPixel.h - bounds.l), Abs_real64(centerPixel.h - bounds.r)));

    const dng_point_real64 radius(maxRadius, maxRadius);
    ...
    if (!fGainTable.Get()) {
        ...
        table32.Initialize(allocator, curve, false);

        // Find maximum scale factor.

        const real64 maxScale = Max_real32(table32.Interpolate(0.0f), table32.Interpolate(1.0f));
...

    {
        ...
        const uint32 bufferSize = ComputeBufferSize(pixelType, tileSize, imagePlanes, padSIMDBytes);
...
void dng_opcode_FixVignetteRadial::ProcessArea(dng_negative &negative,
    ...
    dng_pixel_buffer maskPixelBuffer(dstArea, 0, fImagePlanes, ttShort, pcRowInterleavedAlignSIMD, fMaskBuffers[threadIndex]->Buffer());
...
dng_vignette_radial_params dng_opcode_FixVignetteRadial::MakeParamsForRender(const dng_negative &negative) {
    (void)negative;
