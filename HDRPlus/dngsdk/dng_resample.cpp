...
void dng_resample_coords::Initialize(int32 srcOrigin, int32 dstOrigin, uint32 srcCount, uint32 dstCount, dng_memory_allocator &allocator) {
    fOrigin = dstOrigin;

    uint32 dstEntries = 0;
    uint32 bufferSize = 0;

    if (!RoundUpUint32ToMultiple(dstCount, 8, &dstEntries) || !SafeUint32Mult(dstEntries, sizeof(int32), &bufferSize)) {
        ThrowOverflow(
            "Arithmetic overflow computing size for coordinate "
            "buffer");
    }
    ...
    real64 invScale = (real64)srcCount / (real64)dstCount;
...
void dng_resample_weights::Initialize(real64 scale, const dng_resample_function &kernel, dng_memory_allocator &allocator) {
    uint32 j;
    ...
    DoZeroBytes(fWeights32->Buffer(), fWeights32->LogicalSize());
    ...
    DoZeroBytes(fWeights16->Buffer(), fWeights16->LogicalSize());
...
void dng_resample_weights_2d::Initialize(const dng_resample_function &kernel, dng_memory_allocator &allocator) {
    ...
    if (!SafeUint32Mult(fRadius, 2, &width) || !SafeUint32Mult(width, width, &widthSqr) || !RoundUpUint32ToMultiple(widthSqr, 8, &step)
    ...
    if (!SafeUint32Mult(step, kResampleSubsampleCount2D, &bufferSize) || !SafeUint32Mult(bufferSize, kResampleSubsampleCount2D, &bufferSize)
        || !SafeUint32Mult(bufferSize, (uint32)sizeof(real32), &bufferSize)) {
        ThrowOverflow("Arithmetic overflow computing buffer size.");
    }
    ...
    for (uint32 y = 0; y < kResampleSubsampleCount2D; y++) {
        ...
        for (uint32 x = 0; x < kResampleSubsampleCount2D; x++) {
            ...
            real32 *w32 = (real32 *)Weights32(dng_point((int32)y, (int32)x));
...

            {
                ...
                for (uint32 i = 0; i < width; i++) {
                    ...
                    for (uint32 j = 0; j < width; j++) {
                        ...
                        w32[index] = (real32)kernel.Evaluate(xPos) * (real32)kernel.Evaluate(yPos);
            ...
            {
                int16 *w16 = (int16 *)Weights16(dng_point((int32)y, (int32)x));
...
dng_resample_task::dng_resample_task(const dng_image &srcImage,
    ...
    : dng_filter_task("dng_resample_task", srcImage, dstImage)
...
{
    if (srcImage.PixelSize() <= 2 && dstImage.PixelSize() <= 2 && srcImage.PixelRange() == dstImage.PixelRange()) {
    ...
    fMaxTileSize.v = Pin_int32(fUnitCell.v, Round_int32(fMaxTileSize.v * fRowScale), fMaxTileSize.v);

    fMaxTileSize.h = Pin_int32(fUnitCell.h, Round_int32(fMaxTileSize.h * fColScale), fMaxTileSize.h);
}
...
void dng_resample_task::Start(uint32 threadCount,
    ...
    fRowCoords.Initialize(fSrcBounds.t, fDstBounds.t, fSrcBounds.H(), fDstBounds.H(), *allocator);

    fColCoords.Initialize(fSrcBounds.l, fDstBounds.l, fSrcBounds.W(), fDstBounds.W(), *allocator);

    // Compute resampling kernels.

    fWeightsV.Initialize(fRowScale, fKernel, *allocator);

    fWeightsH.Initialize(fColScale, fKernel, *allocator);
    ...
    if (!RoundUpUint32ToMultiple(fSrcTileSize.h, 8, &tempBufferSize)
        || !SafeUint32Mult(tempBufferSize, static_cast<uint32>(sizeof(real32)), &tempBufferSize)) {
        ThrowOverflow("Arithmetic overflow computing buffer size.");
    }
...
void dng_resample_task::ProcessArea(uint32 threadIndex, dng_pixel_buffer &srcBuffer, dng_pixel_buffer &dstBuffer) {
    ...
    if (fSrcPixelType == ttFloat) {
        ...
        for (int32 dstRow = dstArea.t; dstRow < dstArea.b; dstRow++) {
            ...
            for (uint32 plane = 0; plane < dstBuffer.fPlanes; plane++) {
                const real32 *sPtr = srcBuffer.ConstPixel_real32(srcRow, srcArea.l, plane);

                DoResampleDown32(sPtr, tPtr, srcCols, srcBuffer.fRowStep, weightsV, widthV);

                real32 *dPtr = dstBuffer.DirtyPixel_real32(dstRow, dstArea.l, plane);

                DoResampleAcross32(ttPtr, dPtr, dstCols, colCoords, weightsH, widthH, stepH);
            }
        }
    }
        ...
        for (int32 dstRow = dstArea.t; dstRow < dstArea.b; dstRow++) {
            ...
            for (uint32 plane = 0; plane < dstBuffer.fPlanes; plane++) {
                const uint16 *sPtr = srcBuffer.ConstPixel_uint16(srcRow, srcArea.l, plane);

                DoResampleDown16(sPtr, tPtr, srcCols, srcBuffer.fRowStep, weightsV, widthV, pixelRange);

                uint16 *dPtr = dstBuffer.DirtyPixel_uint16(dstRow, dstArea.l, plane);

                DoResampleAcross16(ttPtr, dPtr, dstCols, colCoords, weightsH, widthH, stepH, pixelRange);
            }
        }
    }
}
...
void ResampleImage(dng_host &host,
                   ...
                   const dng_rect &dstBounds,
                   const dng_resample_function &kernel) {
    dng_resample_task task(srcImage, dstImage, srcBounds, dstBounds, kernel);

    host.PerformAreaTask(task, dstBounds);
}
