...
public:
    ...
    {
        fName.Set(name);
        DNG_ASSERT(!fFingerprint.IsValid() || fName.NotEmpty(), "Cannot have profile fingerprint without name");
    }
    ...
    const dng_string &Name() const { return fName; }

    /// Getter for the fingerprint of the camera profile ID.
    /// \retval The fingerprint of the camera profile ID.

    const dng_fingerprint &Fingerprint() const { return fFingerprint; }

    /// Test for equality of two camera profile IDs.
    /// \param id The id of the camera profile ID to compare.

    bool operator==(const dng_camera_profile_id &id) const { return fName == id.fName && fFingerprint == id.fFingerprint; }

    /// Test for inequality of two camera profile IDs.
    /// \param id The id of the camera profile ID to compare.

    bool operator!=(const dng_camera_profile_id &id) const { return !(*this == id); }
    ...
    void Clear() { *this = dng_camera_profile_id(); }
};
...
public:
    ...
    bool NameIsEmbedded() const { return fName.Matches(kProfileName_Embedded, true); }
    ...
    uint32 CalibrationIlluminant1() const { return fCalibrationIlluminant1; }
    ...
    uint32 CalibrationIlluminant2() const { return fCalibrationIlluminant2; }

    /// Getter for first of up to two light sources used for calibration, returning
    /// result as color temperature.

    real64 CalibrationTemperature1() const { return IlluminantToTemperature(CalibrationIlluminant1()); }

    /// Getter for second of up to two light sources used for calibration, returning
    /// result as color temperature.

    real64 CalibrationTemperature2() const { return IlluminantToTemperature(CalibrationIlluminant2()); }
    ...
    const dng_matrix &ColorMatrix1() const { return fColorMatrix1; }

    /// Getter for second of up to two color matrices used for calibrations.

    const dng_matrix &ColorMatrix2() const { return fColorMatrix2; }
    ...
    const dng_matrix &ForwardMatrix1() const { return fForwardMatrix1; }

    /// Getter for second of up to two forward matrices used for calibrations.

    const dng_matrix &ForwardMatrix2() const { return fForwardMatrix2; }
    ...
    const dng_matrix &ReductionMatrix1() const { return fReductionMatrix1; }

    /// Getter for second of up to two dimensionality reduction hints for four color cameras.

    const dng_matrix &ReductionMatrix2() const { return fReductionMatrix2; }
    ...
    dng_camera_profile_id ProfileID() const { return dng_camera_profile_id(Name(), Fingerprint()); }
    ...
    const dng_string &Copyright() const { return fCopyright; }
    ...
    uint32 EmbedPolicy() const { return fEmbedPolicy; }

    /// Returns true iff the profile is legal to embed in a DNG, per the
    /// profile's embed policy.

    bool IsLegalToEmbed() const {
        return WasReadFromDNG() || EmbedPolicy() == pepAllowCopying || EmbedPolicy() == pepEmbedIfUsed
    ...
    bool HasHueSatDeltas() const { return fHueSatDeltas1.IsValid(); }

    /// Getter for first HueSatMap color table (for calibration illuminant 1).

    const dng_hue_sat_map &HueSatDeltas1() const { return fHueSatDeltas1; }
    ...
    const dng_hue_sat_map &HueSatDeltas2() const { return fHueSatDeltas2; }
    ...
    uint32 HueSatMapEncoding() const { return fHueSatMapEncoding; }
    ...
    bool HasLookTable() const { return fLookTable.IsValid(); }

    /// Getter for LookTable.

    const dng_hue_sat_map &LookTable() const { return fLookTable; }
    ...
    uint32 LookTableEncoding() const { return fLookTableEncoding; }
    ...
    const dng_srational &BaselineExposureOffset() const { return fBaselineExposureOffset; }
    ...
    uint32 DefaultBlackRender() const { return fDefaultBlackRender; }

    // Accessors for tone curve.

    /// Returns the tone curve of the profile.

    const dng_tone_curve &ToneCurve() const { return fToneCurve; }
    ...
    const dng_string &ProfileCalibrationSignature() const { return fProfileCalibrationSignature; }
    ...
    const dng_string &UniqueCameraModelRestriction() const { return fUniqueCameraModelRestriction; }
    ...
    void SetWasReadFromDNG(bool state = true) { fWasReadFromDNG = state; }

    /// Was this profile read from a DNG?

    bool WasReadFromDNG() const { return fWasReadFromDNG; }
    ...
    void SetWasReadFromDisk(bool state = true) { fWasReadFromDisk = state; }

    /// Was this profile read from disk?

    bool WasReadFromDisk() const { return fWasReadFromDisk; }
    ...
    void SetWasBuiltinMatrix(bool state = true) { fWasBuiltinMatrix = state; }

    /// Was this profile a built-in matrix profile?

    bool WasBuiltinMatrix() const { return fWasBuiltinMatrix; }
    ...
    bool WasStubbed() const { return fWasStubbed; }

protected:
    static real64 IlluminantToTemperature(uint32 light);

    void ClearFingerprint() { fFingerprint.Clear(); }
