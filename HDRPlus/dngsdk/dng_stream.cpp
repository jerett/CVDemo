...
dng_stream::dng_stream(dng_abort_sniffer *sniffer, uint32 bufferSize, uint64 offsetInOriginalFile)
...
dng_stream::dng_stream(const void *data, uint32 count, uint64 offsetInOriginalFile)
...
void dng_stream::DoRead(void * /* data */, uint32 /* count */, uint64 /* offset */) {
    ThrowProgramError();
}
...
void dng_stream::DoWrite(const void * /* data */, uint32 /* count */, uint64 /* offset */) {
    ThrowProgramError();
}
...
void dng_stream::SetBufferSize(dng_memory_allocator &allocator, uint32 newBufferSize) {
    if (newBufferSize != fBufferSize && newBufferSize >= gDNGStreamBlockSize * 2 && !Data() && !fBufferDirty) {
        try {
...
void dng_stream::Get(void *data, uint32 count, uint32 maxOverRead) {
    while (count) {
        ...
        if (fPosition >= fBufferStart && fPosition + count <= fBufferEnd) {
            memcpy(data, fBuffer + (uint32)(fPosition - fBufferStart), count);
        ...
        if (fPosition >= fBufferStart && fPosition < fBufferEnd) {
            ...
            memcpy(data, fBuffer + (fPosition - fBufferStart), block);
        ...
        DoRead(fBuffer, (uint32)(fBufferEnd - fBufferStart), fBufferStart);
    }
}
...
void dng_stream::Flush() {
    if (fBufferDirty) {
        ...
        DoWrite(fBuffer, (uint32)(fBufferEnd - fBufferStart), fBufferStart);
...
void dng_stream::Put(const void *data, uint32 count) {
    // See if we can replace or append to the existing buffer.

    uint64 endPosition = fPosition + count;

    if (fBufferDirty && fPosition >= fBufferStart && fPosition <= fBufferEnd && endPosition <= fBufferLimit) {
        memcpy(fBuffer + (uint32)(fPosition - fBufferStart), data, count);
    ...
    // Else we need to write to the file.
        ...
        if (fBufferDirty && fPosition >= fBufferStart && fPosition <= fBufferEnd && fPosition < fBufferLimit) {
            uint32 subCount = (uint32)(fBufferLimit - fPosition);

            memcpy(fBuffer + (uint32)(fPosition - fBufferStart), data, subCount);
        ...
        uint32 alignedSize = (uint32)(((fPosition + fBufferSize) & blockMask) - fPosition);
        ...
        if (count > alignedSize) {
            uint32 alignedCount = (uint32)(((fPosition + count) & blockMask) - fPosition);
...
void dng_stream::PutZeros(uint64 count) {
    ...
    if (count >= kZeroBufferSize) {
        ...
        DoZeroBytes(zeroBuffer.Buffer(), kZeroBufferSize);
...
void dng_stream::CopyToStream(dng_stream &dstStream, uint64 count) {
    ...
    if (count <= sizeof(smallBuffer)) {
        ...
        const uint32 bigBufferSize = (uint32)Min_uint64(kBigBufferSize, count);

        dng_memory_data bigBuffer(bigBufferSize);

        while (count) {
            uint32 blockCount = (uint32)Min_uint64(bigBufferSize, count);

            Get(bigBuffer.Buffer(), blockCount);

            dstStream.Put(bigBuffer.Buffer(), blockCount);
...
dng_stream_contiguous_read_hint::dng_stream_contiguous_read_hint(dng_stream &stream,
...
TempBigEndian::TempBigEndian(dng_stream &stream, bool bigEndian)
...
TempStreamSniffer::TempStreamSniffer(dng_stream &stream, dng_abort_sniffer *sniffer)
