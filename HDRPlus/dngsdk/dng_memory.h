...
protected:
    dng_memory_block(uint32 logicalSize) : fLogicalSize(logicalSize), fBuffer(NULL) {}
    ...
    uint32 PhysicalSize() {
        ...
        // slightly block. Someone on the image core team did not
    ...
    void SetBuffer(void *p) { fBuffer = (char *)DNG_ALIGN_SIMD(p); }
...
public:
    ...
    uint32 LogicalSize() const { return fLogicalSize; }
...
public:
    ...
    template <class U>
    dng_std_allocator(const dng_std_allocator<U> &) {}
...
template <class T>
bool operator==(const dng_std_allocator<T> & /* a1 */, const dng_std_allocator<T> & /* a2 */) {
    return true;
}
