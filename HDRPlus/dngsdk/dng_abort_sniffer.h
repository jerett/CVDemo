...
public:
    ...
    dng_priority Priority() const { return fPriority; }
    ...
    void SniffNoPriorityWait() { Sniff(); }
...
public:
    ...
    void Sniff() { dng_abort_sniffer::SniffForAbort(fSniffer); }
    ...
    void UpdateProgress(uint32 done, uint32 total) { UpdateProgress((real64)done / (real64)total); }

    /// Signal task completed for progress purposes.

    void Finish() { UpdateProgress(1.0); }
};
