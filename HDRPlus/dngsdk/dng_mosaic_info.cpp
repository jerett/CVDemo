...
void dng_bilinear_kernel::Add(const dng_point &delta, real32 weight) {
...
void dng_bilinear_kernel::Finalize(const dng_point &scale, uint32 patRow, uint32 patCol, int32 rowStep, int32 colStep) {
    uint32 j;
    ...
    while (true) {
        ...
        for (j = 1; j < fCount; j++) {
            ...
            if (delta0.v > delta1.v || (delta0.v == delta1.v && delta0.h > delta1.h)) {
                didSwap = true;
    ...
    for (j = 0; j < fCount; j++) {
        fOffset[j] = rowStep * fDelta[j].v + colStep * fDelta[j].h;
    }
...
private:
    ...
    uint32 DeltaRow(uint32 row, int32 delta) { return (row + fPatRows + (uint32)delta) % fPatRows; }

    DNG_ATTRIB_NO_SANITIZE("unsigned-integer-overflow")
    uint32 DeltaCol(uint32 col, int32 delta) { return (col + fPatCols + (uint32)delta) % fPatCols; }
...
void dng_bilinear_pattern::Calculate(const dng_mosaic_info &info, uint32 dstPlane, int32 rowStep, int32 colStep) {
    uint32 j;
    uint32 k;
    uint32 patRow;
    uint32 patCol;
    ...
    bool map[kMaxPattern][kMaxPattern];
    ...
    switch (info.fCFALayout) {
        ...
        {
            uint32 eRow = (info.fCFALayout == 6 || info.fCFALayout == 7) ? 1 : 3;

            uint32 eCol = (info.fCFALayout == 6 || info.fCFALayout == 8) ? 1 : 3;
            ...
            for (j = 0; j < fPatRows; j++) {
                for (k = 0; k < fPatCols; k++) {
                    ...
                    if ((jj != 0 && jj != eRow) || (kk != 0 && kk != eCol)) {
                        map[j][k] = false;
                    }

                    else {
                        map[j][k] =
                            (info.fCFAPattern[((j >> 1) & ~1) + Min_uint32(jj, 1)][((k >> 1) & ~1) + Min_uint32(kk, 1)] == planeColor);
                    }
                }
            }
    ...
    for (patRow = 0; patRow < fPatRows; patRow += tempScale.v) {
        for (patCol = 0; patCol < fPatCols; patCol += tempScale.h) {
            ...
            kernel.Add(dng_point(dv1, dh1), LinearWeight1(dh1, dh2) * w1);

            kernel.Add(dng_point(dv1, dh2), LinearWeight2(dh1, dh2) * w1);
            ...
            kernel.Add(dng_point(dv2, dh1), LinearWeight1(dh1, dh2) * w2);

            kernel.Add(dng_point(dv2, dh2), LinearWeight2(dh1, dh2) * w2);
            ...
            kernel.Add(dng_point(dv1, dh1), LinearWeight1(dv1, dv2) * w1);

            kernel.Add(dng_point(dv2, dh1), LinearWeight2(dv1, dv2) * w1);
            ...
            kernel.Add(dng_point(dv1, dh2), LinearWeight1(dv1, dv2) * w2);

            kernel.Add(dng_point(dv2, dh2), LinearWeight2(dv1, dv2) * w2);
        }
    }
    ...
    if (tempScale == dng_point(2, 2)) {
        ...
        for (patRow = 0; patRow < fPatRows; patRow++) {
            for (patCol = 0; patCol < fPatCols; patCol++) {
                ...
                kernel.Finalize(fScale, patRow, patCol, rowStep, colStep);
...
dng_bilinear_interpolator::dng_bilinear_interpolator(const dng_mosaic_info &info, int32 rowStep, int32 colStep) {
    for (uint32 dstPlane = 0; dstPlane < info.fColorPlanes; dstPlane++) {
        fPattern[dstPlane].Calculate(info, dstPlane, rowStep, colStep);
    }
}

/*****************************************************************************/

void dng_bilinear_interpolator::Interpolate(dng_pixel_buffer &srcBuffer, dng_pixel_buffer &dstBuffer) {
    ...
    for (int32 dstRow = dstBuffer.fArea.t; dstRow < dstBuffer.fArea.b; dstRow++) {
        int32 srcRow = dstRow >> sRowShift;

        uint32 patRow = dstRow % patRows;

        for (uint32 dstPlane = 0; dstPlane < dstBuffer.fPlanes; dstPlane++) {
            const void *sPtr = srcBuffer.ConstPixel(srcRow, srcCol, srcBuffer.fPlane);

            void *dPtr = dstBuffer.DirtyPixel(dstRow, dstCol, dstPlane);
...
dng_fast_interpolator::dng_fast_interpolator(const dng_mosaic_info &info,
    ...
    : dng_filter_task("dng_fast_interpolator", srcImage, dstImage)
...

{
    ...
    fMaxTileSize = dng_point(256 / fDownScale.v, 256 / fDownScale.h);
...
dng_rect dng_fast_interpolator::SrcArea(const dng_rect &dstArea) {
    return dng_rect(dstArea.t * fDownScale.v, dstArea.l * fDownScale.h, dstArea.b * fDownScale.v, dstArea.r * fDownScale.h);
}

/*****************************************************************************/

void dng_fast_interpolator::ProcessArea(uint32 /* threadIndex */, dng_pixel_buffer &srcBuffer, dng_pixel_buffer &dstBuffer) {
    ...
    for (int32 dstRow = dstArea.t; dstRow < dstArea.b; dstRow++) {
        const uint16 *sPtr = srcBuffer.ConstPixel_uint16(srcRow, srcArea.l, fSrcPlane);
...
void dng_mosaic_info::Parse(dng_host & /* host */, dng_stream & /* stream */, dng_info &info) {
    // Find main image IFD.
...
void dng_mosaic_info::PostParse(dng_host & /* host */, dng_negative &negative) {
    ...
    fAspectRatio = negative.DefaultScaleH().As_real64() / negative.DefaultScaleV().As_real64();
}
...
bool dng_mosaic_info::SetFourColorBayer() {
    ...
    if ((fCFAPattern[0][0] == color1 && fCFAPattern[1][1] == color1) || (fCFAPattern[0][1] == color1 && fCFAPattern[1][0] == color1)) {
        ...
        while (color3 == color0 || color3 == color1 || color3 == color2) {
            color3++;
        }
...
bool dng_mosaic_info::IsSafeDownScale(const dng_point &downScale) const {
    if (downScale.v >= fCFAPatternSize.v && downScale.h >= fCFAPatternSize.h) {
        return true;
    }
        ...
        for (int32 phaseH = 0; phaseH < fCFAPatternSize.h; phaseH++) {
            ...
            for (int32 srcRow = 0; srcRow < test.v; srcRow++) {
                for (int32 srcCol = 0; srcCol < test.h; srcCol++) {
                    uint8 srcKey = fCFAPattern[(srcRow + phaseV) % fCFAPatternSize.v][(srcCol + phaseH) % fCFAPatternSize.h];
...
bool dng_mosaic_info::ValidSizeDownScale(const dng_point &downScale, uint32 minSize) const {
    const int32 kMaxDownScale = 64;

    if (downScale.h > kMaxDownScale || downScale.v > kMaxDownScale) {
        return false;
    }
...
dng_point dng_mosaic_info::DownScale(uint32 minSize, uint32 prefSize, real64 cropFactor) const {
    ...
    if (prefSize && IsColorFilterArray()) {
        ...
        if (Abs_int32(testSize - (int32)prefSize) <= Abs_int32(bestSize - (int32)prefSize)) {
            bestScale = testScale;
            bestSize = testSize;
        }
...
dng_point dng_mosaic_info::DstSize(const dng_point &downScale) const {
    if (downScale == dng_point(1, 1)) {
        ...
        return dng_point(fSrcSize.v * scale.v, fSrcSize.h * scale.h);
    }
...
void dng_mosaic_info::InterpolateGeneric(dng_host &host,
    ...
    dng_pixel_buffer srcBuffer(dng_rect(srcTileSize), srcPlane, 1, srcImage.PixelType(), pcInterleaved, NULL);

    uint32 srcBufferSize = ComputeBufferSize(srcBuffer.fPixelType, srcTileSize, srcBuffer.fPlanes, padNone);
    ...
    dng_pixel_buffer dstBuffer(dng_rect(dstTileSize), 0, fColorPlanes, dstImage.PixelType(), pcRowInterleaved, NULL);

    uint32 dstBufferSize = ComputeBufferSize(dstBuffer.fPixelType, dstTileSize, dstBuffer.fPlanes, padNone);
    ...
    AutoPtr<dng_bilinear_interpolator> interpolator(new dng_bilinear_interpolator(*this, srcBuffer.fRowStep, srcBuffer.fColStep));
    ...
    while (iter1.GetOneTile(dstArea)) {
        ...
        while (iter2.GetOneTile(dstTile)) {
            ...
            srcImage.Get(srcBuffer, dng_image::edge_repeat, fCFAPatternSize.v, fCFAPatternSize.h);

            // Process data.

            interpolator->Interpolate(srcBuffer, dstBuffer);
...
void dng_mosaic_info::InterpolateFast(dng_host &host,
    ...
    dng_fast_interpolator interpolator(*this, srcImage, dstImage, downScale, srcPlane);
    ...
    host.PerformAreaTask(interpolator, bounds);
}
...
void dng_mosaic_info::Interpolate(dng_host &host,
    ...
    if (scaleTransforms && downScale != dng_point(1, 1)) {
        for (uint32 plane = 0; plane < dstImage.Planes(); plane++) {
            scaleTransforms[plane] = dng_matrix_3by3(1.0 / downScale.v, 0.0, 0.0, 0.0, 1.0 / downScale.h, 0.0, 0.0, 0.0, 1.0);
        }
    }

    if (downScale == dng_point(1, 1)) {
        InterpolateGeneric(host, negative, srcImage, dstImage, srcPlane);
    }

    else {
        InterpolateFast(host, negative, srcImage, dstImage, downScale, srcPlane);
    }
}
