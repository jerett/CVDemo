...
dng_tile_buffer::dng_tile_buffer(const dng_image &image, const dng_rect &tile, bool dirty)

    : fImage(image),
      fRefData(NULL)

{
    fImage.AcquireTileBuffer(*this, tile, dirty);
}
...
dng_const_tile_buffer::dng_const_tile_buffer(const dng_image &image, const dng_rect &tile)
...
dng_dirty_tile_buffer::dng_dirty_tile_buffer(dng_image &image, const dng_rect &tile)
...
dng_image::dng_image(const dng_rect &bounds, uint32 planes, uint32 pixelType)
...

{
    ...
    if (bounds.W() > kLimit || bounds.H() > kLimit) {
...
void dng_image::AcquireTileBuffer(dng_tile_buffer & /* buffer */, const dng_rect & /* area */, bool /* dirty */) const {
    ThrowProgramError();
}
...
void dng_image::DoGet(dng_pixel_buffer &buffer) const {
    ...
    while (iter.GetOneTile(tile)) {
        ...
        buffer.CopyArea(tileBuffer, tile, buffer.fPlane, buffer.fPlanes);
    }
}
...
void dng_image::DoPut(const dng_pixel_buffer &buffer) {
    ...
    while (iter.GetOneTile(tile)) {
        ...
        tileBuffer.CopyArea(buffer, tile, buffer.fPlane, buffer.fPlanes);
    }
}

/*****************************************************************************/

void dng_image::GetRepeat(dng_pixel_buffer &buffer, const dng_rect &srcArea, const dng_rect &dstArea) const {
    // If we already have the entire srcArea in the
    // buffer, we can just repeat that.

    if ((srcArea & buffer.fArea) == srcArea) {
        buffer.RepeatArea(srcArea, dstArea);
    }
    ...
    // Else we first need to get the srcArea into the buffer area.
        ...
        dng_point phase = dng_pixel_buffer::RepeatPhase(srcArea, dstArea);

        // Find new source area at top-left of dstArea.

        dng_rect newArea = srcArea + (dstArea.TL() - srcArea.TL());
        ...
        dng_rect dst1(dng_rect(newArea.t, newArea.l, splitV, splitH) & dstArea);

        if (dst1.NotEmpty()) {
            dng_pixel_buffer temp(buffer);

            temp.fArea = dst1 + (srcArea.TL() - dstArea.TL() + dng_point(phase.v, phase.h));

            temp.fData = buffer.DirtyPixel(dst1.t, dst1.l, buffer.fPlane);
        ...
        dng_rect dst2(dng_rect(newArea.t, splitH, splitV, newArea.r) & dstArea);

        if (dst2.NotEmpty()) {
            dng_pixel_buffer temp(buffer);

            temp.fArea = dst2 + (srcArea.TL() - dstArea.TL() + dng_point(phase.v, -phase.h));

            temp.fData = buffer.DirtyPixel(dst2.t, dst2.l, buffer.fPlane);
        ...
        dng_rect dst3(dng_rect(splitV, newArea.l, newArea.b, splitH) & dstArea);

        if (dst3.NotEmpty()) {
            dng_pixel_buffer temp(buffer);

            temp.fArea = dst3 + (srcArea.TL() - dstArea.TL() + dng_point(-phase.v, phase.h));

            temp.fData = buffer.DirtyPixel(dst3.t, dst3.l, buffer.fPlane);
        ...
        dng_rect dst4(dng_rect(splitV, splitH, newArea.b, newArea.r) & dstArea);

        if (dst4.NotEmpty()) {
            dng_pixel_buffer temp(buffer);

            temp.fArea = dst4 + (srcArea.TL() - dstArea.TL() + dng_point(-phase.v, -phase.h));

            temp.fData = buffer.DirtyPixel(dst4.t, dst4.l, buffer.fPlane);
        ...
        buffer.RepeatArea(newArea, dstArea);
    }
}

/*****************************************************************************/

void dng_image::GetEdge(dng_pixel_buffer &buffer, edge_option edgeOption, const dng_rect &srcArea, const dng_rect &dstArea) const {
    switch (edgeOption) {
        case edge_zero: {
            buffer.SetZero(dstArea, buffer.fPlane, buffer.fPlanes);

            break;
        }

        case edge_repeat: {
            GetRepeat(buffer, srcArea, dstArea);
        ...
        case edge_repeat_zero_last: {
            if (buffer.fPlanes > 1) {
                ...
                GetEdge(buffer1, edge_repeat, srcArea, dstArea);
            }
            ...
            buffer2.fData = buffer.DirtyPixel(buffer2.fArea.t, buffer2.fArea.l, buffer2.fPlane);

            GetEdge(buffer2, edge_zero, srcArea, dstArea);
...
void dng_image::Get(dng_pixel_buffer &buffer, edge_option edgeOption, uint32 repeatV, uint32 repeatH) const {
    ...
    if (overlap.NotEmpty()) {
        ...
        temp.fData = buffer.DirtyPixel(overlap.t, overlap.l, buffer.fPlane);
    ...
    if ((edgeOption != edge_none) && (overlap != buffer.fArea)) {
        ...
        if (areaTL.NotEmpty()) {
            GetEdge(buffer, edgeOption, dng_rect(fBounds.t, fBounds.l, fBounds.t + repeatV, fBounds.l + repeatH), areaTL);
        }
        ...
        if (areaTM.NotEmpty()) {
            GetEdge(buffer, edgeOption, dng_rect(fBounds.t, areaTM.l, fBounds.t + repeatV, areaTM.r), areaTM);
        }
        ...
        if (areaTR.NotEmpty()) {
            GetEdge(buffer, edgeOption, dng_rect(fBounds.t, fBounds.r - repeatH, fBounds.t + repeatV, fBounds.r), areaTR);
        }
        ...
        if (areaLM.NotEmpty()) {
            GetEdge(buffer, edgeOption, dng_rect(areaLM.t, fBounds.l, areaLM.b, fBounds.l + repeatH), areaLM);
        }
        ...
        if (areaRM.NotEmpty()) {
            GetEdge(buffer, edgeOption, dng_rect(areaRM.t, fBounds.r - repeatH, areaRM.b, fBounds.r), areaRM);
        }
        ...
        if (areaBL.NotEmpty()) {
            GetEdge(buffer, edgeOption, dng_rect(fBounds.b - repeatV, fBounds.l, fBounds.b, fBounds.l + repeatH), areaBL);
        }
        ...
        if (areaBM.NotEmpty()) {
            GetEdge(buffer, edgeOption, dng_rect(fBounds.b - repeatV, areaBM.l, fBounds.b, areaBM.r), areaBM);
        }
        ...
        if (areaBR.NotEmpty()) {
            GetEdge(buffer, edgeOption, dng_rect(fBounds.b - repeatV, fBounds.r - repeatH, fBounds.b, fBounds.r), areaBR);
        }
    }
}
...
void dng_image::Put(const dng_pixel_buffer &buffer) {
    ...
    if (overlap.NotEmpty()) {
        ...
        temp.fData = (void *)buffer.ConstPixel(overlap.t, overlap.l, buffer.fPlane);

        // Move the overlapping planes.

        if (temp.fPlane < Planes()) {
            temp.fPlanes = Min_uint32(temp.fPlanes, Planes() - temp.fPlane);
...
void dng_image::DoCopyArea(const dng_image &src, const dng_rect &area, uint32 srcPlane, uint32 dstPlane, uint32 planes) {
    if (&src == this)
        return;
...
bool dng_image::EqualArea(const dng_image &src, const dng_rect &area, uint32 plane, uint32 planes) const {
    if (&src == this)
        return true;
...
void dng_image::SetConstant(uint32 value, const dng_rect &area) {
    ...
    while (iter.GetOneTile(tileArea)) {
        ...
        buffer.SetConstant(tileArea, 0, fPlanes, value);
    }
}
