...
public:
    ...
    void Clear() { *this = dng_fingerprint(); }
    ...
    bool operator!=(const dng_fingerprint &print) const { return !(*this == print); }
...
struct dng_fingerprint_less_than {
    ...
    bool operator()(const dng_fingerprint &a, const dng_fingerprint &b) const { return memcmp(a.data, b.data, sizeof(a.data)) < 0; }
};
...
struct dng_fingerprint_hash {
    ...
    size_t operator()(const dng_fingerprint &digest) const { return (size_t)digest.Collapse32(); }
};
...
public:
    ...
    void Process(const char *text) { Process(text, (uint32)strlen(text)); }
...
public:
    ...
    virtual uint64 DoGetLength() { return fNextOffset; }

    virtual void DoRead(void * /* data */, uint32 /* count */, uint64 /* offset */) { ThrowProgramError(); }
