...
public:
    ...
    bool SwapBytes() const { return fSwapBytes; }
    ...
    void SetSwapBytes(bool swapBytes) { fSwapBytes = swapBytes; }
    ...
    bool LittleEndian() const { return !BigEndian(); }

    /// Setter for whether data in stream is big endian.
    /// \param littleEndian If true, data in stream is big endian.

    void SetLittleEndian(bool littleEndian = true) { SetBigEndian(!littleEndian); }

    /// Returns the size of the buffer used by the stream.

    uint32 BufferSize() const { return fBufferSize; }
    ...
    uint64 Position() const { return fPosition; }
    ...
    void Skip(uint64 delta) { SetReadPosition(Position() + delta); }

    /// Quick check to see if data range in completely buffered.

    bool DataInBuffer(uint32 count, uint64 offset) { return (offset >= fBufferStart && offset + count <= fBufferEnd); }
    ...
    dng_abort_sniffer *Sniffer() const { return fSniffer; }

    /// Putter for sniffer associated with stream.
    /// \param sniffer The new sniffer to use (or NULL for none).

    void SetSniffer(dng_abort_sniffer *sniffer) { fSniffer = sniffer; }
...
public:
    dng_stream_double_buffered(dng_stream &stream, uint32 bufferSize = kDefaultBufferSize)

        : dng_stream((dng_abort_sniffer *)NULL, bufferSize, stream.OffsetInOriginalFile())
...
protected:
    virtual uint64 DoGetLength() { return fStream.Length(); }
...
public:
    ...
    ~PreserveStreamReadPosition() { fStream.SetReadPosition(fPosition); }
};
