...
void dng_opcode_list::Apply(dng_host &host, dng_negative &negative, AutoPtr<dng_image> &image) {
    DNG_REQUIRE(image.Get(), "Bad image in dng_opcode_list::Apply");

    for (uint32 index = 0; index < Count(); index++) {
        dng_opcode &opcode(Entry(index));

        if (opcode.AboutToApply(host, negative, image->Bounds(), image->Planes())) {
...
void dng_opcode_list::Parse(dng_host &host, dng_stream &stream, uint32 byteCount, uint64 streamOffset) {
    Clear();
    ...
    for (uint32 index = 0; index < count; index++) {
        ...
        AutoPtr<dng_opcode> opcode(host.Make_dng_opcode(opcodeID, stream));
