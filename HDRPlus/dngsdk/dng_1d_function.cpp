...
real64 dng_1d_function::EvaluateInverse(real64 y) const {
    ...
    for (uint32 iteration = 0; iteration < kMaxIterations; iteration++) {
        ...
        real64 x2 = Pin_real64(0.0, x1 + (y - y1) * (x1 - x0) / (y1 - y0), 1.0);
...
dng_1d_concatenate::dng_1d_concatenate(const dng_1d_function &function1, const dng_1d_function &function2)
...
bool dng_1d_concatenate::IsIdentity() const {
    return fFunction1.IsIdentity() && fFunction2.IsIdentity();
}
