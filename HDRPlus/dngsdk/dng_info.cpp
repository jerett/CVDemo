...
void dng_info::ParseTag(dng_host &host,
                        ...
                        uint64 tagOffset,
                        int64 offsetDelta) {
    bool isSubIFD = parentCode >= tcFirstSubIFD && parentCode <= tcLastSubIFD;

    bool isMainIFD = (parentCode == 0 || isSubIFD) && ifd && ifd->fUsesNewSubFileType && ifd->fNewSubFileType == sfMainImage;
    ...
    if (fMagic == 85 && parentCode == 0 && (tagCode < tcNewSubFileType || (tagCode >= 280 && tagCode <= 283))) {
    ...
    if (ifd && ifd->ParseTag(stream, parentCode, tagCode, tagType, tagCount, tagOffset)) {
        return;
    }

    stream.SetReadPosition(tagOffset);

    if (exif && shared && exif->ParseTag(stream, *shared, parentCode, isMainIFD, tagCode, tagType, tagCount, tagOffset)) {
        return;
    }

    stream.SetReadPosition(tagOffset);

    if (shared && exif && shared->ParseTag(stream, *exif, parentCode, isMainIFD, tagCode, tagType, tagCount, tagOffset, offsetDelta)) {
        return;
    }

    if (parentCode == tcLeicaMakerNote && tagType == ttUndefined && tagCount >= 14) {
        if (ParseMakerNoteIFD(host, stream, tagCount, tagOffset, offsetDelta, tagOffset, stream.Length(), tcLeicaMakerNote)) {
            return;
        }
    }

    if (parentCode == tcOlympusMakerNote && tagType == ttUndefined && tagCount >= 14) {
        ...
        if (olympusMakerParent) {
            ...
            if (ParseMakerNoteIFD(
                    host, stream, stream.Length() - tagOffset, tagOffset, offsetDelta, tagOffset, stream.Length(), olympusMakerParent)) {
                return;
            }
        }
    }

    if (parentCode == tcRicohMakerNote && tagCode == 0x2001 && tagType == ttUndefined && tagCount > 22) {
        char header[20];
        ...
        if (memcmp(header, "[Ricoh Camera Info]", 19) == 0) {
            ParseMakerNoteIFD(
                host, stream, tagCount - 20, tagOffset + 20, offsetDelta, tagOffset + 20, tagOffset + tagCount, tcRicohMakerNoteCameraInfo);
...

    {
        ...
        if (gVerbose) {
            ...
            DumpTagValues(stream, LookupTagType(tagType), parentCode, tagCode, tagType, tagCount);
        }
...
bool dng_info::ValidateIFD(dng_stream &stream, uint64 ifdOffset, int64 offsetDelta) {
...
void dng_info::ParseIFD(dng_host &host,
    ...
    bool isMakerNote = (parentCode >= tcFirstMakerNoteIFD && parentCode <= tcLastMakerNoteIFD);
    ...
    if (gVerbose) {
        printf("%s: Offset = %u, Entries = %u\n\n", LookupParentCode(parentCode), (unsigned)ifdOffset, (unsigned)ifdEntries);
    }

    if (generateOddOffsetWarnings && (ifdOffset & 1) && !isMakerNote) {
        char message[256];

        sprintf(message, "%s has odd offset (%u)", LookupParentCode(parentCode), (unsigned)ifdOffset);
    ...
    for (uint32 tag_index = 0; tag_index < ifdEntries; tag_index++) {
        ...
        if (tagCode == 0 && tagType == 0) {
            ...
            sprintf(message, "%s had zero/zero tag code/type entry", LookupParentCode(parentCode));
...

        {
            if (tag_index > 0 && tagCode <= prev_tag_code && !isMakerNote) {
                ...
                sprintf(message, "%s tags are not sorted in ascending numerical order", LookupParentCode(parentCode));
        ...
        if (tagCount * tag_type_size > 4) {
            ...
            {
                if (generateOddOffsetWarnings && !(ifdOffset & 1) && (tagOffset & 1) && !isMakerNote && parentCode != tcKodakDCRPrivateIFD
            ...
            localTag = ifdStream.DataInBuffer(tagCount * tag_type_size, tagOffset);
        ...
        ParseTag(host, localTag ? ifdStream : stream, exif, shared, ifd, parentCode, tagCode, tagType, tagCount, tagOffset, offsetDelta);
    }
    ...
    if (nextIFD) {
        if (parentCode != 0 && (parentCode < tcFirstChainedIFD || parentCode > tcLastChainedIFD)) {
            char message[256];

            sprintf(message, "%s has an unexpected non-zero NextIFD (%u)", LookupParentCode(parentCode), (unsigned)nextIFD);
...
bool dng_info::ParseMakerNoteIFD(dng_host &host,
    ...
    for (tagIndex = 0; tagIndex < ifdEntries; tagIndex++) {
        ...
        if (tagSize > 4) {
            ...
            if (tagOffset < minOffset || tagOffset + tagSize > maxOffset) {
        ...
        if (parentCode == tcOlympusMakerNote && tagType == ttIFD && tagCount == 1) {
            uint32 olympusMakerParent = 0;
            ...
            if (olympusMakerParent) {
                ...
                if (subMakerNoteOffset >= minOffset && subMakerNoteOffset < maxOffset) {
...
void dng_info::ParseMakerNote(dng_host &host,
    ...
    stream.Get(firstBytes, (uint32)Min_uint64(sizeof(firstBytes), makerNoteCount));

    // Epson MakerNote with header.

    if (memcmp(firstBytes, "EPSON\000\001\000", 8) == 0) {
        if (makerNoteCount > 8) {
            ParseMakerNoteIFD(host, stream, makerNoteCount - 8, makerNoteOffset + 8, offsetDelta, minOffset, maxOffset, tcEpsonMakerNote);
        }
    ...
    if ((memcmp(firstBytes, "LEICA\000\000\000", 8) == 0) || (memcmp(firstBytes, "LEICA0\003\000", 8) == 0)
        || (memcmp(firstBytes, "LEICA\000\001\000", 8) == 0) || (memcmp(firstBytes, "LEICA\000\004\000", 8) == 0)
        || (memcmp(firstBytes, "LEICA\000\005\000", 8) == 0) || (memcmp(firstBytes, "LEICA\000\006\000", 8) == 0)) {
        if (makerNoteCount > 8) {
            ParseMakerNoteIFD(
                host, stream, makerNoteCount - 8, makerNoteOffset + 8, makerNoteOffset, minOffset, maxOffset, tcLeicaMakerNote);
        }
    ...
    if ((memcmp(firstBytes, "LEICA\000\002\377", 8) == 0) || (memcmp(firstBytes, "LEICA\000\002\000", 8) == 0)) {
        if (makerNoteCount > 8) {
            ParseMakerNoteIFD(host, stream, makerNoteCount - 8, makerNoteOffset + 8, offsetDelta, minOffset, maxOffset, tcLeicaMakerNote);
        }
    ...
    if (memcmp(firstBytes, "OLYMPUS\000", 8) == 0) {
        ...
        if (makerNoteCount > 12) {
            ParseMakerNoteIFD(
                host, stream, makerNoteCount - 12, makerNoteOffset + 12, makerNoteOffset, minOffset, maxOffset, tcOlympusMakerNote);
        }
    ...
    if (memcmp(firstBytes, "OLYMP", 5) == 0) {
        if (makerNoteCount > 8) {
            ParseMakerNoteIFD(host, stream, makerNoteCount - 8, makerNoteOffset + 8, offsetDelta, minOffset, maxOffset, tcOlympusMakerNote);
        }
    ...
    if (memcmp(firstBytes, "Panasonic\000\000\000", 12) == 0) {
        if (makerNoteCount > 12) {
            ParseMakerNoteIFD(
                host, stream, makerNoteCount - 12, makerNoteOffset + 12, offsetDelta, minOffset, maxOffset, tcPanasonicMakerNote);
        }
    ...
    if (memcmp(firstBytes, "RICOH", 5) == 0 || memcmp(firstBytes, "Ricoh", 5) == 0) {
        if (makerNoteCount > 8) {
            TempBigEndian tempEndian(stream);

            ParseMakerNoteIFD(host, stream, makerNoteCount - 8, makerNoteOffset + 8, offsetDelta, minOffset, maxOffset, tcRicohMakerNote);
        }
    ...
    if (fExif->fMake.StartsWith("NIKON")) {
        ParseMakerNoteIFD(host, stream, makerNoteCount, makerNoteOffset, offsetDelta, minOffset, maxOffset, tcNikonMakerNote);
    ...
    if (fExif->fMake.StartsWith("CANON")) {
        ParseMakerNoteIFD(host, stream, makerNoteCount, makerNoteOffset, offsetDelta, minOffset, maxOffset, tcCanonMakerNote);
    ...
    if (fExif->fMake.StartsWith("MINOLTA") || fExif->fMake.StartsWith("KONICA MINOLTA")) {
        ParseMakerNoteIFD(host, stream, makerNoteCount, makerNoteOffset, offsetDelta, minOffset, maxOffset, tcMinoltaMakerNote);
    ...
    if (fExif->fMake.StartsWith("SONY")) {
        ParseMakerNoteIFD(host, stream, makerNoteCount, makerNoteOffset, offsetDelta, minOffset, maxOffset, tcSonyMakerNote);
    ...
    if (fExif->fMake.StartsWith("EASTMAN KODAK")) {
        ParseMakerNoteIFD(host, stream, makerNoteCount, makerNoteOffset, offsetDelta, minOffset, maxOffset, tcKodakMakerNote);
    ...
    if (fExif->fMake.StartsWith("Mamiya")) {
        ParseMakerNoteIFD(host, stream, makerNoteCount, makerNoteOffset, offsetDelta, minOffset, maxOffset, tcMamiyaMakerNote);

        // Mamiya uses a MakerNote chain.

        while (fMakerNoteNextIFD) {
            ParseMakerNoteIFD(
                host, stream, makerNoteCount, offsetDelta + fMakerNoteNextIFD, offsetDelta, minOffset, maxOffset, tcMamiyaMakerNote);
        }
    ...
    if (fExif->fMake.StartsWith("Hasselblad")) {
        ParseMakerNoteIFD(host, stream, makerNoteCount, makerNoteOffset, offsetDelta, minOffset, maxOffset, tcHasselbladMakerNote);
    ...
    if (fExif->fMake.StartsWith("CASIO COMPUTER") && memcmp(firstBytes, "QVC\000\000\000", 6) == 0) {
        ParseMakerNoteIFD(host, stream, makerNoteCount - 6, makerNoteOffset + 6, makerNoteOffset, minOffset, maxOffset, tcCasioMakerNote);
...
void dng_info::ParseDNGPrivateData(dng_host &host, dng_stream &stream) {
...

    {
        ...
        uint32 readLength = Min_uint32(fShared->fDNGPrivateDataCount, sizeof(buffer) - 1);
    ...
    if (privateName.StartsWith("PENTAX") || privateName.StartsWith("SAMSUNG")) {
#if qDNGValidate
    ...
    while (section_offset + 8 < fShared->fDNGPrivateDataCount) {
        ...
        if (section_key == DNG_CHAR4('M', 'a', 'k', 'N') && section_count > 6) {
            ...
            dng_stream tempStream(tempBlock->Buffer(), tempSize, positionInOriginalFile);

            tempStream.SetBigEndian(order_mark == byteOrderMM);

            ParseMakerNote(host, tempStream, tempSize, 0, 0 - old_offset, 0, tempSize);
        }
            ...
            ParseSonyPrivateData(host, stream, section_count - 6, old_offset, new_offset);
        }
            ...
            if (tagCount) {
                ...
                ParseTag(host, stream, fExif.Get(), fShared.Get(), NULL, tcFujiRAF, tcFujiHeader, ttUndefined, tagCount, tagOffset, 0);
            ...
            if (tagCount) {
                ...
                ParseTag(host, stream, fExif.Get(), fShared.Get(), NULL, tcFujiRAF, tcFujiRawInfo1, ttUndefined, tagCount, tagOffset, 0);
            ...
            if (tagCount) {
                ...
                ParseTag(host, stream, fExif.Get(), fShared.Get(), NULL, tcFujiRAF, tcFujiRawInfo2, ttUndefined, tagCount, tagOffset, 0);
            ...
            if (tagCount) {
                ...
                ParseTag(host, stream, fExif.Get(), fShared.Get(), NULL, tcContaxRAW, tcContaxHeader, ttUndefined, tagCount, tagOffset, 0);
            }
        }
            ...
            for (uint32 parsePass = 1; parsePass <= 2; parsePass++) {
                ...
                for (uint32 index = 0; index < entries; index++) {
                    ...
                    if ((parsePass == 1) == (tagCode == 0x5834)) {
                        ...
                        ParseTag(host, stream, fExif.Get(), fShared.Get(), NULL, tcCanonCRW, tagCode, ttUndefined, tagCount, tagOffset, 0);
                    }
            ...
            if (parentCode) {
                ...
                for (uint32 index = 0; index < entries; index++) {
                    uint32 tagCode = code32 ? stream.Get_uint32() : stream.Get_uint16();

                    uint32 tagType = hasType ? stream.Get_uint16() : ttUndefined;
                    ...
                    ParseTag(host, stream, fExif.Get(), fShared.Get(), NULL, parentCode, tagCode, tagType, tagCount, tagOffset, 0);
...
void dng_info::Parse(dng_host &host, dng_stream &stream) {
    ...
    ParseIFD(host, stream, fExif.Get(), fShared.Get(), fIFD[0], fTIFFBlockOffset + next_offset, fTIFFBlockOffset, 0);
    ...
    while (next_offset) {
        ...
        if (!ValidateIFD(stream, fTIFFBlockOffset + next_offset, fTIFFBlockOffset)) {
#if qDNGValidate
    ...
    while (searchedIFDs < IFDCount() && !tooManySubIFDs) {
        ...
        for (uint32 searchIndex = searchedIFDs; searchIndex < searchLimit && !tooManySubIFDs; searchIndex++) {
            for (uint32 subIndex = 0; subIndex < fIFD[searchIndex]->fSubIFDsCount; subIndex++) {
                ...
                stream.SetReadPosition(fIFD[searchIndex]->fSubIFDsOffset + subIndex * 4);
    ...
    for (uint32 chainedIndex = 0; chainedIndex < ChainedIFDCount(); chainedIndex++) {
        for (uint32 subIndex = 0; subIndex < fChainedIFD[chainedIndex]->fSubIFDsCount; subIndex++) {
            ...
            stream.SetReadPosition(fChainedIFD[chainedIndex]->fSubIFDsOffset + subIndex * 4);
    ...
    if (fShared->fExifIFD) {
        ParseIFD(host, stream, fExif.Get(), fShared.Get(), NULL, fTIFFBlockOffset + fShared->fExifIFD, fTIFFBlockOffset, tcExifIFD);
    }

    // Parse GPS IFD.

    if (fShared->fGPSInfo) {
        ParseIFD(host, stream, fExif.Get(), fShared.Get(), NULL, fTIFFBlockOffset + fShared->fGPSInfo, fTIFFBlockOffset, tcGPSInfo);
    }
    ...
    if (fShared->fInteroperabilityIFD) {
        ...
        if (ValidateIFD(stream, fTIFFBlockOffset + fShared->fInteroperabilityIFD, fTIFFBlockOffset)) {
            ParseIFD(host,
                     stream,
                     fExif.Get(),
                     fShared.Get(),
    ...
    if (fShared->fDNGPrivateDataCount && fShared->fDNGVersion) {
...
void dng_info::PostParse(dng_host &host) {
    ...
    if (fShared->fDNGVersion != 0) {
        ...
        for (index = 0; index < IFDCount(); index++) {
            if (fIFD[index]->fUsesNewSubFileType && fIFD[index]->fNewSubFileType == sfMainImage) {
                if (fMainIndex == -1) {
                    fMainIndex = index;
                }
            ...
            else if (fIFD[index]->fNewSubFileType == sfPreviewImage || fIFD[index]->fNewSubFileType == sfAltPreviewImage) {
...
bool dng_info::IsValidDNG() {
    ...
    for (uint32 index = 0; index < IFDCount(); index++) {
        ...
        if (!fIFD[index]->IsValidDNG(*fShared.Get(), parentCode)) {
            // Only errors in the main and transparency mask IFDs are fatal to parsing.

            if (index == (uint32)fMainIndex || index == (uint32)fMaskIndex) {
                return false;
            }
