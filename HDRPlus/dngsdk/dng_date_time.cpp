...
dng_date_time::dng_date_time(uint32 year, uint32 month, uint32 day, uint32 hour, uint32 minute, uint32 second)
...
bool dng_date_time::IsValid() const {
    return fYear >= 1 && fYear <= 9999 && fMonth >= 1 && fMonth <= 12 && fDay >= 1 && fDay <= 31 && fHour <= 23 && fMinute <= 59
           && fSecond <= 59;
}
...
void dng_date_time_info::SetDate(uint32 year, uint32 month, uint32 day) {
...
void dng_date_time_info::SetTime(uint32 hour, uint32 minute, uint32 second) {
    fDateOnly = false;
...
void dng_date_time_info::SetOffsetTime(const dng_string &s) {
    ...
    if (s.Length() == 6 && (s.Get()[0] == '+' || s.Get()[0] == '-') && (s.Get()[1] >= '0' && s.Get()[1] <= '1')
        && (s.Get()[2] >= '0' && s.Get()[2] <= '9') && (s.Get()[3] == ':') && (s.Get()[4] >= '0' && s.Get()[4] <= '5')
        && (s.Get()[5] >= '0' && s.Get()[5] <= '9')) {
        int32 hours = (s.Get()[1] - '0') * 10 + (s.Get()[2] - '0');

        int32 minutes = (s.Get()[4] - '0') * 10 + (s.Get()[5] - '0');
...
void dng_date_time_info::Decode_ISO_8601(const char *s) {
    ...
    if (sscanf(s, "%u-%u-%u", &year, &month, &day) != 3) {
        return;
    }

    SetDate((uint32)year, (uint32)month, (uint32)day);
    ...
    for (uint32 j = 0; j < len; j++) {
        if (s[j] == 'T') {
            ...
            int items = sscanf(s + j + 1, "%u:%u:%u", &hour, &minute, &second);

            if (items >= 2 && items <= 3) {
                SetTime((uint32)hour, (uint32)minute, (uint32)second);
                ...
                for (uint32 k = j + 1; k < len; k++) {
                    ...
                    if (s[k] == '+' || s[k] == '-') {
                        ...
                        if (sscanf(s + k + 1, "%u:%u", &tzhour, &tzmin) > 0) {
...
dng_string dng_date_time_info::Encode_ISO_8601() const {
    ...
    if (IsValid()) {
        ...
        sprintf(s, "%04u-%02u-%02u", (unsigned)fDateTime.fYear, (unsigned)fDateTime.fMonth, (unsigned)fDateTime.fDay);

        result.Set(s);

        if (!fDateOnly) {
            sprintf(s, "T%02u:%02u:%02u", (unsigned)fDateTime.fHour, (unsigned)fDateTime.fMinute, (unsigned)fDateTime.fSecond);
            ...
            if (fSubseconds.NotEmpty()) {
                ...
                for (uint32 index = 0; index < len; index++) {
                    if (fSubseconds.Get()[index] < '0' || fSubseconds.Get()[index] > '9') {
...
void dng_date_time_info::Decode_IPTC_Date(const char *s) {
    if (strlen(s) == 8) {
        ...
        if (sscanf(s, "%4u%2u%2u", &year, &month, &day) == 3) {
            SetDate((uint32)year, (uint32)month, (uint32)day);
        }
    }
}
...
void dng_date_time_info::Decode_IPTC_Time(const char *s) {
    if (strlen(s) == 11) {
        ...
        if (time[6] == '+' || time[6] == '-') {
            ...
            if (sscanf(time, "%2u%2u%2u", &hour, &minute, &second) == 3 && sscanf(time + 7, "%2u%2u", &tzhour, &tzmin) == 2) {
                dng_time_zone zone;
        ...
        if (sscanf(s, "%2u%2u%2u", &hour, &minute, &second) == 3) {
            SetTime((uint32)hour, (uint32)minute, (uint32)second);
        }
    }
        ...
        if (sscanf(s, "%2u%2u", &hour, &minute) == 2) {
            SetTime((uint32)hour, (uint32)minute, 0);
        }
    }
}
...
dng_string dng_date_time_info::Encode_IPTC_Time() const {
    ...
    if (IsValid() && !fDateOnly) {
        ...
        if (fTimeZone.IsValid()) {
            ...
            sprintf(s, "%02u%02u%02u", (unsigned)fDateTime.fHour, (unsigned)fDateTime.fMinute, (unsigned)fDateTime.fSecond);
        }
...
void CurrentDateTimeAndZone(dng_date_time_info &info) {
    ...
    bool zonePositive = (t.tm_year > zt.tm_year) || (t.tm_year == zt.tm_year && t.tm_yday > zt.tm_yday)
    ...
    if (dt.IsValid()) {
        ...
        if (zoneRef) {
            ...
            if (CFCalendarComposeAbsoluteTime(
                    calendar, &absTime, "yMdHms", dt.fYear, dt.fMonth, dt.fDay, dt.fHour, dt.fMinute, dt.fSecond)) {
        ...
        if (GetTimeZoneInformation != NULL && SystemTimeToTzSpecificLocalTime != NULL && SystemTimeToFileTime != NULL) {
...
dng_date_time_storage_info::dng_date_time_storage_info(uint64 offset, dng_date_time_format format)
