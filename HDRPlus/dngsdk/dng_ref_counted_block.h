...
private:
    struct header {
        ...
        header(uint32 size) : fMutex(), fRefCount(1), fSize(size) {}
...
public:
    ...
    uint32 LogicalSize() const { return fBuffer ? ((header *)fBuffer)->fSize : 0; }

    void *Buffer() { return fBuffer ? (void *)((char *)fBuffer + sizeof(header)) : NULL; }

    /// Return pointer to allocated memory as a const void *.
    /// \retval const void * valid for as many bytes as were allocated.

    const void *Buffer() const { return fBuffer ? (const void *)((char *)fBuffer + sizeof(header)) : NULL; }
