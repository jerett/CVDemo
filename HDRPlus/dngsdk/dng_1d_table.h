...
public:
    ...
    uint32 Count() const { return fTableCount; }
    ...
    real32 Interpolate(real32 x) const {
        ...
        return fTable[index] * (1.0f - fract) + fTable[index + 1] * (fract);
    }

    /// Direct access function for table data.

    const real32 *Table() const { return fTable; }
...
private:
    void SubDivide(const dng_1d_function &function, uint32 lower, uint32 upper, real32 maxDelta);
};
