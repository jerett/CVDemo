...
void RefZeroBytes(void *dPtr, uint32 count) {
    memset(dPtr, 0, count);
}

/*****************************************************************************/

void RefCopyBytes(const void *sPtr, void *dPtr, uint32 count) {
    memcpy(dPtr, sPtr, count);
}

/*****************************************************************************/

void RefSwapBytes16(uint16 *dPtr, uint32 count) {
...
void RefSwapBytes32(uint32 *dPtr, uint32 count) {
    for (uint32 j = 0; j < count; j++) {
...
void RefSetArea8(uint8 *dPtr, uint8 value, uint32 rows, uint32 cols, uint32 planes, int32 rowStep, int32 colStep, int32 planeStep) {
...
void RefSetArea(destType *dPtr, destType value, uint32 rows, uint32 cols, uint32 planes, int32 rowStep, int32 colStep, int32 planeStep) {
...
template void RefSetArea<Scalar, uint16>(uint16 *dPtr,
                                         uint16 value,
                                         uint32 rows,
                                         uint32 cols,
                                         uint32 planes,
                                         ...
                                         int32 colStep,
                                         int32 planeStep);
template void RefSetArea<Scalar, uint32>(uint32 *dPtr,
                                         uint32 value,
                                         uint32 rows,
                                         uint32 cols,
                                         uint32 planes,
...
INTEL_COMPILER_NEEDED_NOTE
#if !qDNGIntelCompiler
template void RefCopyArea16_S16<Scalar>(const uint16 *sPtr,
                                        int16 *dPtr,
                                        uint32 rows,
                                        uint32 cols,
                                        uint32 planes,
...
void RefRepeatArea8(const uint8 *sPtr,
                    ...
                    uint32 phaseV,
                    uint32 phaseH) {
    const uint8 *sPtr0 = sPtr + phaseV * rowStep + phaseH * colStep;
...
void RefRepeatArea16(const uint16 *sPtr,
                     ...
                     uint32 phaseV,
                     uint32 phaseH) {
    const uint16 *sPtr0 = sPtr + phaseV * rowStep + phaseH * colStep;
...
void RefRepeatArea32(const uint32 *sPtr,
                     ...
                     uint32 phaseV,
                     uint32 phaseH) {
    const uint32 *sPtr0 = sPtr + phaseV * rowStep + phaseH * colStep;
...
void RefShiftRight16(uint16 *dPtr, uint32 rows, uint32 cols, uint32 planes, int32 rowStep, int32 colStep, int32 planeStep, uint32 shift) {
    for (uint32 row = 0; row < rows; row++) {
        uint16 *dPtr1 = dPtr;
...
void RefBaselineHueSatMap(const real32 *sPtrR,
    ...
    lut.GetDivisions(hueDivisions, satDivisions, valDivisions);
    ...
    for (uint32 j = 0; j < count; j++) {
        ...
        if (valDivisions < 2)  // Optimize most common case of "2.5D" table.
        {
            ...
            const dng_hue_sat_map::HSBModify *entry00 = tableBase + hIndex0 * hueStep + sIndex0;

            const dng_hue_sat_map::HSBModify *entry01 = entry00 + (hIndex1 - hIndex0) * hueStep;

            real32 hueShift0 = hFract0 * entry00->fHueShift + hFract1 * entry01->fHueShift;

            real32 satScale0 = hFract0 * entry00->fSatScale + hFract1 * entry01->fSatScale;

            real32 valScale0 = hFract0 * entry00->fValScale + hFract1 * entry01->fValScale;

            entry00++;
            entry01++;

            real32 hueShift1 = hFract0 * entry00->fHueShift + hFract1 * entry01->fHueShift;

            real32 satScale1 = hFract0 * entry00->fSatScale + hFract1 * entry01->fSatScale;

            real32 valScale1 = hFract0 * entry00->fValScale + hFract1 * entry01->fValScale;
            ...
            const dng_hue_sat_map::HSBModify *entry00 = tableBase + vIndex0 * valStep + hIndex0 * hueStep + sIndex0;
            ...
            real32 hueShift0 = vFract0 * (hFract0 * entry00->fHueShift + hFract1 * entry01->fHueShift)
                               + vFract1 * (hFract0 * entry10->fHueShift + hFract1 * entry11->fHueShift);

            real32 satScale0 = vFract0 * (hFract0 * entry00->fSatScale + hFract1 * entry01->fSatScale)
                               + vFract1 * (hFract0 * entry10->fSatScale + hFract1 * entry11->fSatScale);

            real32 valScale0 = vFract0 * (hFract0 * entry00->fValScale + hFract1 * entry01->fValScale)
                               + vFract1 * (hFract0 * entry10->fValScale + hFract1 * entry11->fValScale);
            ...
            real32 hueShift1 = vFract0 * (hFract0 * entry00->fHueShift + hFract1 * entry01->fHueShift)
                               + vFract1 * (hFract0 * entry10->fHueShift + hFract1 * entry11->fHueShift);

            real32 satScale1 = vFract0 * (hFract0 * entry00->fSatScale + hFract1 * entry01->fSatScale)
                               + vFract1 * (hFract0 * entry10->fSatScale + hFract1 * entry11->fSatScale);

            real32 valScale1 = vFract0 * (hFract0 * entry00->fValScale + hFract1 * entry01->fValScale)
                               + vFract1 * (hFract0 * entry10->fValScale + hFract1 * entry11->fValScale);
...
void RefBaseline1DTable(const real32 *sPtr, real32 *dPtr, uint32 count, const dng_1d_table &table) {
    for (uint32 col = 0; col < count; col++) {
        real32 x = sPtr[col];
...
void RefResampleDown16(const uint16 *sPtr,
    ...
    for (uint32 j = 0; j < sCount; j++) {
        ...
        dPtr[j] = (uint16)Pin_int32(0, total >> 14, pixelRange);
    }
}

/*****************************************************************************/

void RefResampleDown32(const real32 *sPtr, real32 *dPtr, uint32 sCount, int32 sRowStep, const real32 *wPtr, uint32 wCount) {
    uint32 col;
    ...
    for (col = 0; col < sCount; col++) {
        dPtr[col] = Pin_real32(0.0f, dPtr[col] + w * sPtr[col], 1.0f);
    }
}
...
void RefResampleAcross16(const uint16 *sPtr,
    ...
    for (uint32 j = 0; j < dCount; j++) {
        ...
        dPtr[j] = (uint16)Pin_int32(0, (total + 8192) >> 14, pixelRange);
    }
}
...
bool RefEqualBytes(const void *sPtr, const void *dPtr, uint32 count) {
...
void RefMapArea16(uint16 *dPtr, uint32 count0, uint32 count1, uint32 count2, int32 step0, int32 step1, int32 step2, const uint16 *map) {
...
void RefBaselineMapPoly32(real32 *dPtr,
    ...
    for (uint32 row = 0; row < rows; row += rowPitch) {
        ...
        switch (degree) {
            case 0: {
                real32 y = Pin_real32(-1.0f, coefficients[0], 1.0f);
