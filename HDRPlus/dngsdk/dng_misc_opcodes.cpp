...
dng_opcode_TrimBounds::dng_opcode_TrimBounds(const dng_rect &bounds)
    ...
    : dng_opcode(dngOpcode_TrimBounds, dngVersion_1_3_0_0, kFlag_None)
...
dng_opcode_TrimBounds::dng_opcode_TrimBounds(dng_stream &stream)
    ...
    : dng_opcode(dngOpcode_TrimBounds, stream, "TrimBounds")
...

{
    ...
    if (gVerbose) {
        printf("Bounds: t=%d, l=%d, b=%d, r=%d\n", (int)fBounds.t, (int)fBounds.l, (int)fBounds.b, (int)fBounds.r);
    }
...
void dng_opcode_TrimBounds::Apply(dng_host & /* host */, dng_negative & /* negative */, AutoPtr<dng_image> &image) {
...
void dng_area_spec::GetData(dng_stream &stream) {
    ...
    if (fArea.IsEmpty()) {
        ...
        if (!SafeInt32Sub(fArea.b, fArea.t, &height) || !SafeInt32Sub(fArea.r, fArea.l, &width) || fRowPitch > static_cast<uint32>(height)
...
dng_opcode_MapTable::dng_opcode_MapTable(dng_host &host, const dng_area_spec &areaSpec, const uint16 *table, uint32 count)

    : dng_inplace_opcode(dngOpcode_MapTable, dngVersion_1_3_0_0, kFlag_None)
...
dng_opcode_MapTable::dng_opcode_MapTable(dng_host &host, dng_stream &stream)

    : dng_inplace_opcode(dngOpcode_MapTable, stream, "MapTable")
...
void dng_opcode_MapTable::ProcessArea(dng_negative & /* negative */,
    ...
    if (overlap.NotEmpty()) {
        const uint16 *table = fBlackAdjustedTable.Get() ? fBlackAdjustedTable->Buffer_uint16() : fTable->Buffer_uint16();

        for (uint32 plane = fAreaSpec.Plane(); plane < fAreaSpec.Plane() + fAreaSpec.Planes() && plane < buffer.Planes(); plane++) {
...
dng_opcode_MapPolynomial::dng_opcode_MapPolynomial(const dng_area_spec &areaSpec, uint32 degree, const real64 *coefficient)

    : dng_inplace_opcode(dngOpcode_MapPolynomial, dngVersion_1_3_0_0, kFlag_None)
...
dng_opcode_MapPolynomial::dng_opcode_MapPolynomial(dng_stream &stream)
    ...
    : dng_inplace_opcode(dngOpcode_MapPolynomial, stream, "MapPolynomial")
...
void dng_opcode_MapPolynomial::ProcessArea(dng_negative &negative,
    ...
    if (overlap.NotEmpty()) {
        ...
        for (uint32 plane = fAreaSpec.Plane(); plane < fAreaSpec.Plane() + fAreaSpec.Planes() && plane < buffer.Planes(); plane++) {
            DoProcess(buffer, overlap, plane, rowPitch, colPitch, fCoefficient32, fDegree, blackLevel);
        }
    }
}
...
void dng_opcode_MapPolynomial::DoProcess(dng_pixel_buffer &buffer,
    ...
    DoBaselineMapPoly32(buffer.DirtyPixel_real32(area.t, area.l, plane),
...
dng_opcode_DeltaPerRow::dng_opcode_DeltaPerRow(const dng_area_spec &areaSpec, AutoPtr<dng_memory_block> &table)

    : dng_inplace_opcode(dngOpcode_DeltaPerRow, dngVersion_1_3_0_0, kFlag_None)
...
dng_opcode_DeltaPerRow::dng_opcode_DeltaPerRow(dng_host &host, dng_stream &stream)

    : dng_inplace_opcode(dngOpcode_DeltaPerRow, stream, "DeltaPerRow")
...

{
    ...
    uint32 deltas = SafeUint32DivideUp(fAreaSpec.Area().H(), fAreaSpec.RowPitch());
    ...
    fTable.Reset(host.Allocate(SafeUint32Mult(deltas, static_cast<uint32>(sizeof(real32)))));
...
void dng_opcode_DeltaPerRow::ProcessArea(dng_negative &negative,
    ...
    if (overlap.NotEmpty()) {
        ...
        for (uint32 plane = fAreaSpec.Plane(); plane < fAreaSpec.Plane() + fAreaSpec.Planes() && plane < buffer.Planes(); plane++) {
            const real32 *table = fTable->Buffer_real32() + ((overlap.t - fAreaSpec.Area().t) / fAreaSpec.RowPitch());
...
dng_opcode_DeltaPerColumn::dng_opcode_DeltaPerColumn(const dng_area_spec &areaSpec, AutoPtr<dng_memory_block> &table)

    : dng_inplace_opcode(dngOpcode_DeltaPerColumn, dngVersion_1_3_0_0, kFlag_None)
...
dng_opcode_DeltaPerColumn::dng_opcode_DeltaPerColumn(dng_host &host, dng_stream &stream)

    : dng_inplace_opcode(dngOpcode_DeltaPerColumn, stream, "DeltaPerColumn")
...

{
    ...
    uint32 deltas = SafeUint32DivideUp(fAreaSpec.Area().W(), fAreaSpec.ColPitch());
...
void dng_opcode_DeltaPerColumn::ProcessArea(dng_negative &negative,
    ...
    if (overlap.NotEmpty()) {
        uint32 rows = (overlap.H() + fAreaSpec.RowPitch() - 1) / fAreaSpec.RowPitch();
        ...
        for (uint32 plane = fAreaSpec.Plane(); plane < fAreaSpec.Plane() + fAreaSpec.Planes() && plane < buffer.Planes(); plane++) {
            const real32 *table = fTable->Buffer_real32() + ((overlap.l - fAreaSpec.Area().l) / fAreaSpec.ColPitch());
...
dng_opcode_ScalePerRow::dng_opcode_ScalePerRow(const dng_area_spec &areaSpec, AutoPtr<dng_memory_block> &table)

    : dng_inplace_opcode(dngOpcode_ScalePerRow, dngVersion_1_3_0_0, kFlag_None)
...
dng_opcode_ScalePerRow::dng_opcode_ScalePerRow(dng_host &host, dng_stream &stream)

    : dng_inplace_opcode(dngOpcode_ScalePerRow, stream, "ScalePerRow")
...

{
    ...
    uint32 scales = SafeUint32DivideUp(fAreaSpec.Area().H(), fAreaSpec.RowPitch());
    ...
    fTable.Reset(host.Allocate(SafeUint32Mult(scales, static_cast<uint32>(sizeof(real32)))));
...
dng_opcode_ScalePerColumn::dng_opcode_ScalePerColumn(const dng_area_spec &areaSpec, AutoPtr<dng_memory_block> &table)

    : dng_inplace_opcode(dngOpcode_ScalePerColumn, dngVersion_1_3_0_0, kFlag_None)
...
dng_opcode_ScalePerColumn::dng_opcode_ScalePerColumn(dng_host &host, dng_stream &stream)

    : dng_inplace_opcode(dngOpcode_ScalePerColumn, stream, "ScalePerColumn")
...

{
    ...
    uint32 scales = SafeUint32DivideUp(fAreaSpec.Area().W(), fAreaSpec.ColPitch());
