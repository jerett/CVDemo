...
void dng_xmp_sdk::InitializeSDK(dng_xmp_namespace *extraNamespaces, const char *software) {
    if (!gInitializedXMP) {
        try {
...

            {
                ...
                SXMPMeta::RegisterNamespace(XMP_NS_CRX, "crx", &ss);
            }
...

            {
                ...
                SXMPMeta::RegisterNamespace(XMP_NS_CRSS, "crss", &ss);
            }
...

            {
                ...
                SXMPMeta::RegisterNamespace(XMP_NS_CRD, "crd", &ss);
            }
...

            {
                ...
                SXMPMeta::RegisterNamespace(XMP_NS_LCP, "stCamera", &ss);
            }
...

            {
                ...
                SXMPMeta::RegisterNamespace(XMP_NS_DNG, "dng", &ss);
            }
...

            {
                ...
                SXMPMeta::RegisterNamespace(XMP_NS_PANO, "panorama", &ss);
            }
            ...
            if (extraNamespaces != NULL) {
                for (; extraNamespaces->fullName != NULL; ++extraNamespaces) {
                    ...
                    SXMPMeta::RegisterNamespace(extraNamespaces->fullName, extraNamespaces->shortName, &ss);
                }
            }
...
void dng_xmp_sdk::Parse(dng_host &host, const char *buffer, uint32 count) {
    MakeMeta();
...
void dng_xmp_sdk::AppendArrayItem(const char *ns, const char *arrayName, const char *itemValue, bool isBag, bool propIsStruct) {
    NeedMeta();

    try {
        fPrivate->fMeta->AppendArrayItem(
            ns, arrayName, isBag ? kXMP_PropValueIsArray : kXMP_PropArrayIsOrdered, itemValue, propIsStruct ? kXMP_PropValueIsStruct : 0);
    }
...
int32 dng_xmp_sdk::CountArrayItems(const char *ns, const char *path) const {
    if (HasMeta()) {
        try {
...
bool dng_xmp_sdk::Exists(const char *ns, const char *path) const {
    if (HasMeta()) {
        try {
...
bool dng_xmp_sdk::HasNameSpace(const char *ns) const {
    ...
    if (HasMeta()) {
        try {
            ...
            if (iter.Next(&nsTemp, &prop, NULL, NULL)) {
                result = true;
            }
        }
...
void dng_xmp_sdk::Remove(const char *ns, const char *path) {
    if (HasMeta()) {
        try {
...
void dng_xmp_sdk::RemoveProperties(const char *ns) {
    if (HasMeta()) {
        try {
            SXMPUtils::RemoveProperties(fPrivate->fMeta, ns, NULL, kXMPUtil_DoAllProperties);
        }
...
bool dng_xmp_sdk::IsEmptyString(const char *ns, const char *path) {
    if (HasMeta()) {
        try {
            TXMP_STRING_TYPE ss;
            ...
            if (fPrivate->fMeta->GetProperty(ns, path, &ss, &options)) {
                // Item must be simple.

                if (XMP_PropIsSimple(options)) {
                    // Check for null strings.

                    return (ss.c_str() == 0 || ss.c_str()[0] == 0);
                }
            }
        }
...
bool dng_xmp_sdk::IsEmptyArray(const char *ns, const char *path) {
    if (HasMeta()) {
        try {
            TXMP_STRING_TYPE ss;
            ...
            if (fPrivate->fMeta->GetProperty(ns, path, &ss, &options)) {
                if (XMP_PropIsArray(options)) {
                    if (fPrivate->fMeta->GetArrayItem(ns, path, 1, &ss, &options)) {
                        // If the first item is a null string...

                        if (XMP_PropIsSimple(options)) {
                            if ((ss.c_str() == 0 || ss.c_str()[0] == 0)) {
                                // And there is no second item.

                                if (!fPrivate->fMeta->GetArrayItem(ns, path, 2, &ss, &options)) {
...
void dng_xmp_sdk::ComposeArrayItemPath(const char *ns, const char *arrayName, int32 index, dng_string &s) const {
    try {
        std::string ss;
...
void dng_xmp_sdk::ComposeStructFieldPath(const char *ns,
    ...
    try {
        ...
        SXMPUtils::ComposeStructFieldPath(ns, structName, fieldNS, fieldName, &ss);
...
bool dng_xmp_sdk::GetNamespacePrefix(const char *uri, dng_string &s) const {
    bool result = false;
...
bool dng_xmp_sdk::GetString(const char *ns, const char *path, dng_string &s) const {
    bool result = false;
...
void dng_xmp_sdk::ValidateStringList(const char *ns, const char *path) {
    if (Exists(ns, path)) {
        bool bogus = true;
        ...
        try {
            ...
            while (fPrivate->fMeta->GetArrayItem(ns, path, index++, &ss, NULL)) {
            }
...
bool dng_xmp_sdk::GetStringList(const char *ns, const char *path, dng_string_list &list) const {
    bool result = false;
...
bool dng_xmp_sdk::GetAltLangDefault(const char *ns, const char *path, dng_string &s, bool silent) const {
    bool result = false;
    ...
    if (HasMeta()) {
        try {
            ...
            if (fPrivate->fMeta->GetLocalizedText(ns, path, "x-default", "x-default", NULL, &ss, NULL)) {
                s.Set(ss.c_str());
            ...
            else if (fPrivate->fMeta->GetProperty(ns, path, &ss, NULL)) {
                if (ss.empty()) {
                    s.Clear();
...
bool dng_xmp_sdk::GetLocalString(const char *ns, const char *path, dng_local_string &s) const {
        ...
        try {
            ...
            if (count > 1) {
                for (int32 index = 1; index <= count; index++) {
                    ...
                    ComposeArrayItemPath(ns, path, index + 1, arrayItemPath);

                    TXMP_STRING_TYPE langS;

                    if (fPrivate->fMeta->GetQualifier(ns, arrayItemPath.Get(), kXMP_NS_XML, "lang", &langS, NULL)) {
                        dng_string language;

                        language.Set(langS.c_str());

                        if (language.IsEmpty() || language.Matches("x-default")) {
                            continue;
                        }

                        TXMP_STRING_TYPE tranS;

                        if (fPrivate->fMeta->GetProperty(ns, arrayItemPath.Get(), &tranS, NULL)) {
                            dng_string translation;

                            translation.Set(tranS.c_str());

                            s.AddTranslation(language, translation);
                        }
                    }
                }
            }
...
bool dng_xmp_sdk::GetStructField(const char *ns, const char *path, const char *fieldNS, const char *fieldName, dng_string &s) const {
    bool result = false;
    ...
    if (HasMeta()) {
        try {
            ...
            if (fPrivate->fMeta->GetStructField(ns, path, fieldNS, fieldName, &ss, NULL)) {
                s.Set(ss.c_str());
...
void dng_xmp_sdk::Set(const char *ns, const char *path, const char *text) {
    NeedMeta();
...
void dng_xmp_sdk::SetString(const char *ns, const char *path, const dng_string &s) {
    dng_string ss(s);
...
void dng_xmp_sdk::SetStringList(const char *ns, const char *path, const dng_string_list &list, bool isBag) {
    ...
    if (list.Count()) {
        ...
        for (uint32 index = 0; index < list.Count(); index++) {
            ...
            try {
                fPrivate->fMeta->AppendArrayItem(ns, path, isBag ? kXMP_PropValueIsArray : kXMP_PropArrayIsOrdered, s.Get());
            }
...
void dng_xmp_sdk::SetAltLangDefault(const char *ns, const char *path, const dng_string &s) {
    NeedMeta();
    ...
    try {
        fPrivate->fMeta->SetLocalizedText(ns, path, "x-default", "x-default", ss.Get());
    }
...
void dng_xmp_sdk::SetLocalString(const char *ns, const char *path, const dng_local_string &s) {
    ...
    try {
        for (uint32 index = 0; index < s.TranslationCount(); index++) {
            ...
            ComposeArrayItemPath(ns, path, index + 2, arrayItemPath);

            fPrivate->fMeta->SetProperty(ns, arrayItemPath.Get(), s.Translation(index).Get());

            fPrivate->fMeta->SetQualifier(ns, arrayItemPath.Get(), kXMP_NS_XML, "lang", s.Language(index).Get(), 0);
        }
    }
...
void dng_xmp_sdk::SetStructField(const char *ns, const char *path, const char *fieldNS, const char *fieldName, const char *text) {
    NeedMeta();

    try {
        fPrivate->fMeta->SetStructField(ns, path, fieldNS, fieldName, text);
    }
...
void dng_xmp_sdk::DeleteStructField(const char *ns, const char *structName, const char *fieldNS, const char *fieldName) {
    if (HasMeta()) {
        try {
...
dng_memory_block *dng_xmp_sdk::Serialize(dng_memory_allocator &allocator,
    ...
    if (HasMeta()) {
        ...
        if (asPacket && targetBytes) {
            try {
                fPrivate->fMeta->SerializeToBuffer(&s, formatOption | kXMP_ExactPacketLength, targetBytes, "", " ");
        ...
        if (!havePacket) {
            try {
                fPrivate->fMeta->SerializeToBuffer(
                    &s, formatOption | (asPacket ? 0 : kXMP_OmitPacketWrapper), (asPacket ? padBytes : 0), "", " ");
            }
        ...
        if (forJPEG && asPacket && padBytes > 0 && targetBytes <= kJPEG_XMP_Limit && packetLen > kJPEG_XMP_Limit) {
            ...
            try {
                fPrivate->fMeta->SerializeToBuffer(&s, formatOption, padBytes, "", " ");
            }
...
void dng_xmp_sdk::PackageForJPEG(dng_memory_allocator &allocator,
    ...
    if (HasMeta()) {
        ...
        try {
            SXMPUtils::PackageForJPEG(*fPrivate->fMeta, &stdStr, &extStr, &digestStr);
        }
...
void dng_xmp_sdk::MergeFromJPEG(const dng_xmp_sdk *xmp) {
    if (xmp && xmp->HasMeta()) {
        ...
        try {
            SXMPUtils::MergeFromJPEG(fPrivate->fMeta, *xmp->fPrivate->fMeta);
        }
...
bool dng_xmp_sdk::IteratePaths(IteratePathsCallback *callback, void *callbackData, const char *startingNS, const char *startingPath) {
    if (HasMeta()) {
        try {
            ...
            while (iter.Next(&ns, &prop, NULL, NULL)) {
                if (!callback(ns.c_str(), prop.c_str(), callbackData)) {
                    return false;
                }
            }
        }
...
void dng_xmp_sdk::DocOpsOpenXMP(const char *srcMIME) {
    if (srcMIME[0]) {
        ...
        try {
            ...
            docOps.OpenXMP(fPrivate->fMeta, srcMIME);
        }

        CATCH_XMP("DocOpsOpenXMP", false)

        Set(XMP_NS_DC, "format", srcMIME);
    }
}

/*****************************************************************************/

void dng_xmp_sdk::DocOpsPrepareForSave(const char *srcMIME, const char *dstMIME, bool newPath) {
    NeedMeta();

    try {
        SXMPDocOps docOps;

        docOps.OpenXMP(fPrivate->fMeta, srcMIME, "old path");

        docOps.NoteChange(kXMP_Part_All);

        docOps.PrepareForSave(dstMIME, newPath ? "new path" : "old path");
    }

    CATCH_XMP("DocOpsPrepareForSave", false)

    Set(XMP_NS_DC, "format", dstMIME);
}
