...
dng_matrix::dng_matrix(uint32 rows, uint32 cols)

    : fRows(0),
      fCols(0)

{
    if (rows < 1 || rows > kMaxColorPlanes || cols < 1 || cols > kMaxColorPlanes) {
        ThrowProgramError();
    }
...
bool dng_matrix::AlmostEqual(const dng_matrix &m, real64 slop) const {
    if (Rows() != m.Rows() || Cols() != m.Cols()) {
        return false;
    }
...
dng_matrix_3by3::dng_matrix_3by3(real64 a00, real64 a01, real64 a02, real64 a10, real64 a11, real64 a12, real64 a20, real64 a21, real64 a22)
...
dng_matrix_4by3::dng_matrix_4by3(real64 a00,
                                 real64 a01,
                                 real64 a02,
                                 real64 a10,
                                 real64 a11,
...

{
    ...
    const bool is3by3 = (m.Rows() == 3 && m.Cols() == 3);

    const bool is4by4 = (m.Rows() == 4 && m.Cols() == 4);
...
dng_matrix_4by4::dng_matrix_4by4(real64 a00, real64 a11, real64 a22, real64 a33)
...
dng_vector_3::dng_vector_3(real64 a0, real64 a1, real64 a2)
...
dng_vector_4::dng_vector_4(real64 a0, real64 a1, real64 a2, real64 a3)
...
dng_vector operator-(const dng_vector &a, const dng_vector &b) {
    ...
    DNG_REQUIRE(count == b.Count(), "Mismatch count in Dot");
...
static dng_matrix Invert3by3(const dng_matrix &A) {
    ...
    real64 det = (a00 * temp[0][0] + a01 * temp[1][0] + a02 * temp[2][0]);
...
dng_matrix Invert(const dng_matrix &A, const dng_matrix &hint) {
    if (A.Rows() == A.Cols() || A.Rows() != hint.Cols() || A.Cols() != hint.Rows()) {
        return Invert(A);
    }
...
real64 Dot(const dng_vector &a, const dng_vector &b) {
    DNG_REQUIRE(a.Count() == b.Count(), "Cannot take dot product between vectors of different size.");
