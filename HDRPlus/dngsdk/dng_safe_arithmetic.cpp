...
T SafeAdd(T arg1, T arg2) {
    ...
    if ((arg1 >= 0 && arg2 <= std::numeric_limits<T>::max() - arg1) || (arg1 < 0 && arg2 >= std::numeric_limits<T>::min() - arg1)) {
        return arg1 + arg2;
    }
    else {
...
bool ConvertUint32ToInt32(uint32 val, int32 *result) {
    const uint32 kInt32MaxAsUint32 = static_cast<uint32>(std::numeric_limits<int32>::max());
