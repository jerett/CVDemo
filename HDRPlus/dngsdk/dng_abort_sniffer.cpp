...
public:
    ...
    void Increment(dng_priority priority, const char *name);

    void Decrement(dng_priority priority, const char *name);
...
private:
    dng_priority MinPriority() {
        ...
        for (uint32 level = dng_priority_maximum; level > dng_priority_minimum; level--) {
            if (fCounter[level]) {
...
{
    for (uint32 level = dng_priority_minimum; level <= dng_priority_maximum; level++) {
        fCounter[level] = 0;
    }
}
...
void dng_priority_manager::Increment(dng_priority priority, const char *name) {
...
void dng_priority_manager::Decrement(dng_priority priority, const char *name) {
...
dng_set_minimum_priority::dng_set_minimum_priority(dng_priority priority, const char *name)
...
void dng_abort_sniffer::StartTask(const char * /* name */, real64 /* fract */) {}
