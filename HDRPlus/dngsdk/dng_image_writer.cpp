...
static void SpoolAdobeData(dng_stream &stream,
    ...
    if (metadata && metadata->GetXMP()) {
        ...
        if (metadata->GetXMP()->GetBoolean(XMP_NS_XAP_RIGHTS, "Marked", marked)) {
        ...
        if (metadata->GetXMP()->GetString(XMP_NS_XAP_RIGHTS, "WebStatement", webStatement)) {
...
static dng_memory_block *BuildAdobeData(dng_host &host,
    ...
    SpoolAdobeData(stream, metadata, preview, imageResources);
...
tag_string::tag_string(uint16 code, const dng_string &s, bool forceASCII)
...
tag_encoded_text::tag_encoded_text(uint16 code, const dng_string &text)
...
tag_matrix::tag_matrix(uint16 code, const dng_matrix &m)
...
tag_icc_profile::tag_icc_profile(const void *profileData, uint32 profileSize)

    : tag_data_ptr(tcICCProfile, ttUndefined, 0, NULL)
...
tag_exif_date_time::tag_exif_date_time(uint16 code, const dng_date_time &dt)
...
tag_iptc::tag_iptc(const void *data, uint32 length)
...
void dng_tiff_directory::Put(dng_stream &stream, OffsetsBase offsetsBase, uint32 explicitBase) const {
    if (!fEntries)
        return;
...
dng_basic_tag_set::dng_basic_tag_set(dng_tiff_directory &directory, const dng_ifd &info)
    ...
    : fNewSubFileType(tcNewSubFileType, info.fNewSubFileType)
      ...
      ,
      fPhotoInterpretation(tcPhotometricInterpretation, (uint16)info.fPhotometricInterpretation)
      ...
      ,
      fTileLength(fStrips ? tcRowsPerStrip : tcTileLength, info.fTileLength)
      ...
      ,
      fTileOffsets(fStrips ? tcStripOffsets : tcTileOffsets, fTileOffsetData, info.TilesPerImage())

      ,
      fTileByteCountData(fTileOffsetData + info.TilesPerImage())

      ,
      fTileByteCounts(fStrips ? tcStripByteCounts : tcTileByteCounts, fTileByteCountData, info.TilesPerImage())
      ...
      ,
      fExtraSamples(tcExtraSamples, fExtraSamplesData, info.fExtraSamplesCount)

      ,
      fSampleFormat(tcSampleFormat, fSampleFormatData, info.fSamplesPerPixel)

      ,
      fRowInterleaveFactor(tcRowInterleaveFactor, (uint16)info.fRowInterleaveFactor)

      ,
      fSubTileBlockSize(tcSubTileBlockSize, fSubTileBlockSizeData, 2)

{
    ...
    if (info.fBitsPerSample[0] != 8 && info.fBitsPerSample[0] != 16 && info.fBitsPerSample[0] != 32) {
    ...
    if (info.fSubTileBlockRows != 1 || info.fSubTileBlockCols != 1) {
...
exif_tag_set::exif_tag_set(dng_tiff_directory &directory,
    ...
    : fExifIFD(),
      ...
      ,
      fISOSpeedRatings(tcISOSpeedRatings, (uint16)Min_uint32(65535, exif.fISOSpeedRatings[0]))
      ...
      ,
      fCFAPattern(tcCFAPatternExif, exif.fCFARepeatPatternRows, exif.fCFARepeatPatternCols, &exif.fCFAPattern[0][0])
...

{
    ...
    if (exif.fCFARepeatPatternRows && exif.fCFARepeatPatternCols) {
    ...
    if (exif.fColorSpace == 1 || exif.fColorSpace == 0xFFFF) {
    ...
    if (exif.fLensInfo[0].IsValid() && exif.fLensInfo[1].IsValid()) {
    ...
    if (exif.fImageUniqueID.IsValid()) {
        for (uint32 j = 0; j < 16; j++) {
            sprintf(fImageUniqueIDData + j * 2, "%02X", (unsigned)exif.fImageUniqueID.data[j]);
        }
    ...
    if (exif.AtLeastVersion0230()) {
        ...
        if (exif.fSensitivityType != 0 && exif.fISOSpeedRatings[0] != 0) {
            ...
            if (exif.fISOSpeed != 0) {
                ...
                if (exif.fISOSpeedLatitudeyyy != 0 && exif.fISOSpeedLatitudezzz != 0) {
    ...
    if (exif.AtLeastVersion0231()) {
        if (exif.fDateTime.IsValid() && exif.fDateTime.TimeZone().IsValid()) {
            fExifIFD.Add(&fOffsetTime);
        }

        if (exif.fDateTimeOriginal.IsValid() && exif.fDateTimeOriginal.TimeZone().IsValid()) {
            fExifIFD.Add(&fOffsetTimeOriginal);
        }

        if (exif.fDateTimeDigitized.IsValid() && exif.fDateTimeDigitized.TimeZone().IsValid()) {
    ...
    if (exif.fGPSLatitudeRef.NotEmpty() && exif.fGPSLatitude[0].IsValid()) {
        fGPSIFD.Add(&fGPSLatitudeRef);
        fGPSIFD.Add(&fGPSLatitude);
    }

    if (exif.fGPSLongitudeRef.NotEmpty() && exif.fGPSLongitude[0].IsValid()) {
    ...
    if (exif.fGPSDestLatitudeRef.NotEmpty() && exif.fGPSDestLatitude[0].IsValid()) {
        fGPSIFD.Add(&fGPSDestLatitudeRef);
        fGPSIFD.Add(&fGPSDestLatitude);
    }

    if (exif.fGPSDestLongitudeRef.NotEmpty() && exif.fGPSDestLongitude[0].IsValid()) {
...
private:
    ...
    dng_urational fBlackLevelData[kMaxBlackPattern * kMaxBlackPattern * kMaxSamplesPerPixel];
...
range_tag_set::range_tag_set(dng_tiff_directory &directory, const dng_negative &negative)

    : fActiveArea(tcActiveArea, fActiveAreaData, 4)

      ,
      fMaskedAreas(tcMaskedAreas, fMaskedAreaData, 0)

      ,
      fLinearizationTable(tcLinearizationTable, NULL, 0)

      ,
      fBlackLevelRepeatDim(tcBlackLevelRepeatDim, fBlackLevelRepeatDimData, 2)

      ,
      fBlackLevel(tcBlackLevel, fBlackLevelData)
      ...
      ,
      fWhiteLevel16(tcWhiteLevel, fWhiteLevelData16)

      ,
      fWhiteLevel32(tcWhiteLevel, fWhiteLevelData32)

{
    ...
    if (rangeInfo) {
...

        {
            ...
            fBlackLevel.SetCount(rangeInfo->fBlackLevelRepeatRows * rangeInfo->fBlackLevelRepeatCols * rawImage.Planes());
...
mosaic_tag_set::mosaic_tag_set(dng_tiff_directory &directory, const dng_mosaic_info &info)

    : fCFARepeatPatternDim(tcCFARepeatPatternDim, fCFARepeatPatternDimData, 2)

      ,
      fCFAPattern(tcCFAPattern, fCFAPatternData)

      ,
      fCFAPlaneColor(tcCFAPlaneColor, fCFAPlaneColorData)

      ,
      fCFALayout(tcCFALayout, (uint16)info.fCFALayout)

      ,
      fGreenSplit(tcBayerGreenSplit, info.fBayerGreenSplit)

{
    if (info.IsColorFilterArray()) {
        ...
        fCFAPattern.SetCount(info.fCFAPatternSize.v * info.fCFAPatternSize.h);
        ...
        if (info.fCFAPatternSize == dng_point(2, 2) && info.fColorPlanes == 3) {
...
color_tag_set::color_tag_set(dng_tiff_directory &directory, const dng_negative &negative)

    : fColorChannels(negative.ColorChannels())

      ,
      fCameraCalibration1(tcCameraCalibration1, negative.CameraCalibration1())

      ,
      fCameraCalibration2(tcCameraCalibration2, negative.CameraCalibration2())

      ,
      fCameraCalibrationSignature(tcCameraCalibrationSignature, negative.CameraCalibrationSignature())

      ,
      fAsShotProfileName(tcAsShotProfileName, negative.AsShotProfileName())

      ,
      fAnalogBalance(tcAnalogBalance, fAnalogBalanceData, fColorChannels)

      ,
      fAsShotNeutral(tcAsShotNeutral, fAsShotNeutralData, fColorChannels)

      ,
      fAsShotWhiteXY(tcAsShotWhiteXY, fAsShotWhiteXYData, 2)

      ,
      fLinearResponseLimit(tcLinearResponseLimit, negative.LinearResponseLimitR())

{
    if (fColorChannels > 1) {
        ...
        if (fCameraCalibration1.Count() == channels2 || fCameraCalibration2.Count() == channels2) {
        ...
        if (negative.HasCameraNeutral()) {
            ...
            negative.GetCameraWhiteXY(fAsShotWhiteXYData[0], fAsShotWhiteXYData[1]);
...
profile_tag_set::profile_tag_set(dng_tiff_directory &directory, const dng_camera_profile &profile)

    : fCalibrationIlluminant1(tcCalibrationIlluminant1, (uint16)profile.CalibrationIlluminant1())

      ,
      fCalibrationIlluminant2(tcCalibrationIlluminant2, (uint16)profile.CalibrationIlluminant2())

      ,
      fColorMatrix1(tcColorMatrix1, profile.ColorMatrix1())

      ,
      fColorMatrix2(tcColorMatrix2, profile.ColorMatrix2())

      ,
      fForwardMatrix1(tcForwardMatrix1, profile.ForwardMatrix1())

      ,
      fForwardMatrix2(tcForwardMatrix2, profile.ForwardMatrix2())

      ,
      fReductionMatrix1(tcReductionMatrix1, profile.ReductionMatrix1())

      ,
      fReductionMatrix2(tcReductionMatrix2, profile.ReductionMatrix2())

      ,
      fProfileName(tcProfileName, profile.Name(), false)

      ,
      fProfileCalibrationSignature(tcProfileCalibrationSignature, profile.ProfileCalibrationSignature(), false)

      ,
      fEmbedPolicyTag(tcProfileEmbedPolicy, profile.EmbedPolicy())

      ,
      fCopyrightTag(tcProfileCopyright, profile.Copyright(), false)

      ,
      fHueSatMapDims(tcProfileHueSatMapDims, fHueSatMapDimData, 3)

      ,
      fHueSatData1(tcProfileHueSatMapData1, ttFloat, profile.HueSatDeltas1().DeltasCount() * 3, profile.HueSatDeltas1().GetConstDeltas())

      ,
      fHueSatData2(tcProfileHueSatMapData2, ttFloat, profile.HueSatDeltas2().DeltasCount() * 3, profile.HueSatDeltas2().GetConstDeltas())

      ,
      fHueSatMapEncodingTag(tcProfileHueSatMapEncoding, profile.HueSatMapEncoding())

      ,
      fLookTableDims(tcProfileLookTableDims, fLookTableDimData, 3)

      ,
      fLookTableData(tcProfileLookTableData, ttFloat, profile.LookTable().DeltasCount() * 3, profile.LookTable().GetConstDeltas())

      ,
      fLookTableEncodingTag(tcProfileLookTableEncoding, profile.LookTableEncoding())

      ,
      fBaselineExposureOffsetTag(tcBaselineExposureOffset, profile.BaselineExposureOffset())

      ,
      fDefaultBlackRenderTag(tcDefaultBlackRender, profile.DefaultBlackRender())

      ,
      fToneCurveBuffer()

      ,
      fToneCurveTag(tcProfileToneCurve, ttFloat, 0, NULL)

{
    if (profile.HasColorMatrix1()) {
        ...
        bool haveHueSat2 = profile.HueSatDeltas2().IsValid() && profile.HasColorMatrix2();
        ...
        if (haveHueSat1 || haveHueSat2) {
            ...
            if (haveHueSat1) {
                profile.HueSatDeltas1().GetDivisions(hueDivs, satDivs, valDivs);
            }

            else {
                profile.HueSatDeltas2().GetDivisions(hueDivs, satDivs, valDivs);
            }
        ...
        if (profile.HasLookTable()) {
            ...
            profile.LookTable().GetDivisions(hueDivs, satDivs, valDivs);
        ...
        if (profile.ToneCurve().IsValid()) {
            ...
            fToneCurveBuffer.Allocate(dng_safe_uint32(toneCurvePoints) * 2u, sizeof(real32));
...
tiff_dng_extended_color_profile::tiff_dng_extended_color_profile(const dng_camera_profile &profile)
...
void tiff_dng_extended_color_profile::Put(dng_stream &stream, bool includeModelRestriction) {
    // Profile header.
    ...
    tag_string cameraModelTag(tcUniqueCameraModel, fProfile.UniqueCameraModelRestriction());
...
tag_dng_noise_profile::tag_dng_noise_profile(const dng_noise_profile &profile)
    ...
    : tag_data_ptr(tcNoiseProfile, ttDouble, 2 * profile.NumFunctions(), fValues)

{
    DNG_REQUIRE(profile.NumFunctions() <= kMaxColorPlanes, "Too many noise functions in tag_dng_noise_profile.");
...
uint32 dng_image_writer::CompressedBufferSize(const dng_ifd &ifd, uint32 uncompressedSize) {
...
static void EncodeDelta8(uint8 *dPtr, uint32 rows, uint32 cols, uint32 channels) {
...
static void EncodeDelta16(uint16 *dPtr, uint32 rows, uint32 cols, uint32 channels) {
    const uint32 dRowStep = cols * channels;
...
static void EncodeDelta32(uint32 *dPtr, uint32 rows, uint32 cols, uint32 channels) {
    const uint32 dRowStep = cols * channels;
...
static void EncodeFPDelta(uint8 *buffer, uint8 *temp, int32 cols, int32 channels, int32 bytesPerSample) {
    ...
    switch (ifd.fPredictor) {
        ...
        case cpHorizontalDifferenceX4: {
            ...
            switch (buffer.fPixelType) {
                case ttByte: {
                    EncodeDelta8((uint8 *)buffer.fData, buffer.fArea.H(), buffer.fArea.W() / xFactor, buffer.fPlanes * xFactor);

                    return;
                }

                case ttShort: {
                    EncodeDelta16((uint16 *)buffer.fData, buffer.fArea.H(), buffer.fArea.W() / xFactor, buffer.fPlanes * xFactor);

                    return;
                }

                case ttLong: {
                    EncodeDelta32((uint32 *)buffer.fData, buffer.fArea.H(), buffer.fArea.W() / xFactor, buffer.fPlanes * xFactor);
        ...
        case cpFloatingPointX4: {
            ...
            dng_safe_uint32 tempBufferSize = dng_safe_uint32(buffer.fPixelSize) * static_cast<uint32>(buffer.fRowStep);

            if (!tempBuffer.Get() || tempBuffer->LogicalSize() < tempBufferSize.Get()) {
...
void dng_image_writer::ByteSwapBuffer(dng_host & /* host */, dng_pixel_buffer &buffer) {
    uint32 pixels = buffer.fRowStep * buffer.fArea.H();

    switch (buffer.fPixelSize) {
        case 2: {
            DoSwapBytes16((uint16 *)buffer.fData, pixels);

            break;
        }

        case 4: {
            DoSwapBytes32((uint32 *)buffer.fData, pixels);
...
void dng_image_writer::ReorderSubTileBlocks(const dng_ifd &ifd,
    ...
    DoCopyBytes(subTileBlockBuffer->Buffer(), uncompressedBuffer->Buffer(), uncompressedBuffer->LogicalSize());
}
...
private:
    enum { kResetCode = 256, kEndCode = 257, kTableSize = 4096 };
...
private:
    ...
    int32 SearchTable(int32 w, int32 k) const {
        DNG_ASSERT((w >= 0) && (w <= kTableSize), "Bad w value in dng_lzw_compressor::SearchTable");
...
void dng_lzw_compressor::AddTable(int32 w, int32 k) {
    DNG_ASSERT((w >= 0) && (w <= kTableSize), "Bad w value in dng_lzw_compressor::AddTable");

    LZWCompressorNode *node = &fTable[w];

    int32 nextCode = fNextCode;

    DNG_ASSERT((nextCode >= 0) && (nextCode <= kTableSize), "Bad fNextCode value in dng_lzw_compressor::AddTable");
...
void dng_lzw_compressor::Compress(const uint8 *sPtr, uint8 *dPtr, uint32 sCount, uint32 &dCount) {
    fDstPtr = dPtr;
...
static void jpeg_set_adobe_quality(struct jpeg_compress_struct *cinfo, int32 quality) {
    ...
    const int kLibJPEGQuality[13] = {5, 11, 23, 34, 46, 63, 76, 77, 86, 90, 94, 97, 99};
    ...
    switch (ifd.fCompression) {
        case ccUncompressed: {
            ...
            if (ifd.fBitsPerSample[0] == 8 && buffer.fPixelType == ttShort) {
                uint32 count = buffer.fRowStep * buffer.fArea.H();
                ...
                stream.Put(buffer.fData, buffer.fRowStep * buffer.fArea.H() * buffer.fPixelSize);
            }
        ...
        case ccDeflate: {
            ...
            uint32 sBytes = buffer.fRowStep * buffer.fArea.H() * buffer.fPixelSize;
            ...
            if (ifd.fCompression == ccLZW) {
                ...
                lzwCompressor.Compress(sBuffer, dBuffer, sBytes, dBytes);
            }
                ...
                if (ifd.fCompressionQuality >= Z_BEST_SPEED && ifd.fCompressionQuality <= Z_BEST_COMPRESSION) {
                    level = ifd.fCompressionQuality;
                }

                int zResult = ::compress2(dBuffer, &dCount, sBuffer, sBytes, level);
        ...
        case ccJPEG: {
            ...
            if (buffer.fPixelType == ttByte) {
                ...
                temp.CopyArea(buffer, buffer.fArea, buffer.fPlane, buffer.fPlanes);
            }
        ...
        case ccLossyJPEG: {
            ...
            try {
                ...
                for (int32 row = buffer.fArea.t; row < buffer.fArea.b; row++) {
                    ...
                    sampArray[0] = buffer.DirtyPixel_uint8(row, buffer.fArea.l, 0);
        ...
        default: { ThrowProgramError(); }
    }
}

/******************************************************************************/

void dng_image_writer::EncodeJPEGPreview(dng_host &host, const dng_image &image, dng_jpeg_preview &preview, int32 quality) {
#if qDNGUseLibJPEG
    ...
    try {
        ...
        dng_pixel_buffer buffer(image.Bounds(), 0, image.Planes(), ttByte, pcInterleaved, NULL);
...
void dng_image_writer::WriteTile(dng_host &host,
    ...
    dng_pixel_buffer buffer(tileArea, 0, ifd.fSamplesPerPixel, image.PixelType(), pcInterleaved, uncompressedBuffer->Buffer());
    ...
    if (ifd.fSubTileBlockRows > 1) {
        ReorderSubTileBlocks(ifd, buffer, uncompressedBuffer, subTileBlockBuffer);
    }
    ...
    if (ifd.fSampleFormat[0] == sfFloatingPoint) {
        ...
        if (ifd.fBitsPerSample[0] == 24) {
            ...
            if (stream.BigEndian() || ifd.fPredictor == cpFloatingPoint || ifd.fPredictor == cpFloatingPointX2
    ...
    EncodePredictor(host, ifd, buffer, tempBuffer);
    ...
    WriteData(host, ifd, stream, buffer, compressedBuffer, usingMultipleThreads);
}

/*****************************************************************************/

dng_write_tiles_task::dng_write_tiles_task(dng_image_writer &imageWriter,
                                           dng_host &host,
                                           const dng_ifd &ifd,
...
void dng_write_tiles_task::Process(uint32 /* threadIndex */, const dng_rect & /* tile */, dng_abort_sniffer *sniffer) {
    try {
        AutoPtr<dng_memory_block> compressedBuffer;
        AutoPtr<dng_memory_block> uncompressedBuffer;
        AutoPtr<dng_memory_block> subTileBlockBuffer;
        ...
        while (true) {
            ...
            ProcessTask(
                tileIndex, compressedBuffer, uncompressedBuffer, subTileBlockBuffer, tempBuffer, tileByteCount, tileStream, sniffer);
...

            {
                ...
                while (!fTaskFailed && fWriteTileIndex != tileIndex) {
            ...
            WriteTask(tileIndex, tileByteCount, tileStream, sniffer);
...
void dng_write_tiles_task::ProcessTask(uint32 tileIndex,
                                       AutoPtr<dng_memory_block> &compressedBuffer,
                                       AutoPtr<dng_memory_block> &uncompressedBuffer,
                                       AutoPtr<dng_memory_block> &subTileBlockBuffer,
                                       AutoPtr<dng_memory_block> &tempBuffer,
    ...
    dng_host host(&fHost.Allocator(), sniffer);
...
void dng_write_tiles_task::WriteTask(uint32 tileIndex, uint32 tileByteCount, dng_memory_stream &tileStream, dng_abort_sniffer *sniffer) {
...
void dng_image_writer::DoWriteTiles(dng_host &host,
    ...
    uint32 threadCount = Min_uint32(tilesDown * tilesAcross, host.PerformAreaTaskThreads());

    dng_write_tiles_task task(
        *this, host, ifd, basic, stream, image, fakeChannels, tilesDown, tilesAcross, compressedSize, uncompressedSize.Get());

    host.PerformAreaTask(task, dng_rect(0, 0, 16, 16 * threadCount));
}
...
void dng_image_writer::WriteImage(dng_host &host,
    ...
    if (ifd.fRowInterleaveFactor > 1 && ifd.fRowInterleaveFactor < ifd.fImageLength) {
        dng_ifd tempIFD(ifd);

        tempIFD.fRowInterleaveFactor = 1;

        dng_row_interleaved_image tempImage(*((dng_image *)&image), ifd.fRowInterleaveFactor);

        WriteImage(host, tempIFD, basic, stream, tempImage, fakeChannels);
    ...
    if (ifd.TileByteCount(ifd.TileArea(0, 0)) != 0) {
        subTileLength = Pin_uint32(ifd.fSubTileBlockRows, kImageBufferSize / tileRowBytes.Get(), ifd.fTileLength);

        // Don't split sub-tiles across subTileBlocks.

        subTileLength = subTileLength / ifd.fSubTileBlockRows * ifd.fSubTileBlockRows;
    }
    ...
    bool useMultipleThreads = (tilesDown * tilesAcross >= 2) && (host.PerformAreaTaskThreads() > 1) && (subTileLength == ifd.fTileLength)
                              && (ifd.fCompression != ccUncompressed);

    if (useMultipleThreads) {
        DoWriteTiles(host, ifd, basic, stream, image, fakeChannels, tilesDown, tilesAcross, compressedSize, uncompressedSize.Get());
    }
        ...
        for (uint32 rowIndex = 0; rowIndex < tilesDown; rowIndex++) {
            for (uint32 colIndex = 0; colIndex < tilesAcross; colIndex++) {
                ...
                uint32 subTileCount = (tileArea.H() + subTileLength - 1) / subTileLength;
                ...
                for (uint32 subIndex = 0; subIndex < subTileCount; subIndex++) {
                    ...
                    subArea.b = Min_int32(subArea.t + subTileLength, tileArea.b);
...
static void CopyString(const dng_xmp &oldXMP, dng_xmp &newXMP, const char *ns, const char *path, dng_string *exif = NULL) {
    dng_string s;
...
static void CopyStringList(const dng_xmp &oldXMP, dng_xmp &newXMP, const char *ns, const char *path, bool isBag) {
...
static void CopyAltLangDefault(const dng_xmp &oldXMP, dng_xmp &newXMP, const char *ns, const char *path, dng_string *exif = NULL) {
    dng_string s;
...
static void CopyStructField(const dng_xmp &oldXMP, dng_xmp &newXMP, const char *ns, const char *path, const char *field) {
    dng_string s;
...
static void CopyBoolean(const dng_xmp &oldXMP, dng_xmp &newXMP, const char *ns, const char *path) {
    bool b;
...
void dng_image_writer::CleanUpMetadata(dng_host &host,
    ...
    if (metadata.GetXMP() && metadata.GetExif()) {
        ...
        if (software) {
            ...
            newXMP.Set(XMP_NS_XAP, "CreatorTool", software);
        }

#if qDNGXMPDocOps

        newXMP.DocOpsPrepareForSave(metadata.SourceMIME().Get(), dstMIME);
        ...
        newXMP.SyncExif(newEXIF, metadata.GetOriginalExif(), false, true);
        ...
        if (metadataSubset == kMetadataSubset_CopyrightOnly || metadataSubset == kMetadataSubset_CopyrightAndContact) {
            ...
            CopyAltLangDefault(oldXMP, newXMP, XMP_NS_DC, "rights", &newEXIF.fCopyright);

            CopyAltLangDefault(oldXMP, newXMP, XMP_NS_XAP_RIGHTS, "UsageTerms");

            CopyString(oldXMP, newXMP, XMP_NS_XAP_RIGHTS, "WebStatement");

            CopyBoolean(oldXMP, newXMP, XMP_NS_XAP_RIGHTS, "Marked");

#if qDNGXMPDocOps

            // Include basic DocOps fields, but not the full history.

            CopyString(oldXMP, newXMP, XMP_NS_MM, "OriginalDocumentID");

            CopyString(oldXMP, newXMP, XMP_NS_MM, "DocumentID");

            CopyString(oldXMP, newXMP, XMP_NS_MM, "InstanceID");

            CopyString(oldXMP, newXMP, XMP_NS_XAP, "MetadataDate");
            ...
            if (metadataSubset == kMetadataSubset_CopyrightAndContact) {
                ...
                CopyStringList(oldXMP, newXMP, XMP_NS_DC, "creator", false);
                ...
                CopyString(oldXMP, newXMP, XMP_NS_PHOTOSHOP, "AuthorsPosition");

                CopyStructField(oldXMP, newXMP, XMP_NS_IPTC, "CreatorContactInfo", "CiEmailWork");

                CopyStructField(oldXMP, newXMP, XMP_NS_IPTC, "CreatorContactInfo", "CiAdrExtadr");

                CopyStructField(oldXMP, newXMP, XMP_NS_IPTC, "CreatorContactInfo", "CiAdrCity");

                CopyStructField(oldXMP, newXMP, XMP_NS_IPTC, "CreatorContactInfo", "CiAdrRegion");

                CopyStructField(oldXMP, newXMP, XMP_NS_IPTC, "CreatorContactInfo", "CiAdrPcode");

                CopyStructField(oldXMP, newXMP, XMP_NS_IPTC, "CreatorContactInfo", "CiAdrCtry");

                CopyStructField(oldXMP, newXMP, XMP_NS_IPTC, "CreatorContactInfo", "CiTelWork");

                CopyStructField(oldXMP, newXMP, XMP_NS_IPTC, "CreatorContactInfo", "CiUrlWork");

                CopyAltLangDefault(oldXMP, newXMP, XMP_NS_DC, "title");
            }
        }

        else if (metadataSubset == kMetadataSubset_AllExceptCameraInfo || metadataSubset == kMetadataSubset_AllExceptCameraAndLocation
                 || metadataSubset == kMetadataSubset_AllExceptLocationInfo || metadataSubset == KMetadataSubset_AllExceptCameraRawInfo
                 || metadataSubset == KMetadataSubset_AllExceptCameraRawInfoAndLocation) {
            dng_xmp oldXMP(newXMP);
            dng_exif oldEXIF(newEXIF);

            if (metadataSubset == kMetadataSubset_AllExceptCameraInfo || metadataSubset == kMetadataSubset_AllExceptCameraAndLocation
            ...
            if (metadataSubset == kMetadataSubset_AllExceptLocationInfo || metadataSubset == kMetadataSubset_AllExceptCameraAndLocation
                || metadataSubset == KMetadataSubset_AllExceptCameraRawInfoAndLocation) {
                // Remove GPS fields.
        ...
        if (!isDNG) {
            metadata.RebuildIPTC(host.Allocator(), isTIFF);
        }
...
void dng_image_writer::UpdateExifColorSpaceTag(dng_metadata &metadata, const void *profileData, const uint32 profileSize) {
    if (!metadata.GetExif()) {
        return;
    }
    ...
    if (profileData && profileSize) {
        ...
        if (dng_space_sRGB::Get().ICCProfile(sRGB_size, sRGB_data)) {
            if ((sRGB_size == profileSize) && !memcmp(profileData, (const void *)sRGB_data, (size_t)sRGB_size)) {
                // Yes.
...
void dng_image_writer::WriteTIFFWithProfile(dng_host &host,
    ...
    if (constMetadata) {
        ...
        CleanUpMetadata(host, *metadata, metadataSubset, "image/tiff");

        UpdateExifColorSpaceTag(*metadata, profileData, profileSize);
    }
    ...
    tag_iptc tagIPTC(metadata.Get() ? metadata->IPTCData() : NULL, metadata.Get() ? metadata->IPTCLength() : 0);
    ...
    AutoPtr<dng_memory_block> adobeData(BuildAdobeData(host, metadata.Get(), thumbnail, imageResources));

    tag_uint8_ptr tagAdobe(tcAdobeData, adobeData->Buffer_uint8(), adobeData->LogicalSize());
    ...
    exif_tag_set exifSet(mainIFD,
                         metadata.Get() && metadata->GetExif() ? *metadata->GetExif() : dng_exif(),
    ...
    WriteImage(host, ifd, basic, stream, image);
...
void dng_image_writer::WriteDNG(dng_host &host,
                                ...
                                bool uncompressed) {
    WriteDNGWithMetadata(host, stream, negative, negative.Metadata(), previewList, maxBackwardVersion, uncompressed);
}
...
void dng_image_writer::WriteDNGWithMetadata(dng_host &host,
    ...
    CleanUpMetadata(host, *metadata, kMetadataSubset_All, "image/dng");
...

    {
        ...
        dng_point defaultFinalSize(negative.DefaultFinalHeight(), negative.DefaultFinalWidth());

        saveOriginalDefaultFinalSize = (negative.OriginalDefaultFinalSize() != defaultFinalSize);
        ...
        if (saveOriginalDefaultFinalSize) {
            ...
            saveOriginalBestQualityFinalSize = (negative.OriginalBestQualityFinalSize() != negative.OriginalDefaultFinalSize());

            saveOriginalDefaultCropSize =
                (negative.OriginalDefaultCropSizeV() != dng_urational(negative.OriginalDefaultFinalSize().v, 1))
                || (negative.OriginalDefaultCropSizeH() != dng_urational(negative.OriginalDefaultFinalSize().h, 1));
        }

        else {
            // Else these two tags default to the normal non-proxy size image values.

            dng_point bestQualityFinalSize(negative.BestQualityFinalHeight(), negative.BestQualityFinalWidth());

            saveOriginalBestQualityFinalSize = (negative.OriginalBestQualityFinalSize() != bestQualityFinalSize);

            saveOriginalDefaultCropSize = (negative.OriginalDefaultCropSizeV() != negative.DefaultCropSizeV())
                                          || (negative.OriginalDefaultCropSizeH() != negative.DefaultCropSizeH());
        }
    }
    ...
    bool hasEnhancedImage = (&negative.RawImage() != negative.Stage3Image()) && negative.EnhanceParams().NotEmpty();

    // Should we save a compressed 32-bit integer file?

    bool isCompressed32BitInteger =
        (negative.RawImage().PixelType() == ttLong) && (maxBackwardVersion >= dngVersion_1_4_0_0) && (!uncompressed);
    ...
    dngBackwardVersion = Max_uint32(dngBackwardVersion, negative.OpcodeList1().MinVersion(false));

    dngBackwardVersion = Max_uint32(dngBackwardVersion, negative.OpcodeList2().MinVersion(false));

    dngBackwardVersion = Max_uint32(dngBackwardVersion, negative.OpcodeList3().MinVersion(false));

    if (negative.GetMosaicInfo() && negative.GetMosaicInfo()->fCFALayout >= 6) {
    ...
    if (previewList) {
        ...
        for (j = 0; j < previewList->Count(); j++) {
            ...
            if (imagePreview) {
                uint32 thisArea = imagePreview->fImage->Bounds().W() * imagePreview->fImage->Bounds().H();
            ...
            if (jpegPreview) {
                uint32 thisArea = jpegPreview->fPreviewSize.h * jpegPreview->fPreviewSize.v;
    ...
    info.fPhotometricInterpretation = mosaicInfo.IsColorFilterArray() ? piCFA : piLinearRaw;
    ...
    if (info.fCompression == ccJPEG) {
        if (mosaicInfo.IsColorFilterArray()) {
            ...
            while (fakeChannels * info.fSamplesPerPixel > 4 && fakeChannels > 1) {
                fakeChannels >>= 1;
            }
        }
    }
    ...
    if (rawJPEGImage) {
        DNG_ASSERT(rawPixelType == ttByte, "Unexpected jpeg pixel type");

        DNG_ASSERT(info.fImageWidth == (uint32)rawJPEGImage->fImageSize.h && info.fImageLength == (uint32)rawJPEGImage->fImageSize.v,
    ...
    tag_data_ptr tagJPEGTables(tcJPEGTables, ttUndefined, 0, NULL);
    ...
    tag_urational_ptr tagDefaultScale(tcDefaultScale, defaultScaleData, 2);

    rawIFD.Add(&tagDefaultScale);

    // Best quality scale tag.

    tag_urational tagBestQualityScale(tcBestQualityScale, negative.BestQualityScale());
    ...
    tag_urational_ptr tagDefaultCropOrigin(tcDefaultCropOrigin, defaultCropOriginData, 2);
    ...
    tag_urational_ptr tagDefaultCropSize(tcDefaultCropSize, defaultCropSizeData, 2);
    ...
    tag_urational_ptr tagDefaultUserCrop(tcDefaultUserCrop, defaultUserCropData, 4);
    ...
    tag_urational tagChromaBlurRadius(tcChromaBlurRadius, negative.ChromaBlurRadius());
    ...
    tag_urational tagAntiAliasStrength(tcAntiAliasStrength, negative.AntiAliasStrength());
    ...
    if (!negative.IsMonochrome()) {
        ...
        profileSet.Reset(new profile_tag_set(mainIFD, mainProfile));

        colorSet.Reset(new color_tag_set(mainIFD, negative));
    ...
    tag_uint32_ptr extraProfileTag(tcExtraCameraProfiles, extraProfileOffsets.Buffer_uint32(), extraProfileCount);
    ...
    tag_srational tagBaselineExposure(tcBaselineExposure, negative.BaselineExposureR());

    mainIFD.Add(&tagBaselineExposure);

    tag_urational tagBaselineNoise(tcBaselineNoise, negative.BaselineNoiseR());

    mainIFD.Add(&tagBaselineNoise);

    dng_urational rawNoiseReductionApplied = hasEnhancedImage ? negative.RawNoiseReductionApplied() : negative.NoiseReductionApplied();

    tag_urational tagNoiseReductionAppliedMainIFD(tcNoiseReductionApplied, rawNoiseReductionApplied);

    tag_urational tagNoiseReductionAppliedRawIFD(tcNoiseReductionApplied, rawNoiseReductionApplied);
    ...
    dng_noise_profile rawNoiseProfile = hasEnhancedImage ? negative.RawNoiseProfile() : negative.NoiseProfile();
    ...
    tag_urational tagBaselineSharpness(tcBaselineSharpness, negative.RawBaselineSharpness());

    mainIFD.Add(&tagBaselineSharpness);

    tag_string tagUniqueName(tcUniqueCameraModel, negative.ModelName(), true);

    mainIFD.Add(&tagUniqueName);

    tag_string tagLocalName(tcLocalizedCameraModel, negative.LocalName(), false);

    if (negative.LocalName().NotEmpty()) {
        mainIFD.Add(&tagLocalName);
    }

    tag_urational tagShadowScale(tcShadowScale, negative.ShadowScaleR());

    mainIFD.Add(&tagShadowScale);

    tag_uint16 tagColorimetricReference(tcColorimetricReference, (uint16)negative.ColorimetricReference());
    ...
    tag_uint8_ptr tagRawImageDigest(useNewDigest ? tcNewRawImageDigest : tcRawImageDigest,
                                    compression == ccLossyJPEG
                                        ...
                                        : (useNewDigest ? negative.NewRawImageDigest().data : negative.RawImageDigest().data),
                                    16);
    ...
    tag_uint8_ptr tagRawDataUniqueID(tcRawDataUniqueID, rawDataUniqueID.data, 16);

    if (rawDataUniqueID.IsValid()) {
        mainIFD.Add(&tagRawDataUniqueID);
    }

    tag_string tagOriginalRawFileName(tcOriginalRawFileName, negative.OriginalRawFileName(), false);
    ...
    tag_data_ptr tagOriginalRawFileData(
        tcOriginalRawFileData, ttUndefined, negative.OriginalRawFileDataLength(), negative.OriginalRawFileData());

    tag_uint8_ptr tagOriginalRawFileDigest(tcOriginalRawFileDigest, negative.OriginalRawFileDigest().data, 16);
    ...
    exif_tag_set exifSet(
        mainIFD, *metadata->GetExif(), metadata->IsMakerNoteSafe(), metadata->MakerNoteData(), metadata->MakerNoteLength(), true);

    // Private data.

    tag_uint8_ptr tagPrivateData(tcDNGPrivateData, negative.PrivateData(), negative.PrivateLength());
    ...
    tag_uint32_ptr tagOriginalDefaultFinalSize(tcOriginalDefaultFinalSize, originalDefaultFinalSizeData, 2);
    ...
    tag_uint32_ptr tagOriginalBestQualityFinalSize(tcOriginalBestQualityFinalSize, originalBestQualityFinalSizeData, 2);
    ...
    tag_urational_ptr tagOriginalDefaultCropSize(tcOriginalDefaultCropSize, originalDefaultCropSizeData, 2);
    ...
    tag_uint16 tagDepthFormat(tcDepthFormat, (uint16)negative.DepthFormat());

    tag_urational tagDepthNear(tcDepthNear, negative.DepthNear());

    tag_urational tagDepthFar(tcDepthFar, negative.DepthFar());

    tag_uint16 tagDepthUnits(tcDepthUnits, (uint16)negative.DepthUnits());

    tag_uint16 tagDepthMeasureType(tcDepthMeasureType, (uint16)negative.DepthMeasureType());
    ...
    tag_string enhanceParams(tcEnhanceParams, negative.EnhanceParams(), false);

    tag_urational enhanceBaselineSharpness(tcBaselineSharpness, negative.BaselineSharpnessR());

    tag_urational enhanceNoiseReductionApplied(tcNoiseReductionApplied, negative.NoiseReductionApplied());

    tag_dng_noise_profile enhanceNoiseProfile(negative.NoiseProfile());

    uint16 enhanceBlackLevelData[kMaxColorPlanes];

    tag_uint16_ptr enhanceBlackLevel(tcBlackLevel, enhanceBlackLevelData);
    ...
    tag_uint32_ptr tagSubFile(tcSubIFDs, subFileData, subFileCount);
    ...
    if (thumbnail) {
        thumbnail->WriteData(host, *this, *thmBasic, stream);
    }
    ...
    for (j = 0; j < previewCount; j++) {
        if (thumbnail != &previewList->Preview(j)) {
            previewList->Preview(j).WriteData(host, *this, *previewBasic[j], stream);
        }
    }
    ...
    if (rawJPEGImage) {
        uint32 tileCount = info.TilesAcross() * info.TilesDown();
        ...
        for (uint32 tileIndex = 0; tileIndex < tileCount; tileIndex++) {
            ...
            stream.Put(rawJPEGImage->fJPEGData[tileIndex]->Buffer(), rawJPEGImage->fJPEGData[tileIndex]->LogicalSize());
        ...
        WriteImage(host, info, rawBasic, stream, rawImage, fakeChannels);
    }
    ...
    if (hasTransparencyMask) {
        ...
        WriteImage(host, *maskInfo, *maskBasic, stream, *negative.RawTransparencyMask());
    }
    ...
    if (hasDepthMap) {
        ...
        WriteImage(host, *depthInfo, *depthBasic, stream, *negative.RawDepthMap());
    }
    ...
    if (hasEnhancedImage) {
        ...
        WriteImage(host, *enhancedInfo, *enhancedBasic, stream, *negative.Stage3Image());
    }
