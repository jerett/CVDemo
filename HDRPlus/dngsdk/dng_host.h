...
public:
    ...
    dng_abort_sniffer *Sniffer() { return fSniffer; }
    ...
    void SetNeedsMeta(bool needs) { fNeedsMeta = needs; }

    /// Getter for flag determining whether all XMP metadata should be parsed.

    bool NeedsMeta() const { return fNeedsMeta; }
    ...
    bool ForFastSaveToDNG() const { return fForFastSaveToDNG; }

    uint32 FastSaveToDNGSize() const { return fFastSaveToDNGSize; }

    /// Setter for the cropping factor.
    /// \param cropFactor Fraction of image to be used after crop.

    void SetCropFactor(real64 cropFactor) { fCropFactor = cropFactor; }

    /// Getter for the cropping factor.

    real64 CropFactor() const { return fCropFactor; }
    ...
    void SetSaveDNGVersion(uint32 version) { fSaveDNGVersion = version; }
    ...
    void SetSaveLinearDNG(bool linear) { fSaveLinearDNG = linear; }
    ...
    void SetKeepOriginalFile(bool keep) { fKeepOriginalFile = keep; }

    /// Getter for flag determining whether to keep original RAW file data.

    bool KeepOriginalFile() { return fKeepOriginalFile; }
    ...
    bool WantsPreserveStage2() const { return fPreserveStage2; }

    /// Setter for flag determining whether we should preserve the stage 2
    /// image after building the stage 3 image.

    void SetWantsPreserveStage2(bool flag) { fPreserveStage2 = flag; }
};
