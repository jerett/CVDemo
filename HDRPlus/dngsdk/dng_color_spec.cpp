...
dng_matrix_3by3 MapWhiteMatrix(const dng_xy_coord &white1, const dng_xy_coord &white2) {
    ...
    dng_matrix_3by3 Mb(0.8951, 0.2664, -0.1614, -0.7502, 1.7135, 0.0367, 0.0389, -0.0685, 1.0296);
...
dng_color_spec::dng_color_spec(const dng_negative &negative, const dng_camera_profile *profile)
...

{
    if (fChannels > 1) {
        ...
        if (negative.CameraCalibrationSignature() == profile->ProfileCalibrationSignature()) {
            if (negative.CameraCalibration1().Rows() == fChannels && negative.CameraCalibration1().Cols() == fChannels) {
                fCameraCalibration1 = negative.CameraCalibration1();
            }

            if (negative.CameraCalibration2().Rows() == fChannels && negative.CameraCalibration2().Cols() == fChannels) {
        ...
        if (!profile->HasColorMatrix2() || fTemperature1 <= 0.0 || fTemperature2 <= 0.0 || fTemperature1 == fTemperature2) {
...
dng_matrix dng_color_spec::FindXYZtoCamera(const dng_xy_coord &white,
    ...
    if (g >= 1.0)
        ...
        colorMatrix = (g)*fColorMatrix1 + (1.0 - g) * fColorMatrix2;
    ...
    if (forwardMatrix) {
        ...
        if (has1 && has2) {
            if (g >= 1.0)
                ...
                *forwardMatrix = (g)*fForwardMatrix1 + (1.0 - g) * fForwardMatrix2;
        }
    ...
    if (reductionMatrix) {
        ...
        if (has1 && has2) {
            if (g >= 1.0)
                ...
                *reductionMatrix = (g)*fReductionMatrix1 + (1.0 - g) * fReductionMatrix2;
        }
    ...
    if (cameraCalibration) {
        if (g >= 1.0)
            ...
            *cameraCalibration = (g)*fCameraCalibration1 + (1.0 - g) * fCameraCalibration2;
    }
...
void dng_color_spec::SetWhiteXY(const dng_xy_coord &white) {
    ...
    colorMatrix = FindXYZtoCamera(fWhiteXY, &forwardMatrix, &reductionMatrix, &cameraCalibration);
    ...
    for (uint32 j = 0; j < fChannels; j++) {
        ...
        fCameraWhite[j] = Pin_real64(0.001, whiteScale * fCameraWhite[j], 1.0);
    }
...
dng_xy_coord dng_color_spec::NeutralToXY(const dng_vector &neutral) {
    ...
    for (uint32 pass = 0; pass < kMaxPasses; pass++) {
        ...
        if (Abs_real64(next.x - last.x) + Abs_real64(next.y - last.y) < 0.0000001) {
            return next;
        }
