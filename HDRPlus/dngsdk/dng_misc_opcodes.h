...
public:
    ...
    uint32 Plane() const { return fPlane; }
    ...
    uint32 RowPitch() const { return fRowPitch; }

    /// The column pitch (i.e., stride). A pitch of 1 means all columns.

    uint32 ColPitch() const { return fColPitch; }
...
public:
    ...
    uint32 Degree() const { return fDegree; }

    const real64 *Coefficients() const { return fCoefficient; }
