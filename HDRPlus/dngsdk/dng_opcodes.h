...
public:
    ...
    uint32 OpcodeID() const { return fOpcodeID; }

    /// The first DNG version that supports this opcode.

    uint32 MinVersion() const { return fMinVersion; }
    ...
    bool Optional() const { return (Flags() & kFlag_Optional) != 0; }

    /// Should the opcode be skipped when rendering preview images?

    bool SkipIfPreview() const { return (Flags() & kFlag_SkipIfPreview) != 0; }

    /// Was this opcode read from a data stream?

    bool WasReadFromStream() const { return fWasReadFromStream; }

    /// Which image processing stage (1, 2, 3) is associated with this
    /// opcode?

    uint32 Stage() const { return fStage; }
    ...
    void SetStage(uint32 stage) { fStage = stage; }
    ...
    virtual bool IsValidForNegative(const dng_negative & /* negative */) const { return true; }
    ...
    bool AboutToApply(dng_host &host, dng_negative &negative, const dng_rect &imageBounds, uint32 imagePlanes);
...
public:
    ...
    virtual uint32 BufferPixelType(uint32 imagePixelType) { return imagePixelType; }
    ...
    virtual dng_rect ModifiedBounds(const dng_rect &imageBounds) { return imageBounds; }

    /// Returns the width and height (in pixels) of the repeating mosaic pattern.

    virtual dng_point SrcRepeat() { return dng_point(1, 1); }
    ...
    virtual dng_point SrcTileSize(const dng_point &dstTileSize, const dng_rect &imageBounds) {
        return SrcArea(dng_rect(dstTileSize), imageBounds).Size();
    }
