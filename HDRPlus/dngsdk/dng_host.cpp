...
dng_host::dng_host(dng_memory_allocator *allocator, dng_abort_sniffer *sniffer)
...
void dng_host::PerformAreaTask(dng_area_task &task, const dng_rect &area, dng_area_task_progress *progress) {
...
dng_image *dng_host::Make_dng_image(const dng_rect &bounds, uint32 planes, uint32 pixelType) {
...
dng_opcode *dng_host::Make_dng_opcode(uint32 opcodeID, dng_stream &stream) {
    ...
    switch (opcodeID) {
        ...
        case dngOpcode_MapTable: {
            result = new dng_opcode_MapTable(*this, stream);
        ...
        case dngOpcode_GainMap: {
            result = new dng_opcode_GainMap(*this, stream);

            break;
        }

        case dngOpcode_DeltaPerRow: {
            result = new dng_opcode_DeltaPerRow(*this, stream);

            break;
        }

        case dngOpcode_DeltaPerColumn: {
            result = new dng_opcode_DeltaPerColumn(*this, stream);

            break;
        }

        case dngOpcode_ScalePerRow: {
            result = new dng_opcode_ScalePerRow(*this, stream);

            break;
        }

        case dngOpcode_ScalePerColumn: {
            result = new dng_opcode_ScalePerColumn(*this, stream);

            break;
        }

        default: { result = new dng_opcode_Unknown(*this, opcodeID, stream); }
    }
...
void dng_host::ApplyOpcodeList(dng_opcode_list &list, dng_negative &negative, AutoPtr<dng_image> &image) {
    list.Apply(*this, negative, image);
}

/*****************************************************************************/

void dng_host::ResampleImage(const dng_image &srcImage, dng_image &dstImage) {
    ::ResampleImage(*this, srcImage, dstImage, srcImage.Bounds(), dstImage.Bounds(), dng_resample_bicubic::Get());
}
