...
private:
    ...
    typedef std::pair<dng_fingerprint, int32> RefCountsPair;

    typedef std::map<dng_fingerprint, int32, dng_fingerprint_less_than> RefCountsMap;
...
void dng_big_table_cache::UseTable(dng_lock_std_mutex &lock, const dng_fingerprint &fingerprint) {
...
void dng_big_table_cache::CacheIncrement(dng_lock_std_mutex &lock, const dng_fingerprint &fingerprint) {
    if (fingerprint.IsValid()) {
        RefCountsMap::iterator it = fRefCounts.find(fingerprint);

        if (it == fRefCounts.end()) {
            DNG_REPORT(
                "dng_big_table_cache::CacheIncrement"
...
void dng_big_table_cache::CacheDecrement(dng_lock_std_mutex &lock, const dng_fingerprint &fingerprint) {
    if (fingerprint.IsValid()) {
        RefCountsMap::iterator it = fRefCounts.find(fingerprint);

        if (it == fRefCounts.end()) {
            DNG_REPORT(
                "dng_big_table_cache::CacheDecrement"
                "fingerprint not in cache");
        }
...
void dng_big_table_cache::CacheAdd(dng_lock_std_mutex &lock, const dng_big_table &table) {
...
bool dng_big_table_cache::CacheExtract(dng_lock_std_mutex &lock, const dng_fingerprint &fingerprint, dng_big_table &table) {
    if (fingerprint.IsValid()) {
        RefCountsMap::iterator it = fRefCounts.find(fingerprint);
...
void dng_big_table_cache::Increment(dng_big_table_cache *cache, const dng_fingerprint &fingerprint) {
    if (cache) {
...
void dng_big_table_cache::Decrement(dng_big_table_cache *cache, const dng_fingerprint &fingerprint) {
    if (cache) {
        dng_lock_std_mutex lock(cache->fMutex);
...
void dng_big_table_cache::Add(dng_big_table_cache *cache, const dng_big_table &table) {
    if (cache) {
        dng_lock_std_mutex lock(cache->fMutex);
...
bool dng_big_table_cache::Extract(dng_big_table_cache *cache, const dng_fingerprint &fingerprint, dng_big_table &table) {
    if (cache) {
        dng_lock_std_mutex lock(cache->fMutex);
...
private:
    typedef std::pair<dng_fingerprint, dng_look_table::table_data> TableDataPair;

    typedef std::map<dng_fingerprint, dng_look_table::table_data, dng_fingerprint_less_than> TableDataMap;
...
public:
    ...
    virtual void InsertTableData(dng_lock_std_mutex & /* lock */, const dng_big_table &table) {
        const dng_look_table *lookTable = static_cast<const dng_look_table *>(&table);

        fTableData.insert(TableDataPair(lookTable->Fingerprint(), lookTable->fData));
    }

    virtual void EraseTableData(dng_lock_std_mutex & /* lock */, const dng_fingerprint &fingerprint) {
        ...
        if (it != fTableData.end()) {
            ...
            DNG_REPORT(
                "dng_look_table_cache::EraseTableData"
                "fingerprint not in cache");
        }
    }

    virtual void ExtractTableData(dng_lock_std_mutex & /* lock */, const dng_fingerprint &fingerprint, dng_big_table &table) {
        TableDataMap::iterator it = fTableData.find(fingerprint);

        if (it != fTableData.end()) {
            dng_look_table *lookTable = static_cast<dng_look_table *>(&table);
            ...
            DNG_REPORT(
                "dng_look_table_cache::ExtractTableData"
                "fingerprint not in cache");
        }
    }
};
...
private:
    typedef std::pair<dng_fingerprint, dng_rgb_table::table_data> TableDataPair;

    typedef std::map<dng_fingerprint, dng_rgb_table::table_data, dng_fingerprint_less_than> TableDataMap;
...
public:
    ...
    virtual void InsertTableData(dng_lock_std_mutex & /* lock */, const dng_big_table &table) {
        const dng_rgb_table *rgbTable = static_cast<const dng_rgb_table *>(&table);

        fTableData.insert(TableDataPair(rgbTable->Fingerprint(), rgbTable->fData));
    }
    ...
    virtual void EraseTableData(dng_lock_std_mutex & /* lock */, const dng_fingerprint &fingerprint) {
        ...
        if (it != fTableData.end()) {
            ...
            DNG_REPORT(
                "dng_rgb_table_cache::EraseTableData"
                "fingerprint not in cache");
        }
    }
    ...
    virtual void ExtractTableData(dng_lock_std_mutex & /* lock */, const dng_fingerprint &fingerprint, dng_big_table &table) {
        ...
        if (it != fTableData.end()) {
            dng_rgb_table *rgbTable = static_cast<dng_rgb_table *>(&table);
            ...
            DNG_REPORT(
                "dng_rgb_table_cache::ExtractTableData"
                "fingerprint not in cache");
        }
    }
};
...
dng_big_table &dng_big_table::operator=(const dng_big_table &table) {
    if (fFingerprint != table.fFingerprint || fCache != table.fCache) {
...
bool dng_big_table::DecodeFromBinary(const uint8 *compressedData, uint32 compressedSize, dng_memory_allocator &allocator) {
    // Decompress the data.
...

    {
        ...
        uint32 uncompressedSize = (((uint32)compressedData[0])) + (((uint32)compressedData[1]) << 8) + (((uint32)compressedData[2]) << 16)
        ...
        int zResult = ::uncompress(block3->Buffer_uint8(), &destLen, compressedData + 4, compressedSize - 4);
    ...
    {
        dng_stream stream(block3->Buffer(), block3->LogicalSize());
...
bool dng_big_table::DecodeFromString(const dng_string &block1, dng_memory_allocator &allocator) {
    ...
    return DecodeFromBinary(block2->Buffer_uint8(), compressedSize, allocator);
}

/*****************************************************************************/

dng_memory_block *dng_big_table::EncodeAsBinary(dng_memory_allocator &allocator, uint32 &compressedSize) const {
...

    {
        ...
        int zResult = ::compress2(dPtr + 4, &dCount, block1->Buffer_uint8(), uncompressedSize, Z_DEFAULT_COMPRESSION);
...
dng_memory_block *dng_big_table::EncodeAsString(dng_memory_allocator &allocator) const {
...

    {
        ...
        uint32 safeEncodedSize = compressedSize + (compressedSize >> 2) + (compressedSize >> 6) + 16;
        ...
        while (compressedSize) {
            uint32 x0 = (((uint32)sPtr[0])) + (((uint32)sPtr[1]) << 8) + (((uint32)sPtr[2]) << 16) + (((uint32)sPtr[3]) << 24);
...
bool dng_big_table::ReadTableFromXMP(const dng_xmp &xmp, const char *ns, const dng_fingerprint &fingerprint) {
    ...
    if (!xmp.GetString(ns, tablePath.Get(), block1)) {
        DNG_REPORT("Missing big table data");

        return false;
    }

    bool ok = DecodeFromString(block1, xmp.Allocator());

    block1.Clear();

    // Validate fingerprint match.

    DNG_ASSERT(Fingerprint() == fingerprint, "dng_big_table fingerprint mismatch");
...
bool dng_big_table::ReadFromXMP(const dng_xmp &xmp, const char *ns, const char *path, dng_big_table_storage &storage) {
...
void dng_big_table::WriteToXMP(dng_xmp &xmp, const char *ns, const char *path, dng_big_table_storage &storage) const {
    ...
    xmp.Set(ns, tablePath.Get(), block->Buffer_char());
}
...
void dng_look_table::Set(const dng_hue_sat_map &map, uint32 encoding) {
    fData.fMap = map;
...
void dng_look_table::GetStream(dng_stream &stream) {
    ...
    if (version != kLookTableVersion1 && version != kLookTableVersion2) {
    ...
    if (hueDivisions < 1 || hueDivisions > kMaxHueSamples || satDivisions < 1 || satDivisions > kMaxSatSamples || valDivisions < 1
        || valDivisions > kMaxValSamples
        || (dng_safe_uint32(hueDivisions) * dng_safe_uint32(satDivisions) * dng_safe_uint32(valDivisions)).Get() > kMaxTotalSamples) {
        ThrowBadFormat();
    }

    data.fMap.SetDivisions(hueDivisions, satDivisions, valDivisions);
    ...
    if (data.fEncoding != encoding_Linear && data.fEncoding != encoding_sRGB) {
...
void dng_look_table::PutStream(dng_stream &stream, bool /* forFingerprint */) const {
    ...
    if (fData.fMinAmount != 1.0 || fData.fMaxAmount != 1.0) {
    ...
    fData.fMap.GetDivisions(hueDivisions, satDivisions, valDivisions);
...
bool dng_rgb_table::IsValid() const {
    ...
    if (fData.fPrimaries == primaries_ProPhoto || fData.fGamut == gamut_extend)
        return false;
...
void dng_rgb_table::Set(uint32 dimensions, uint32 divisions, dng_ref_counted_block samples) {
    if (dimensions == 1) {
        if (divisions < kMinDivisions1D || divisions > kMaxDivisions1D) {
        ...
        if (divisions < kMinDivisions3D || divisions > kMaxDivisions3D_InMemory) {
            ThrowProgramError("Bad 3D divisions");
        }

        if (samples.LogicalSize() != divisions * divisions * divisions * 4 * sizeof(uint16)) {
...
void dng_rgb_table::GetStream(dng_stream &stream) {
    ...
    if (data.fDimensions == 1) {
        if (data.fDivisions < kMinDivisions1D || data.fDivisions > kMaxDivisions1D) {
            ThrowBadFormat("Invalid 1D divisions");
        }
    }

    else if (data.fDimensions == 3) {
        if (data.fDivisions < kMinDivisions3D || data.fDivisions > kMaxDivisions3D) {
    ...
    uint16 nopValue[kMaxDivisions1D > kMaxDivisions3D ? kMaxDivisions1D : kMaxDivisions3D];

    for (uint32 index = 0; index < data.fDivisions; index++) {
        nopValue[index] = (uint16)((index * 0x0FFFF + (data.fDivisions >> 1)) / (data.fDivisions - 1));
    }
    ...
    if (data.fDimensions == 1) {
        ...
        data.fSamples.Allocate(data.fDivisions * data.fDivisions * data.fDivisions * 4 * sizeof(uint16));
...
void dng_rgb_table::PutStream(dng_stream &stream, bool /* forFingerprint */) const {
    ...
    uint16 nopValue[kMaxDivisions1D > kMaxDivisions3D_InMemory ? kMaxDivisions1D : kMaxDivisions3D_InMemory];

    for (uint32 index = 0; index < fData.fDivisions; index++) {
        nopValue[index] = (uint16)((index * 0x0FFFF + (fData.fDivisions >> 1)) / (fData.fDivisions - 1));
    }
