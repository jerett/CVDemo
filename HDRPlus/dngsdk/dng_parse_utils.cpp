...
static const char *LookupName(uint32 key, const dng_name_table *table, uint32 table_entries) {
...
const char *LookupParentCode(uint32 parentCode) {
    const dng_name_table kParentCodeNames[] = {{0, "IFD 0"},
                                               {tcExifIFD, "Exif IFD"},
                                               {tcGPSInfo, "GPS IFD"},
    ...
    const char *name = LookupName(parentCode, kParentCodeNames, sizeof(kParentCodeNames) / sizeof(kParentCodeNames[0]));
    ...
    if (parentCode >= tcFirstSubIFD && parentCode <= tcLastSubIFD) {
        sprintf(s, "SubIFD %u", (unsigned)(parentCode - tcFirstSubIFD + 1));
    }

    else if (parentCode >= tcFirstChainedIFD && parentCode <= tcLastChainedIFD) {
...
const char *LookupTagCode(uint32 parentCode, uint32 tagCode) {
    const dng_name_table kTagNames[] = {{tcNewSubFileType, "NewSubFileType"},
    ...
    const dng_name_table kInteroperabilityTagNames[] = {{tcInteroperabilityIndex, "InteroperabilityIndex"},
    ...
    const dng_name_table kFujiTagNames[] = {
        {tcFujiHeader, "FujiHeader"}, {tcFujiRawInfo1, "FujiRawInfo1"}, {tcFujiRawInfo2, "FujiRawInfo2"}};

    const dng_name_table kContaxTagNames[] = {{tcContaxHeader, "ContaxHeader"}};

    const char *name = NULL;

    if (parentCode == 0 || parentCode == tcExifIFD || parentCode == tcLeafMOS || (parentCode >= tcFirstSubIFD && parentCode <= tcLastSubIFD)
        || (parentCode >= tcFirstChainedIFD && parentCode <= tcLastChainedIFD)) {
        name = LookupName(tagCode, kTagNames, sizeof(kTagNames) / sizeof(kTagNames[0]));
    }

    else if (parentCode == tcGPSInfo) {
        name = LookupName(tagCode, kGPSTagNames, sizeof(kGPSTagNames) / sizeof(kGPSTagNames[0]));
    }

    else if (parentCode == tcInteroperabilityIFD) {
        name = LookupName(tagCode, kInteroperabilityTagNames, sizeof(kInteroperabilityTagNames) / sizeof(kInteroperabilityTagNames[0]));
    }

    else if (parentCode == tcFujiRAF) {
        name = LookupName(tagCode, kFujiTagNames, sizeof(kFujiTagNames) / sizeof(kFujiTagNames[0]));
    }

    else if (parentCode == tcContaxRAW) {
        name = LookupName(tagCode, kContaxTagNames, sizeof(kContaxTagNames) / sizeof(kContaxTagNames[0]));
    }
...
const char *LookupTagType(uint32 tagType) {
    const dng_name_table kTagTypeNames[] = {{ttByte, "Byte"},
                                            {ttAscii, "ASCII"},
                                            {ttShort, "Short"},
                                            {ttLong, "Long"},
                                            {ttRational, "Rational"},
    ...
    const char *name = LookupName(tagType, kTagTypeNames, sizeof(kTagTypeNames) / sizeof(kTagTypeNames[0]));
...
const char *LookupNewSubFileType(uint32 key) {
    const dng_name_table kNewSubFileTypeNames[] = {{sfMainImage, "Main Image"},
    ...
    const char *name = LookupName(key, kNewSubFileTypeNames, sizeof(kNewSubFileTypeNames) / sizeof(kNewSubFileTypeNames[0]));
...
const char *LookupCompression(uint32 key) {
    const dng_name_table kCompressionNames[] = {{ccUncompressed, "Uncompressed"},
                                                {ccLZW, "LZW"},
                                                {ccOldJPEG, "Old JPEG"},
                                                {ccJPEG, "JPEG"},
                                                {ccDeflate, "Deflate"},
    ...
    const char *name = LookupName(key, kCompressionNames, sizeof(kCompressionNames) / sizeof(kCompressionNames[0]));
...
const char *LookupPredictor(uint32 key) {
    const dng_name_table kPredictorNames[] = {{cpNullPredictor, "NullPredictor"},
    ...
    const char *name = LookupName(key, kPredictorNames, sizeof(kPredictorNames) / sizeof(kPredictorNames[0]));
...
const char *LookupSampleFormat(uint32 key) {
    const dng_name_table kSampleFormatNames[] = {{sfUnsignedInteger, "UnsignedInteger"},
                                                 {sfSignedInteger, "SignedInteger"},
                                                 {sfFloatingPoint, "FloatingPoint"},
                                                 {sfUndefined, "Undefined"}};

    const char *name = LookupName(key, kSampleFormatNames, sizeof(kSampleFormatNames) / sizeof(kSampleFormatNames[0]));
...
const char *LookupPhotometricInterpretation(uint32 key) {
    const dng_name_table kPhotometricInterpretationNames[] = {{piWhiteIsZero, "WhiteIsZero"},
    ...
    const char *name = LookupName(
        key, kPhotometricInterpretationNames, sizeof(kPhotometricInterpretationNames) / sizeof(kPhotometricInterpretationNames[0]));
...
const char *LookupOrientation(uint32 key) {
    const dng_name_table kOrientationNames[] = {{1, "1 - 0th row is top, 0th column is left"},
    ...
    const char *name = LookupName(key, kOrientationNames, sizeof(kOrientationNames) / sizeof(kOrientationNames[0]));
...
const char *LookupResolutionUnit(uint32 key) {
    const dng_name_table kResolutionUnitNames[] = {
        {ruNone, "None"}, {ruInch, "Inch"}, {ruCM, "cm"}, {ruMM, "mm"}, {ruMicroM, "Micrometer"}};

    const char *name = LookupName(key, kResolutionUnitNames, sizeof(kResolutionUnitNames) / sizeof(kResolutionUnitNames[0]));
...
const char *LookupCFAColor(uint32 key) {
    ...
    const char *name = LookupName(key, kCFAColorNames, sizeof(kCFAColorNames) / sizeof(kCFAColorNames[0]));
...
const char *LookupSensingMethod(uint32 key) {
    const dng_name_table kSensingMethodNames[] = {{0, "Undefined"},
                                                  {1, "MonochromeArea"},
                                                  {2, "OneChipColorArea"},
                                                  {3, "TwoChipColorArea"},
    ...
    const char *name = LookupName(key, kSensingMethodNames, sizeof(kSensingMethodNames) / sizeof(kSensingMethodNames[0]));
...
const char *LookupExposureProgram(uint32 key) {
    const dng_name_table kExposureProgramNames[] = {{epUnidentified, "Unidentified"},
                                                    {epManual, "Manual"},
    ...
    const char *name = LookupName(key, kExposureProgramNames, sizeof(kExposureProgramNames) / sizeof(kExposureProgramNames[0]));
...
const char *LookupMeteringMode(uint32 key) {
    const dng_name_table kMeteringModeNames[] = {{mmUnidentified, "Unknown"},
                                                 {mmAverage, "Average"},
    ...
    const char *name = LookupName(key, kMeteringModeNames, sizeof(kMeteringModeNames) / sizeof(kMeteringModeNames[0]));
...
const char *LookupLightSource(uint32 key) {
    const dng_name_table kLightSourceNames[] = {{lsUnknown, "Unknown"},
                                                {lsDaylight, "Daylight"},
    ...
    const char *name = LookupName(key, kLightSourceNames, sizeof(kLightSourceNames) / sizeof(kLightSourceNames[0]));
...
const char *LookupColorSpace(uint32 key) {
    const dng_name_table kColorSpaceNames[] = {{1, "sRGB"}, {0xFFFF, "Uncalibrated"}};

    const char *name = LookupName(key, kColorSpaceNames, sizeof(kColorSpaceNames) / sizeof(kColorSpaceNames[0]));
...
const char *LookupFileSource(uint32 key) {
    const dng_name_table kFileSourceNames[] = {{3, "DSC"}};

    const char *name = LookupName(key, kFileSourceNames, sizeof(kFileSourceNames) / sizeof(kFileSourceNames[0]));
...
const char *LookupSceneType(uint32 key) {
    const dng_name_table kSceneTypeNames[] = {{1, "A directly photographed image"}};

    const char *name = LookupName(key, kSceneTypeNames, sizeof(kSceneTypeNames) / sizeof(kSceneTypeNames[0]));
...
const char *LookupCustomRendered(uint32 key) {
    const dng_name_table kCustomRenderedNames[] = {{0, "Normal process"}, {1, "Custom process"}};

    const char *name = LookupName(key, kCustomRenderedNames, sizeof(kCustomRenderedNames) / sizeof(kCustomRenderedNames[0]));
...
const char *LookupExposureMode(uint32 key) {
    const dng_name_table kExposureModeNames[] = {{0, "Auto exposure"}, {1, "Manual exposure"}, {2, "Auto bracket"}};

    const char *name = LookupName(key, kExposureModeNames, sizeof(kExposureModeNames) / sizeof(kExposureModeNames[0]));
...
const char *LookupWhiteBalance(uint32 key) {
    const dng_name_table kWhiteBalanceNames[] = {{0, "Auto white balance"}, {1, "Manual white balance"}};

    const char *name = LookupName(key, kWhiteBalanceNames, sizeof(kWhiteBalanceNames) / sizeof(kWhiteBalanceNames[0]));
...
const char *LookupSceneCaptureType(uint32 key) {
    const dng_name_table kSceneCaptureTypeNames[] = {{0, "Standard"}, {1, "Landscape"}, {2, "Portrait"}, {3, "Night scene"}};

    const char *name = LookupName(key, kSceneCaptureTypeNames, sizeof(kSceneCaptureTypeNames) / sizeof(kSceneCaptureTypeNames[0]));
...
const char *LookupGainControl(uint32 key) {
    ...
    const char *name = LookupName(key, kGainControlNames, sizeof(kGainControlNames) / sizeof(kGainControlNames[0]));
...
const char *LookupContrast(uint32 key) {
    const dng_name_table kContrastNames[] = {{0, "Normal"}, {1, "Soft"}, {2, "Hard"}};

    const char *name = LookupName(key, kContrastNames, sizeof(kContrastNames) / sizeof(kContrastNames[0]));
...
const char *LookupSaturation(uint32 key) {
    const dng_name_table kSaturationNames[] = {{0, "Normal"}, {1, "Low saturation"}, {2, "High saturation"}};

    const char *name = LookupName(key, kSaturationNames, sizeof(kSaturationNames) / sizeof(kSaturationNames[0]));
...
const char *LookupSharpness(uint32 key) {
    const dng_name_table kSharpnessNames[] = {{0, "Normal"}, {1, "Soft"}, {2, "Hard"}};

    const char *name = LookupName(key, kSharpnessNames, sizeof(kSharpnessNames) / sizeof(kSharpnessNames[0]));
...
const char *LookupSubjectDistanceRange(uint32 key) {
    const dng_name_table kSubjectDistanceRangeNames[] = {{0, "Unknown"}, {1, "Macro"}, {2, "Close view"}, {3, "Distant view"}};

    const char *name =
        LookupName(key, kSubjectDistanceRangeNames, sizeof(kSubjectDistanceRangeNames) / sizeof(kSubjectDistanceRangeNames[0]));
...
const char *LookupComponent(uint32 key) {
    const dng_name_table kComponentNames[] = {{0, "-"}, {1, "Y"}, {2, "Cb"}, {3, "Cr"}, {4, "R"}, {5, "G"}, {6, "B"}};

    const char *name = LookupName(key, kComponentNames, sizeof(kComponentNames) / sizeof(kComponentNames[0]));
...
const char *LookupCFALayout(uint32 key) {
    ...
    const char *name = LookupName(key, kCFALayoutNames, sizeof(kCFALayoutNames) / sizeof(kCFALayoutNames[0]));
...
const char *LookupMakerNoteSafety(uint32 key) {
    const dng_name_table kMakerNoteSafetyNames[] = {{0, "Unsafe"}, {1, "Safe"}};

    const char *name = LookupName(key, kMakerNoteSafetyNames, sizeof(kMakerNoteSafetyNames) / sizeof(kMakerNoteSafetyNames[0]));
...
const char *LookupColorimetricReference(uint32 key) {
    const dng_name_table kColorimetricReferenceNames[] = {{crSceneReferred, "Scene Referred"}, {crICCProfilePCS, "ICC Profile PCS"}};

    const char *name =
        LookupName(key, kColorimetricReferenceNames, sizeof(kColorimetricReferenceNames) / sizeof(kColorimetricReferenceNames[0]));
...
const char *LookupPreviewColorSpace(uint32 key) {
    const dng_name_table kPreviewColorSpaceNames[] = {{previewColorSpace_Unknown, "Unknown"},
    ...
    const char *name = LookupName(key, kPreviewColorSpaceNames, sizeof(kPreviewColorSpaceNames) / sizeof(kPreviewColorSpaceNames[0]));
...
const char *LookupJPEGMarker(uint32 key) {
    const dng_name_table kJPEGMarkerNames[] = {
        {M_TEM, "TEM"},     {M_SOF0, "SOF0"},   {M_SOF1, "SOF1"},   {M_SOF2, "SOF2"},   {M_SOF3, "SOF3"},   {M_DHT, "DHT"},
        {M_SOF5, "SOF5"},   {M_SOF6, "SOF6"},   {M_SOF7, "SOF7"},   {M_JPG, "JPG"},     {M_SOF9, "SOF9"},   {M_SOF10, "SOF10"},
        {M_SOF11, "SOF11"}, {M_DAC, "DAC"},     {M_SOF13, "SOF13"}, {M_SOF14, "SOF14"}, {M_SOF15, "SOF15"}, {M_RST0, "RST0"},
        {M_RST1, "RST1"},   {M_RST2, "RST2"},   {M_RST3, "RST3"},   {M_RST4, "RST4"},   {M_RST5, "RST5"},   {M_RST6, "RST6"},
        {M_RST7, "RST7"},   {M_SOI, "SOI"},     {M_EOI, "EOI"},     {M_SOS, "SOS"},     {M_DQT, "DQT"},     {M_DNL, "DNL"},
        {M_DRI, "DRI"},     {M_DHP, "DHP"},     {M_EXP, "EXP"},     {M_APP0, "APP0"},   {M_APP1, "APP1"},   {M_APP2, "APP2"},
        {M_APP3, "APP3"},   {M_APP4, "APP4"},   {M_APP5, "APP5"},   {M_APP6, "APP6"},   {M_APP7, "APP7"},   {M_APP8, "APP8"},
        {M_APP9, "APP9"},   {M_APP10, "APP10"}, {M_APP11, "APP11"}, {M_APP12, "APP12"}, {M_APP13, "APP13"}, {M_APP14, "APP14"},
        {M_APP15, "APP15"}, {M_JPG0, "JPG0"},   {M_JPG1, "JPG1"},   {M_JPG2, "JPG2"},   {M_JPG3, "JPG3"},   {M_JPG4, "JPG4"},
        {M_JPG5, "JPG5"},   {M_JPG6, "JPG6"},   {M_JPG7, "JPG7"},   {M_JPG8, "JPG8"},   {M_JPG9, "JPG9"},   {M_JPG10, "JPG10"},
        {M_JPG11, "JPG11"}, {M_JPG12, "JPG12"}, {M_JPG13, "JPG13"}, {M_COM, "COM"},     {M_ERROR, "ERROR"}};

    const char *name = LookupName(key, kJPEGMarkerNames, sizeof(kJPEGMarkerNames) / sizeof(kJPEGMarkerNames[0]));
...
const char *LookupSensitivityType(uint32 key) {
    ...
    const char *name = LookupName(key, kSensitivityTypeNames, sizeof(kSensitivityTypeNames) / sizeof(kSensitivityTypeNames[0]));
...
const char *LookupDepthFormat(uint32 key) {
    ...
    const char *name = LookupName(key, kDepthFormatNames, sizeof(kDepthFormatNames) / sizeof(kDepthFormatNames[0]));
...
const char *LookupDepthUnits(uint32 key) {
    ...
    const char *name = LookupName(key, kDepthUnitNames, sizeof(kDepthUnitNames) / sizeof(kDepthUnitNames[0]));
...
const char *LookupDepthMeasureType(uint32 key) {
    ...
    const char *name = LookupName(key, kDepthMeasureTypeNames, sizeof(kDepthMeasureTypeNames) / sizeof(kDepthMeasureTypeNames[0]));
...
void DumpHexAscii(dng_stream &stream, uint32 count) {
...
void DumpHexAscii(const uint8 *buf, uint32 count) {
    uint32 rows = (count + 15) >> 4;
...
void DumpXMP(dng_stream &stream, uint32 count) {
    uint32 lineLength = 0;
...
void DumpTagValues(dng_stream &stream,
    ...
    printf("%s:", tag_name ? tag_name : LookupTagCode(parentCode, tagCode));
    ...
    switch (tagType) {
        ...
        case ttAscii: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, s, false);
        ...
        default: {
            ...
            if (tagCount == 1 && (tagType == ttByte || tagType == ttUndefined)) {
...
void DumpDateTime(const dng_date_time &dt) {
    printf("%04d:%02d:%02d %02d:%02d:%02d", (int)dt.fYear, (int)dt.fMonth, (int)dt.fDay, (int)dt.fHour, (int)dt.fMinute, (int)dt.fSecond);
}
...
void DumpHueSatMap(dng_stream &stream, uint32 hues, uint32 sats, uint32 vals, bool skipSat0) {
    uint32 doneLines = 0;
    uint32 skipLines = 0;
    ...
    for (uint32 v = 0; v < vals; v++) {
        for (uint32 h = 0; h < hues; h++) {
            for (uint32 s = skipSat0 ? 1 : 0; s < sats; s++) {
                ...
                if (gDumpLineLimit == 0 || gDumpLineLimit > doneLines) {
                    doneLines++;
...
bool CheckTagType(uint32 parentCode,
                  ...
                  uint16 validType2,
                  uint16 validType3) {
    if (tagType != validType0 && tagType != validType1 && tagType != validType2 && tagType != validType3) {
#if qDNGValidate
...
bool CheckTagCount(uint32 parentCode, uint32 tagCode, uint32 tagCount, uint32 minCount, uint32 maxCount) {
    if (maxCount < minCount)
        maxCount = minCount;

    if (tagCount < minCount || tagCount > maxCount) {
#if qDNGValidate
...
bool CheckColorImage(uint32 parentCode, uint32 tagCode, uint32 colorPlanes) {
    if (colorPlanes == 0) {
#if qDNGValidate
    ...
    if (colorPlanes == 1) {
...

        {
            ...
            sprintf(
                message, "%s %s is not allowed with monochrome images", LookupParentCode(parentCode), LookupTagCode(parentCode, tagCode));
...
bool CheckMainIFD(uint32 parentCode, uint32 tagCode, uint32 newSubFileType) {
...
bool CheckRawIFD(uint32 parentCode, uint32 tagCode, uint32 photometricInterpretation) {
    if (photometricInterpretation != piCFA && photometricInterpretation != piLinearRaw) {
#if qDNGValidate
...
bool CheckCFA(uint32 parentCode, uint32 tagCode, uint32 photometricInterpretation) {
...
void ParseStringTag(dng_stream &stream, uint32 parentCode, uint32 tagCode, uint32 tagCount, dng_string &s, bool trimBlanks) {
    if (tagCount == 0 || tagCount == 0xFFFFFFFF) {
        s.Clear();
    ...
    if (buffer[tagCount - 1] != 0) {
...

        {
            ...
            if (!hasNull && parentCode < tcFirstMakerNoteIFD) {
                ...
                sprintf(message, "%s %s is not NULL terminated", LookupParentCode(parentCode), LookupTagCode(parentCode, tagCode));
...
void ParseDualStringTag(dng_stream &stream, uint32 parentCode, uint32 tagCode, uint32 tagCount, dng_string &s1, dng_string &s2) {
    if (tagCount == 0 || tagCount == 0xFFFFFFFF) {
        s1.Clear();
        s2.Clear();
...
void ParseEncodedStringTag(dng_stream &stream, uint32 parentCode, uint32 tagCode, uint32 tagCount, dng_string &s) {
    if (tagCount < 8) {
#if qDNGValidate
...

    {
        ...
        if (hadLower) {
            ...
            sprintf(
                message, "%s %s text encoding label not all uppercase", LookupParentCode(parentCode), LookupTagCode(parentCode, tagCode));
    ...
    if (memcmp(label, "UNICODE\000", 8) == 0) {
        ...
        enum dng_encoding { dng_encoding_ascii, dng_encoding_jis_x208_1990, dng_encoding_unknown };
        ...
        if (memcmp(label, "ASCII\000\000\000", 8) == 0) {
            ...
            if (memcmp(label, "\000\000\000\000\000\000\000\000\000", 8) == 0) {
                ...
                if (buffer[0] != 0) {
                    ...
                    sprintf(message, "%s %s has unknown encoding", LookupParentCode(parentCode), LookupTagCode(parentCode, tagCode));
                ...
                sprintf(message, "%s %s has unexpected text encoding", LookupParentCode(parentCode), LookupTagCode(parentCode, tagCode));
...

        {
            if (encoding == dng_encoding_ascii && !s.IsASCII()) {
                ...
                sprintf(message, "%s %s has non-ASCII characters", LookupParentCode(parentCode), LookupTagCode(parentCode, tagCode));
...
bool ParseVectorTag(dng_stream &stream, uint32 parentCode, uint32 tagCode, uint32 tagType, uint32 tagCount, uint32 count, dng_vector &v) {
...
bool ParseDateTimeTag(dng_stream &stream, uint32 parentCode, uint32 tagCode, uint32 tagType, uint32 tagCount, dng_date_time &dt) {
    ...
    for (uint32 index = 0; index < 21; index++) {
        ...
        if (c != ' ' && c != ':' && c != '0') {
...

            {
                ...
                sprintf(message, "%s %s is not a valid date/time", LookupParentCode(parentCode), LookupTagCode(parentCode, tagCode));
