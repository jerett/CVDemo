...
real64 dng_exif::SnapExposureTime(real64 et) {
    ...
    static const real64 kStandardSpeed[] = {
        30.0,         25.0,         20.0,         15.0,          13.0,          10.0,          8.0,          6.0,          5.0,
        4.0,          3.2,          3.0,          2.5,           2.0,           1.6,           1.5,          1.3,          1.0,
        0.8,          0.7,          0.6,          0.5,           0.4,           0.3,           1.0 / 4.0,    1.0 / 5.0,    1.0 / 6.0,
        1.0 / 8.0,    1.0 / 10.0,   1.0 / 13.0,   1.0 / 15.0,    1.0 / 20.0,    1.0 / 25.0,    1.0 / 30.0,   1.0 / 40.0,   1.0 / 45.0,
        1.0 / 50.0,   1.0 / 60.0,   1.0 / 80.0,   1.0 / 90.0,    1.0 / 100.0,   1.0 / 125.0,   1.0 / 160.0,  1.0 / 180.0,  1.0 / 200.0,
        1.0 / 250.0,  1.0 / 320.0,  1.0 / 350.0,  1.0 / 400.0,   1.0 / 500.0,   1.0 / 640.0,   1.0 / 750.0,  1.0 / 800.0,  1.0 / 1000.0,
        1.0 / 1250.0, 1.0 / 1500.0, 1.0 / 1600.0, 1.0 / 2000.0,  1.0 / 2500.0,  1.0 / 3000.0,  1.0 / 3200.0, 1.0 / 4000.0, 1.0 / 5000.0,
        1.0 / 6000.0, 1.0 / 6400.0, 1.0 / 8000.0, 1.0 / 10000.0, 1.0 / 12000.0, 1.0 / 12800.0, 1.0 / 16000.0};

    uint32 count = sizeof(kStandardSpeed) / sizeof(kStandardSpeed[0]);
    ...
    for (uint32 fudge = 0; fudge <= 1; fudge++) {
        ...
        for (uint32 index = 0; index < count; index++) {
            if (testSpeed >= kStandardSpeed[index] * 0.98 && testSpeed <= kStandardSpeed[index] * 1.02) {
...
bool dng_exif::HasLensDistortInfo() const {
    return (fLensDistortInfo[0].IsValid() && fLensDistortInfo[1].IsValid() && fLensDistortInfo[2].IsValid()
...
bool dng_exif::ParseTag(dng_stream &stream,
                        ...
                        uint64 tagOffset) {
    if (parentCode == 0) {
        if (Parse_ifd0(stream, shared, parentCode, tagCode, tagType, tagCount, tagOffset)) {
            return true;
        }
    }

    if (parentCode == 0 || isMainIFD) {
        if (Parse_ifd0_main(stream, shared, parentCode, tagCode, tagType, tagCount, tagOffset)) {
            return true;
        }
    }

    if (parentCode == 0 || parentCode == tcExifIFD) {
        if (Parse_ifd0_exif(stream, shared, parentCode, tagCode, tagType, tagCount, tagOffset)) {
            return true;
        }
    }

    if (parentCode == tcGPSInfo) {
        if (Parse_gps(stream, shared, parentCode, tagCode, tagType, tagCount, tagOffset)) {
            return true;
        }
    }

    if (parentCode == tcInteroperabilityIFD) {
        if (Parse_interoperability(stream, shared, parentCode, tagCode, tagType, tagCount, tagOffset)) {
            return true;
        }
    }
...
bool dng_exif::Parse_ifd0(dng_stream &stream,
    ...
    switch (tagCode) {
        case tcImageDescription: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, fImageDescription);
        ...
        case tcMake: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, fMake);
        ...
        case tcModel: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, fModel);
        ...
        case tcSoftware: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, fSoftware);
        ...
        case tcDateTime: {
            ...
            if (!ParseDateTimeTag(stream, parentCode, tagCode, tagType, tagCount, dt)) {
                return false;
            }
        ...
        case tcArtist: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, fArtist);
        ...
        case tcCopyright: {
            ...
            ParseDualStringTag(stream, parentCode, tagCode, tagCount, fCopyright, fCopyright2);
...
bool dng_exif::Parse_ifd0_exif(dng_stream &stream,
    ...
    switch (tagCode) {
        case tcBatteryLevel: {
            ...
            if (tagType == ttAscii) {
                ParseStringTag(stream, parentCode, tagCode, tagCount, fBatteryLevelA);
            }
        ...
        case tcTimeZoneOffset: {
            ...
            if (gVerbose) {
                printf("TimeZoneOffset: DateTimeOriginal = %d", (int)zoneOriginal.ExactHourOffset());

                if (tagCount >= 2) {
                    printf(", DateTime = %d", (int)zoneCurrent.ExactHourOffset());
                }
        ...
        case tcExifVersion: {
            ...
            if (gVerbose) {
                real64 x = (b0 - '0') * 10.00 + (b1 - '0') * 1.00 + (b2 - '0') * 0.10 + (b3 - '0') * 0.01;
        ...
        case tcDateTimeOriginal: {
            ...
            fDateTimeOriginalStorageInfo = dng_date_time_storage_info(tagPosition, dng_date_time_format_exif);
        ...
        case tcDateTimeDigitized: {
            ...
            fDateTimeDigitizedStorageInfo = dng_date_time_storage_info(tagPosition, dng_date_time_format_exif);
        ...
        case tcOffsetTime: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, offsetTime);
        ...
        case tcSubjectDistance: {
            ...
            if (gVerbose) {
                printf("SubjectDistance: %u/%u\n", (unsigned)fSubjectDistance.n, (unsigned)fSubjectDistance.d);
            }
        ...
        case tcExposureIndexExif: {
            ...
            if (gVerbose) {
                printf("%s: ISO %0.1f\n", LookupTagCode(parentCode, tagCode), fExposureIndex.As_real64());
            }
        ...
        case tcUserComment: {
            ...
            ParseEncodedStringTag(stream, parentCode, tagCode, tagCount, fUserComment);
        ...
        case tcSubsecTime: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, subsecs);
        ...
        case tcCFAPatternExif: {
            ...
            if (cols < 1 || cols > kMaxCFAPattern || rows < 1 || rows > kMaxCFAPattern) {
                return false;
            }
        ...
        case tcDigitalZoomRatio: {
            ...
            if (gVerbose) {
                ...
                if (fDigitalZoomRatio.n == 0 || fDigitalZoomRatio.d == 0) {
                    printf("Not used\n");
                }
        ...
        case tcImageUniqueID: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, s);
        ...
        case tcLensMakeExif: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, fLensMake);
...
bool dng_exif::Parse_gps(dng_stream &stream,
    ...
    switch (tagCode) {
        ...
        case tcGPSDestLongitude: {
            ...
            if (!CheckTagType(parentCode, tagCode, tagType, ttRational) && !CheckTagType(parentCode, tagCode, tagType, ttSRational))
                return false;
        ...
        case tcGPSAreaInformation: {
            ...
            ParseEncodedStringTag(stream, parentCode, tagCode, tagCount, *s);
...
bool dng_exif::Parse_interoperability(dng_stream &stream,
    ...
    switch (tagCode) {
        case tcInteroperabilityIndex: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, fInteroperabilityIndex);
        ...
        case tcRelatedImageFileFormat: {
            ...
            ParseStringTag(stream, parentCode, tagCode, tagCount, fRelatedImageFileFormat);
...
void dng_exif::PostParse(dng_host & /* host */, dng_shared & /* shared */) {
#if qDNGValidate
    ...
    if (fMaxApertureValue.d) {
        ...
        if (fLensInfo[2].d && fLensInfo[3].d) {
            ...
            if (fs1 >= 1.0 && fs2 >= 1.0 && fs2 >= fs1) {
                ...
                if (mav < av1 - kAPEX_Slop - 1.0 || mav > av2 + kAPEX_Slop + 2.0) {
    ...
    if (fISOSpeedRatings[0] == 0 || fISOSpeedRatings[0] == 65535) {
        ...
        if (fRecommendedExposureIndex != 0
            && (fSensitivityType == stRecommendedExposureIndex || fSensitivityType == stSOSandREI || fSensitivityType == stREIandISOSpeed
                || fSensitivityType == stSOSandREIandISOSpeed)) {
            fISOSpeedRatings[0] = fRecommendedExposureIndex;
        }

        else if (fStandardOutputSensitivity != 0
                 && (fSensitivityType == stStandardOutputSensitivity || fSensitivityType == stSOSandREI
                     || fSensitivityType == stSOSandISOSpeed || fSensitivityType == stSOSandREIandISOSpeed)) {
            fISOSpeedRatings[0] = fStandardOutputSensitivity;
        }

        else if (fISOSpeed != 0
                 && (fSensitivityType == stISOSpeed || fSensitivityType == stSOSandISOSpeed || fSensitivityType == stREIandISOSpeed
                     || fSensitivityType == stSOSandREIandISOSpeed)) {
    ...
    if (fGPSLatitude[0].NotValid() && fGPSLongitude[0].NotValid() && fGPSAltitude.NotValid() && fGPSTimeStamp[0].NotValid()
