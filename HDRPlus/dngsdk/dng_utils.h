...
inline int32 RoundUpForPixelSizeAsInt32(uint32 x, uint32 pixelSize) {
    uint32 result = 0;
...
inline real32 Pin_real32_Overrange(real32 min, real32 x, real32 max) {
...
inline real64 Pin_real64_Overrange(real64 min, real64 x, real64 max) {
    // Normal numbers in (min,max). No change.
...
inline void DNG_RGBtoHSV(real32 r, real32 g, real32 b, real32 &h, real32 &s, real32 &v) {
...
inline void DNG_HSVtoRGB(real32 h, real32 s, real32 v, real32 &r, real32 &g, real32 &b) {
    if (s > 0.0f) {
        if (h < 0.0f)
            h += 6.0f;
...
public:
    const uint16 *NoiseBuffer16() const { return fNoiseBuffer.Buffer_uint16(); }
};

/*****************************************************************************/

void HistogramArea(dng_host &host, const dng_image &image, const dng_rect &area, uint32 *hist, uint32 histLimit, uint32 plane = 0);

/*****************************************************************************/

void LimitFloatBitDepth(dng_host &host, const dng_image &srcImage, dng_image &dstImage, uint32 bitDepth, real32 scale = 1.0f);
...
public:
    CFReleaseHelper(T ref) : fRef(ref) {}
