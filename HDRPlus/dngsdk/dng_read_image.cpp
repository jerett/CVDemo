...
static void DecodeDelta8(uint8 *dPtr, uint32 rows, uint32 cols, uint32 channels) {
...
static void DecodeDelta16(uint16 *dPtr, uint32 rows, uint32 cols, uint32 channels) {
    const uint32 dRowStep = cols * channels;
...
static void DecodeDelta32(uint32 *dPtr, uint32 rows, uint32 cols, uint32 channels) {
    const uint32 dRowStep = cols * channels;
...
static void DecodeFPDelta(uint8 *input, uint8 *output, int32 cols, int32 channels, int32 bytesPerSample) {
...
void dng_lzw_expander::AddTable(int32 w, int32 k) {
    DNG_ASSERT((w >= 0) && (w <= kTableSize), "bad w value in dng_lzw_expander::AddTable");
    ...
    DNG_ASSERT((nextCode >= 0) && (nextCode <= kTableSize), "bad fNextCode value in dng_lzw_expander::AddTable");
...
bool dng_lzw_expander::Expand(const uint8 *sPtr, uint8 *dPtr, int32 sCount, int32 dCount) {
    ...
    while (true) {
        ...
        do {
            ...
            DNG_ASSERT(code <= fNextCode, "Unexpected LZW code in dng_lzw_expander::Expand");
...
dng_row_interleaved_image::dng_row_interleaved_image(dng_image &image, uint32 factor)

    : dng_image(image.Bounds(), image.Planes(), image.PixelType())
...
void dng_row_interleaved_image::DoGet(dng_pixel_buffer &buffer) const {
    ...
    for (int32 row = buffer.fArea.t; row < buffer.fArea.b; row++) {
        ...
        tempBuffer.fData = (void *)buffer.DirtyPixel(row, buffer.fArea.l, buffer.fPlane);
...
void dng_row_interleaved_image::DoPut(const dng_pixel_buffer &buffer) {
    ...
    for (int32 row = buffer.fArea.t; row < buffer.fArea.b; row++) {
        ...
        tempBuffer.fData = (void *)buffer.ConstPixel(row, buffer.fArea.l, buffer.fPlane);
...
static void ReorderSubTileBlocks(dng_host &host, const dng_ifd &ifd, dng_pixel_buffer &buffer, AutoPtr<dng_memory_block> &tempBuffer) {
    uint32 tempBufferSize = ComputeBufferSize(buffer.fPixelType, buffer.fArea.Size(), buffer.fPlanes, padNone);
    ...
    DoCopyBytes(tempBuffer->Buffer(), buffer.fData, tempBufferSize);
}

/*****************************************************************************/

class dng_image_spooler : public dng_spooler, private dng_uncopyable {
private:
    dng_host &fHost;
...

{
    ...
    uint32 stripLength = Pin_uint32(ifd.fSubTileBlockRows, fBlock.LogicalSize() / bytesPerRow, fTileArea.H());

    stripLength = stripLength / ifd.fSubTileBlockRows * ifd.fSubTileBlockRows;
...
void dng_image_spooler::Spool(const void *data, uint32 count) {
    while (count) {
        ...
        DoCopyBytes(data, fBuffer + fBufferCount, block);
        ...
        if (fBufferCount == fBufferSize) {
            ...
            dng_pixel_buffer buffer(fTileStrip, fPlane, fPlanes, ttShort, pcInterleaved, fBuffer);

            if (fIFD.fSubTileBlockRows > 1) {
                ReorderSubTileBlocks(fHost, fIFD, buffer, fSubTileBuffer);
            }
            ...
            fTileStrip.b = Min_int32(fTileStrip.t + stripLength, fTileArea.b);

            fBufferCount = 0;

            fBufferSize = fTileStrip.W() * fTileStrip.H() * fPlanes * (uint32)sizeof(uint16);
        }
    }
}
...
bool dng_read_image::ReadUncompressed(dng_host &host,
    ...
    if (bitDepth == 8) {
        ...
        if (stream.SwapBytes()) {
            DoSwapBytes16((uint16 *)uncompressedBuffer->Buffer(), samplesPerTile);
        }
    }
        ...
        if (stream.SwapBytes()) {
            DoSwapBytes32((uint32 *)uncompressedBuffer->Buffer(), samplesPerTile);
        }
    }
    ...
    dng_pixel_buffer buffer(tileArea, plane, planes, pixelType, ifd.fPlanarConfiguration, uncompressedBuffer->Buffer());
    ...
    if (ifd.fSubTileBlockRows > 1) {
        ReorderSubTileBlocks(host, ifd, buffer, subTileBlockBuffer);
    }
...
void dng_read_image::DecodeLossyJPEG(dng_host &host,
    ...
    try {
        ...
        jpeg_mem_src(&cinfo, jpegDataInMemory, jpegDataSize);
...

        {
            ...
            ConvertUnsigned(static_cast<unsigned>(cinfo.num_components), &numComponentsAsUint32);

            // Check that dimensions of JPEG correspond to dimensions of tile.

            if (imageWidthAsUint32 != tileArea.W() || imageHeightAsUint32 != tileArea.H() || numComponentsAsUint32 != planes) {
        ...
        dng_pixel_buffer buffer(tileArea, plane, planes, ttByte, pcInterleaved, NULL);
...
static dng_memory_block *ReadJPEGDataToBlock(dng_host &host,
    ...
    AutoPtr<dng_memory_block> buffer(host.Allocate(SafeUint32Add(tablesByteCount, tileByteCount)));

    // Read in table.

    if (tablesByteCount) {
        DoCopyBytes(tablesBlock->Buffer(), buffer->Buffer(), tablesByteCount);
    }
...
bool dng_read_image::ReadBaselineJPEG(dng_host &host,
    ...
    if (fJPEGTables.Get() || !jpegDataInMemory) {
        ...
        jpegDataBlock.Reset(
            ReadJPEGDataToBlock(host, stream, fJPEGTables.Get(), stream.Position(), tileByteCount, ifd.fPatchFirstJPEGByte));
        ...
        DecodeLossyJPEG(
            host, image, tileArea, plane, planes, ifd.fPhotometricInterpretation, tileByteCount, jpegDataInMemory, usingMultipleThreads);
    }
...
bool dng_read_image::ReadLosslessJPEG(dng_host &host,
    ...
    dng_safe_uint32 bytesPerRow = (dng_safe_uint32(tileArea.W()) * planes * static_cast<uint32>(sizeof(uint16)));

    uint32 rowsPerStrip = Pin_uint32(ifd.fSubTileBlockRows, kImageBufferSize / bytesPerRow.Get(), tileArea.H());

    rowsPerStrip = rowsPerStrip / ifd.fSubTileBlockRows * ifd.fSubTileBlockRows;

    dng_safe_uint32 bufferSize = bytesPerRow * rowsPerStrip;

    if (uncompressedBuffer.Get() && uncompressedBuffer->LogicalSize() < bufferSize.Get()) {
    ...
    dng_image_spooler spooler(host, ifd, image, tileArea, plane, planes, *uncompressedBuffer.Get(), subTileBlockBuffer);
    ...
    DecodeLosslessJPEG(stream, spooler, decodedSize.Get(), decodedSize.Get(), bug16, tileOffset + tileByteCount);
...
bool dng_read_image::CanReadTile(const dng_ifd &ifd) {
    if (ifd.fSampleFormat[0] != sfUnsignedInteger && ifd.fSampleFormat[0] != sfFloatingPoint) {
        return false;
    }
    ...
    switch (ifd.fCompression) {
        case ccUncompressed: {
            if (ifd.fSampleFormat[0] == sfFloatingPoint) {
                return (ifd.fBitsPerSample[0] == 16 || ifd.fBitsPerSample[0] == 24 || ifd.fBitsPerSample[0] == 32);
            }

            return ifd.fBitsPerSample[0] >= 8 && ifd.fBitsPerSample[0] <= 32;
        }
        ...
        case ccJPEG: {
            ...
            if (ifd.IsBaselineJPEG()) {
                ...
                return ifd.fBitsPerSample[0] >= 8 && ifd.fBitsPerSample[0] <= 16;
            }
        ...
        case ccPackBits: {
            if (ifd.fSampleFormat[0] == sfFloatingPoint) {
                ...
                if (ifd.fPredictor != cpNullPredictor && ifd.fPredictor != cpFloatingPoint && ifd.fPredictor != cpFloatingPointX2
                    && ifd.fPredictor != cpFloatingPointX4) {
                    return false;
                }

                if (ifd.fBitsPerSample[0] != 16 && ifd.fBitsPerSample[0] != 24 && ifd.fBitsPerSample[0] != 32) {
                    return false;
                }
            }

            else {
                if (ifd.fPredictor != cpNullPredictor && ifd.fPredictor != cpHorizontalDifference
                    && ifd.fPredictor != cpHorizontalDifferenceX2 && ifd.fPredictor != cpHorizontalDifferenceX4) {
                    return false;
                }

                if (ifd.fBitsPerSample[0] != 8 && ifd.fBitsPerSample[0] != 16 && ifd.fBitsPerSample[0] != 32) {
                    return false;
                }
            }
...
bool dng_read_image::NeedsCompressedBuffer(const dng_ifd &ifd) {
    if (ifd.fCompression == ccLZW || ifd.fCompression == ccDeflate || ifd.fCompression == ccOldDeflate || ifd.fCompression == ccPackBits) {
        return true;
    }
...
void dng_read_image::ByteSwapBuffer(dng_host & /* host */, dng_pixel_buffer &buffer) {
...
void dng_read_image::DecodePredictor(dng_host & /* host */, const dng_ifd &ifd, dng_pixel_buffer &buffer) {
    switch (ifd.fPredictor) {
        ...
        case cpHorizontalDifferenceX4: {
            ...
            switch (buffer.fPixelType) {
                case ttByte: {
                    DecodeDelta8((uint8 *)buffer.fData, buffer.fArea.H(), buffer.fArea.W() / xFactor, buffer.fPlanes * xFactor);

                    return;
                }

                case ttShort: {
                    DecodeDelta16((uint16 *)buffer.fData, buffer.fArea.H(), buffer.fArea.W() / xFactor, buffer.fPlanes * xFactor);

                    return;
                }

                case ttLong: {
                    DecodeDelta32((uint32 *)buffer.fData, buffer.fArea.H(), buffer.fArea.W() / xFactor, buffer.fPlanes * xFactor);
...
void dng_read_image::ReadTile(dng_host &host,
    ...
    switch (ifd.fCompression) {
        ...
        case ccPackBits: {
            ...
            dng_safe_uint32 sampleCount = (dng_safe_uint32(planes) * dng_safe_uint32(tileArea.W()) * dng_safe_uint32(tileArea.H()));
            ...
            dng_pixel_buffer buffer(tileArea, plane, planes, pixelType, pcInterleaved, NULL);
            ...
            if (ifd.fPredictor == cpFloatingPoint || ifd.fPredictor == cpFloatingPointX2 || ifd.fPredictor == cpFloatingPointX4) {
                bufferSize += (dng_safe_uint32(dng_safe_int32(buffer.fRowStep)) * dng_safe_uint32(buffer.fPixelSize));
            }
            ...
            if (buffer.fPixelType == ttFloat) {
                bufferSize = Max_uint32(bufferSize.Get(), (sampleCount * 4u).Get());
            }
            ...
            if (ifd.fPredictor == cpFloatingPoint || ifd.fPredictor == cpFloatingPointX2 || ifd.fPredictor == cpFloatingPointX4) {
                buffer.fData = (uint8 *)buffer.fData + buffer.fRowStep * buffer.fPixelSize;
            }
            ...
            if (ifd.fCompression == ccLZW) {
                ...
                if (!expander.Expand(compressedBuffer->Buffer_uint8(), (uint8 *)buffer.fData, tileByteCount, uncompressedSize.Get())) {
                    ThrowBadFormat();
                }
            }

            else if (ifd.fCompression == ccPackBits) {
                dng_stream subStream(compressedBuffer->Buffer_uint8(), tileByteCount);

                if (!DecodePackBits(subStream, (uint8 *)buffer.fData, uncompressedSize.Get())) {
                    ThrowBadFormat();
                }
            }
                ...
                int err = uncompress((Bytef *)buffer.fData, &dstLen, (const Bytef *)compressedBuffer->Buffer(), tileByteCount);
            ...
            if (ifd.fPredictor == cpFloatingPoint || ifd.fPredictor == cpFloatingPointX2 || ifd.fPredictor == cpFloatingPointX4) {
                ...
                for (int32 row = tileArea.t; row < tileArea.b; row++) {
                    ...
                    DecodeFPDelta(srcPtr, dstPtr, tileArea.W() / xFactor, planes * xFactor, bytesPerSample.Get());
                }
                ...
                DecodePredictor(host, ifd, buffer);
            }
        ...
        case ccUncompressed: {
            if (ReadUncompressed(host, ifd, stream, image, tileArea, plane, planes, uncompressedBuffer, subTileBlockBuffer)) {
                return;
            }
        ...
        case ccJPEG: {
            if (ifd.IsBaselineJPEG()) {
                ...
                if (ReadLosslessJPEG(
                        host, ifd, stream, image, tileArea, plane, planes, tileByteCount, uncompressedBuffer, subTileBlockBuffer)) {
                    return;
                }
            }
...
bool dng_read_image::CanRead(const dng_ifd &ifd) {
    if (ifd.fImageWidth < 1 || ifd.fImageLength < 1) {
        return false;
    }
    ...
    for (uint32 j = 1; j < Min_uint32(ifd.fSamplesPerPixel, kMaxSamplesPerPixel); j++) {
        if (ifd.fBitsPerSample[j] != ifd.fBitsPerSample[0]) {
            return false;
        }

        if (ifd.fSampleFormat[j] != ifd.fSampleFormat[0]) {
            return false;
        }
    }

    if ((ifd.fPlanarConfiguration != pcInterleaved) && (ifd.fPlanarConfiguration != pcPlanar)
...
void dng_read_tiles_task::Process(uint32 /* threadIndex */, const dng_rect & /* tile */, dng_abort_sniffer *sniffer) {
    AutoPtr<dng_memory_block> compressedBuffer;
    AutoPtr<dng_memory_block> uncompressedBuffer;
    AutoPtr<dng_memory_block> subTileBlockBuffer;
    ...
    while (true) {
...

        {
            ...
            ReadTask(tileIndex, byteCount, compressedBuffer.Get());
        }

        ProcessTask(tileIndex, byteCount, sniffer, compressedBuffer, uncompressedBuffer, subTileBlockBuffer);
    }
}

/*****************************************************************************/

void dng_read_tiles_task::ReadTask(uint32 tileIndex, uint32 &byteCount, dng_memory_block *compressedBuffer) {
    ...
    fStream.Get(fJPEGImage ? fJPEGImage->fJPEGData[tileIndex]->Buffer() : compressedBuffer->Buffer(), byteCount);
}
...
void dng_read_tiles_task::ProcessTask(uint32 tileIndex,
    ...
    if (fJPEGTileDigest) {
        ...
        printer.Process(compressedBuffer->Buffer(), byteCount);

        fJPEGTileDigest[tileIndex] = printer.Result();
    }

    dng_stream tileStream(fJPEGImage ? fJPEGImage->fJPEGData[tileIndex]->Buffer() : compressedBuffer->Buffer(), byteCount);
    ...
    fReadImage.ReadTile(host,
                        ...
                        fInnerSamples,
                        byteCount,
                        fJPEGImage ? fJPEGImage->fJPEGData[tileIndex] : compressedBuffer,
                        uncompressedBuffer,
                        subTileBlockBuffer,
                        true);
}
...
void dng_read_image::Read(dng_host &host,
    ...
    if (ifd.fImageWidth > kLimit || ifd.fImageLength > kLimit) {
    ...
    if (ifd.fRowInterleaveFactor > 1 && ifd.fRowInterleaveFactor < ifd.fImageLength) {
        ...
        dng_row_interleaved_image tempImage(image, ifd.fRowInterleaveFactor);

        Read(host, tempIFD, stream, tempImage, jpegImage, jpegDigest);
    ...
    if (ifd.TileByteCount(ifd.TileArea(0, 0)) != 0) {
        ...
        uint32 bytesPerRow = SafeUint32Mult(ifd.fTileWidth, innerSamples, bytesPerPixel);

        subTileLength = Pin_uint32(ifd.fSubTileBlockRows, kImageBufferSize / bytesPerRow, ifd.fTileLength);
    ...
    // Else we need to know the byte counts.
        ...
        for (tileIndex = 0; tileIndex < tileCount; tileIndex++) {
            if (tileByteCount[tileIndex] < 1 || tileByteCount[tileIndex] > stream.Length()) {
                ThrowBadFormat();
            }
        }
    }
    ...
    if (tileByteCount) {
        for (tileIndex = 0; tileIndex < tileCount; tileIndex++) {
            maxTileByteCount = Max_uint32(maxTileByteCount, tileByteCount[tileIndex]);
        }
    }
    ...
    if (ifd.fJPEGTablesOffset && ifd.fJPEGTablesCount) {
        if (ifd.IsBaselineJPEG()) {
            ...
            stream.Get(fJPEGTables->Buffer(), fJPEGTables->LogicalSize());
        }
    }
    ...
    if (jpegDigest) {
        jpegTileDigest.Reset(new dng_fingerprint[SafeUint32Add(tileCount, (fJPEGTables.Get() ? 1 : 0))]);
    }
    ...
    dng_stream_contiguous_read_hint readHint(stream, host.Allocator(), tileOffset[0], contiguousByteCount);

    // See if we can do this read using multiple threads.

    bool useMultipleThreads = (outerSamples * tilesDown * tilesAcross >= 2) && (host.PerformAreaTaskThreads() > 1)
                              && (maxTileByteCount > 0 && maxTileByteCount <= 1024 * 1024) && (subTileLength == ifd.fTileLength)
                    ...
                    for (uint32 subIndex = 0; subIndex < subTileCount; subIndex++) {
                        ...
                        if (jpegImage) {
                            ...
                            if (jpegDigest) {
                                ...
                                printer.Process(compressedBuffer->Buffer(), subByteCount);
    ...
    if (jpegDigest) {
        if (fJPEGTables.Get()) {
            ...
            printer.Process(fJPEGTables->Buffer(), fJPEGTables->LogicalSize());
        ...
        for (uint32 j = 0; j < tileCount + (fJPEGTables.Get() ? 1 : 0); j++) {
            printer2.Process(jpegTileDigest[j].data, dng_fingerprint::kDNGFingerprintSize);
        }
