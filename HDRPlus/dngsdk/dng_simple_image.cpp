...
dng_simple_image::dng_simple_image(const dng_rect &bounds, uint32 planes, uint32 pixelType, dng_memory_allocator &allocator)
...
{
    uint32 bytes = ComputeBufferSize(pixelType, bounds.Size(), planes, padSIMDBytes);
...
dng_image *dng_simple_image::Clone() const {
    AutoPtr<dng_simple_image> result(new dng_simple_image(Bounds(), Planes(), PixelType(), fAllocator));

    result->fBuffer.CopyArea(fBuffer, Bounds(), 0, Planes());
...
void dng_simple_image::AcquireTileBuffer(dng_tile_buffer &buffer, const dng_rect &area, bool dirty) const {
    buffer.fArea = area;
    ...
    buffer.fData = (void *)fBuffer.ConstPixel(buffer.fArea.t, buffer.fArea.l, buffer.fPlane);
