...

{
    ...
    if (fSrcPixelType != ttByte && fSrcPixelType != ttShort && fSrcPixelType != ttLong && fSrcPixelType != ttFloat) {
        DNG_REPORT("Unsupported source pixel type");

        ThrowProgramError();
    }

    if (fDstPixelType != ttShort && fDstPixelType != ttFloat) {
        DNG_REPORT("Unsupported destination pixel type");

        ThrowProgramError();
    }

    if (fSrcPixelType == ttFloat && fDstPixelType != ttFloat) {
    ...
    fReal32 = (fSrcPixelType == ttLong || fDstPixelType == ttFloat);
    ...
    if (fBlack_2D_rows) {
        fBlack_2D_buffer.Reset(host.Allocate(SafeUint32Mult(fBlack_2D_rows, fBlack_2D_cols, 4)));

        for (j = 0; j < fBlack_2D_rows; j++) {
            for (k = 0; k < fBlack_2D_cols; k++) {
                real64 x = info.fBlackLevel[j][k % info.fBlackLevelRepeatCols][plane];
    ...
    else if (fBlack_2D_rows == 0 && (info.fBlackLevelRepeatRows > 1 || (fSrcPixelType != ttShort && fSrcPixelType != ttByte))) {
        fBlack_1D_rows = info.fBlackLevelRepeatRows;
    }

    if (fBlack_1D_rows) {
        fBlack_1D_buffer.Reset(host.Allocate(SafeUint32Mult(fBlack_1D_rows, 4)));
        ...
        for (j = 0; j < fBlack_1D_rows; j++) {
            ...
            if (fBlack_2D_rows == 0) {
                x = info.fBlackLevel[j % info.fBlackLevelRepeatRows][0][plane];
            }
    ...
    if (fSrcPixelType != ttLong && fSrcPixelType != ttFloat) {
        ...
        if (fBlack_1D_rows == 0 && fBlack_2D_rows == 0) {
            uint32 scaleLUTEntries = (fSrcPixelType == ttByte ? 0x100 : 0x10000);

            fScale_buffer.Reset(host.Allocate(scaleLUTEntries * TagTypeSize(fDstPixelType)));
...
void dng_linearize_plane::Process(const dng_rect &srcTile) {
    ...
    for (uint32 row = 0; row < rows; row++) {
        ...
        const void *sPtr = srcBuffer.ConstPixel(srcTile.t + row, srcTile.l, fPlane);

        void *dPtr = dstBuffer.DirtyPixel(dstRow, dstCol, fPlane);
        ...
        if (fSrcPixelType == ttFloat) {
            ...
            if (fBlack_1D_rows == 0 && fBlack_2D_cols == 0) {
                ...
                if (b2_count) {
                    b2 = fBlack_2D_buffer->Buffer_real32() + b2_count * (dstRow % fBlack_2D_rows);
        ...
        else if (fBlack_1D_rows == 0 && fBlack_2D_rows == 0 && fSrcPixelType != ttLong) {
            if (fDstPixelType == ttShort) {
        ...
        // Integer math case.
            ...
            if (b2_count) {
                DNG_REQUIRE(fBlack_2D_rows > 0, "Bad fBlack_2D_rows in dng_linearize_plane::Process");
...

{
    ...
    for (uint32 plane = 0; plane < srcImage.Planes(); plane++) {
        fPlaneTask[plane].Reset(new dng_linearize_plane(host, info, dstBlackLevel, forceClipBlackLevel, srcImage, dstImage, plane));
    }
...
void dng_linearize_image::Process(uint32 /* threadIndex */, const dng_rect &srcTile, dng_abort_sniffer * /* sniffer */) {
    // Process each plane.
...
void dng_linearization_info::RoundBlacks() {
    ...
    for (j = 0; j < fBlackLevelRepeatRows; j++)
        for (k = 0; k < fBlackLevelRepeatCols; k++)
            for (n = 0; n < kMaxSamplesPerPixel; n++) {
                maxAbs = Max_real64(maxAbs, Abs_real64(fBlackLevel[j][k][n]));
            }

    uint32 count = RowBlackCount();

    for (j = 0; j < count; j++) {
        maxAbs = Max_real64(maxAbs, Abs_real64(fBlackDeltaV->Buffer_real64()[j]));
    }

    count = ColumnBlackCount();

    for (j = 0; j < count; j++) {
        maxAbs = Max_real64(maxAbs, Abs_real64(fBlackDeltaH->Buffer_real64()[j]));
    }
...
void dng_linearization_info::Parse(dng_host &host, dng_stream &stream, dng_info &info) {
    uint32 j;
    uint32 k;
    uint32 n;
    ...
    if (rawIFD.fLinearizationTableCount) {
        uint32 size = SafeUint32Mult(rawIFD.fLinearizationTableCount, static_cast<uint32>(sizeof(uint16)));
    ...
    if (rawIFD.fBlackLevelDeltaHCount) {
        uint32 size = SafeUint32Mult(rawIFD.fBlackLevelDeltaHCount, static_cast<uint32>(sizeof(real64)));
    ...
    if (rawIFD.fBlackLevelDeltaVCount) {
        uint32 size = SafeUint32Mult(rawIFD.fBlackLevelDeltaVCount, static_cast<uint32>(sizeof(real64)));
...
void dng_linearization_info::PostParse(dng_host & /* host */, dng_negative &negative) {
...
real64 dng_linearization_info::MaxBlackLevel(uint32 plane) const {
    ...
    if (fBlackDeltaH.Get()) {
        ...
        for (j = 0; j < entries; j++) {
            DNG_REQUIRE(fBlackLevelRepeatCols > 0, "Bad fBlackLevelRepeatCols in dng_linearization_info::MaxBlackLevel");
    ...
    if (fBlackDeltaV.Get()) {
        ...
        for (j = 0; j < entries; j++) {
            DNG_REQUIRE(fBlackLevelRepeatRows > 0, "Bad fBlackLevelRepeatRows in dng_linearization_info::MaxBlackLevel");
...
void dng_linearization_info::Linearize(dng_host &host, dng_negative &negative, const dng_image &srcImage, dng_image &dstImage) {
    bool allowPreserveBlackLevels = negative.SupportsPreservedBlackLevels(host);

    if (allowPreserveBlackLevels && negative.ColorimetricReference() == crSceneReferred && dstImage.PixelType() == ttShort) {
    ...
    dng_linearize_image processor(host, *this, negative.Stage3BlackLevel(), forceClipBlackLevel, srcImage, dstImage);
...
dng_urational dng_linearization_info::BlackLevel(uint32 row, uint32 col, uint32 plane) const {
    dng_urational r;
