...
static const ruvt kTempTable[] = {{0, 0.18006, 0.26352, -0.24341},   {10, 0.18066, 0.26589, -0.25479},  {20, 0.18133, 0.26846, -0.26876},
...
void dng_temperature::Set_xy_coord(const dng_xy_coord &xy) {
    ...
    for (uint32 index = 1; index <= 30; index++) {
        ...
        if (dt <= 0.0 || index == 30) {
            ...
            fTemperature = 1.0E6 / (kTempTable[index - 1].r * f + kTempTable[index].r * (1.0 - f));
...
dng_xy_coord dng_temperature::Get_xy_coord() const {
    ...
    for (uint32 index = 0; index <= 29; index++) {
        if (r < kTempTable[index + 1].r || index == 29) {
            ...
            real64 f = (kTempTable[index + 1].r - r) / (kTempTable[index + 1].r - kTempTable[index].r);

            // Interpolate the black body coordinates.

            real64 u = kTempTable[index].u * f + kTempTable[index + 1].u * (1.0 - f);

            real64 v = kTempTable[index].v * f + kTempTable[index + 1].v * (1.0 - f);
