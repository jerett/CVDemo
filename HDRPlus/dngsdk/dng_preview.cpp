...
dng_preview_tag_set::dng_preview_tag_set(dng_tiff_directory &directory, const dng_preview &preview, const dng_ifd &ifd)

    : dng_basic_tag_set(directory, ifd)

      ,
      fApplicationNameTag(tcPreviewApplicationName, preview.fInfo.fApplicationName, false)

      ,
      fApplicationVersionTag(tcPreviewApplicationVersion, preview.fInfo.fApplicationVersion, false)

      ,
      fSettingsNameTag(tcPreviewSettingsName, preview.fInfo.fSettingsName, false)

      ,
      fSettingsDigest(preview.fInfo.fSettingsDigest)

      ,
      fSettingsDigestTag(tcPreviewSettingsDigest, fSettingsDigest.data, 16)

      ,
      fColorSpaceTag(tcPreviewColorSpace, preview.fInfo.fColorSpace)

      ,
      fDateTimeTag(tcPreviewDateTime, preview.fInfo.fDateTime, true)

      ,
      fRawToPreviewGainTag(tcRawToPreviewGain, preview.fInfo.fRawToPreviewGain)

      ,
      fCacheVersionTag(tcCacheVersion, preview.fInfo.fCacheVersion)
...
dng_basic_tag_set *dng_image_preview::AddTagSet(dng_tiff_directory &directory) const {
    fIFD.fNewSubFileType = fInfo.fIsPrimary ? sfPreviewImage : sfAltPreviewImage;
    ...
    fIFD.fPhotometricInterpretation = fIFD.fSamplesPerPixel == 1 ? piBlackIsZero : piRGB;
...
void dng_image_preview::WriteData(dng_host &host, dng_image_writer &writer, dng_basic_tag_set &basic, dng_stream &stream) const {
    writer.WriteImage(host, fIFD, basic, stream, *fImage.Get());
}
...
dng_jpeg_preview_tag_set::dng_jpeg_preview_tag_set(dng_tiff_directory &directory, const dng_jpeg_preview &preview, const dng_ifd &ifd)
...
dng_basic_tag_set *dng_jpeg_preview::AddTagSet(dng_tiff_directory &directory) const {
    ...
    ifd.fNewSubFileType = fInfo.fIsPrimary ? sfPreviewImage : sfAltPreviewImage;
...
void dng_jpeg_preview::WriteData(dng_host & /* host */,
    ...
    stream.Put(fCompressedData->Buffer(), fCompressedData->LogicalSize());
...
void dng_jpeg_preview::SpoolAdobeThumbnail(dng_stream &stream) const {
    DNG_ASSERT(fCompressedData.Get(), "SpoolAdobeThumbnail: no data");

    DNG_ASSERT(fPhotometricInterpretation == piYCbCr, "SpoolAdobeThumbnail: Non-YCbCr");
    ...
    stream.Put(fCompressedData->Buffer(), compressedSize);
...
dng_raw_preview_tag_set::dng_raw_preview_tag_set(dng_tiff_directory &directory, const dng_raw_preview &preview, const dng_ifd &ifd)

    : dng_preview_tag_set(directory, preview, ifd)

      ,
      fOpcodeList2Tag(tcOpcodeList2, ttUndefined, 0, NULL)

      ,
      fWhiteLevelTag(tcWhiteLevel, fWhiteLevelData, preview.fImage->Planes())

      ,
      fBlackLevelTag(tcBlackLevel, fBlackLevelData, preview.fImage->Planes())
...
void dng_raw_preview::WriteData(dng_host &host, dng_image_writer &writer, dng_basic_tag_set &basic, dng_stream &stream) const {
    writer.WriteImage(host, fIFD, basic, stream, *fImage.Get());
}
...
void dng_mask_preview::WriteData(dng_host &host, dng_image_writer &writer, dng_basic_tag_set &basic, dng_stream &stream) const {
    writer.WriteImage(host, fIFD, basic, stream, *fImage.Get());
}
...
dng_basic_tag_set *dng_depth_preview::AddTagSet(dng_tiff_directory &directory) const {
    fIFD.fNewSubFileType = fFullResolution ? sfDepthMap : sfPreviewDepthMap;
...
void dng_depth_preview::WriteData(dng_host &host, dng_image_writer &writer, dng_basic_tag_set &basic, dng_stream &stream) const {
    writer.WriteImage(host, fIFD, basic, stream, *fImage.Get());
}
