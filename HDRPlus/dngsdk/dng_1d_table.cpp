...
{
    DNG_REQUIRE(count >= kMinTableSize, "count must be at least kMinTableSize");

    DNG_REQUIRE((count & (count - 1)) == 0, "count must be power of 2");
}
...
void dng_1d_table::SubDivide(const dng_1d_function &function, uint32 lower, uint32 upper, real32 maxDelta) {
    uint32 range = upper - lower;

    bool subDivide = (range > (fTableCount >> 8));

    if (!subDivide) {
        real32 delta = Abs_real32(fTable[upper] - fTable[lower]);
...
void dng_1d_table::Initialize(dng_memory_allocator &allocator, const dng_1d_function &function, bool subSample) {
    ...
    if (subSample) {
        ...
        real32 maxDelta = Max_real32(Abs_real32(fTable[fTableCount] - fTable[0]), 1.0f) * (1.0f / 256.0f);

        SubDivide(function, 0, fTableCount, maxDelta);
    }
