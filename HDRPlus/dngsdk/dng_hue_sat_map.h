...
private:
    ...
    HSBModify *SafeGetDeltas() { return (HSBModify *)fDeltas.Buffer_real32(); }
...
public:
    ...
    bool IsValid() const { return fHueDivisions > 0 && fSatDivisions > 1 && fValDivisions > 0 && fDeltas.Buffer(); }
    ...
    void EnsureWriteable() { fDeltas.EnsureWriteable(); }
    ...
    void SetDelta(uint32 hueDiv, uint32 satDiv, uint32 valDiv, const HSBModify &modify) {
        ...
        SetDeltaKnownWriteable(hueDiv, satDiv, valDiv, modify);
    }
    ...
    uint32 DeltasCount() const {
        return (dng_safe_uint32(fValDivisions) * dng_safe_uint32(fHueDivisions) * dng_safe_uint32(fSatDivisions)).Get();
    }
    ...
    const HSBModify *GetConstDeltas() const { return (const HSBModify *)fDeltas.Buffer_real32(); }

    void AssignNewUniqueRuntimeFingerprint();

    /// Set Fingerprint. Rare use cases want to set the fingerprint.

    void SetRuntimeFingerprint(const dng_fingerprint fingerprint) { fRuntimeFingerprint = fingerprint; }

    /// Get the runtime fingerprint of this hue sat map.

    const dng_fingerprint &RuntimeFingerprint() const { return fRuntimeFingerprint; }
