...
uint32 ComputeBufferSize(uint32 pixelType, const dng_point &tileSize, uint32 numPlanes, PaddingType paddingType) {
    ...
    if (!SafeUint32Mult(paddedWidth, tileSizeV, &bufferSize) || !SafeUint32Mult(bufferSize, pixelSize, &bufferSize)
...
real64 MaxSquaredDistancePointToRect(const dng_point_real64 &point, const dng_rect_real64 &rect) {
    real64 distSqr = DistanceSquared(point, rect.TL());

    distSqr = Max_real64(distSqr, DistanceSquared(point, rect.BL()));

    distSqr = Max_real64(distSqr, DistanceSquared(point, rect.BR()));

    distSqr = Max_real64(distSqr, DistanceSquared(point, rect.TR()));
...
real64 MaxDistancePointToRect(const dng_point_real64 &point, const dng_rect_real64 &rect) {
    return sqrt(MaxSquaredDistancePointToRect(point, rect));
}
...
void HistogramArea(dng_host & /* host */, const dng_image &image, const dng_rect &area, uint32 *hist, uint32 maxValue, uint32 plane) {
    ...
    while (iter.GetOneTile(tile)) {
        ...
        const void *sPtr = buffer.ConstPixel(tile.t, tile.l, plane);
        ...
        OptimizeOrder(sPtr, buffer.fPixelSize, count0, count1, count2, step0, step1, step2);
...
template <SIMDType simd>
dng_limit_float_depth_task<simd>::dng_limit_float_depth_task(const dng_image &srcImage, dng_image &dstImage, uint32 bitDepth, real32 scale)
...
template <SIMDType simd>
void dng_limit_float_depth_task<simd>::Process(uint32 /* threadIndex */, const dng_rect &tile, dng_abort_sniffer * /* sniffer */) {
    ...
    const void *sPtr = srcBuffer.ConstPixel(tile.t, tile.l, 0);

    void *dPtr = dstBuffer.DirtyPixel(tile.t, tile.l, 0);

    OptimizeOrder(
        sPtr, dPtr, srcBuffer.fPixelSize, dstBuffer.fPixelSize, count0, count1, count2, sStep0, sStep1, sStep2, dStep0, dStep1, dStep2);
    ...
    for (uint32 index0 = 0; index0 < count0; index0++) {
        ...
        for (uint32 index1 = 0; index1 < count1; index1++) {
            ...
            if (limit16) {
                //start by using intrinsic __m256燺mm256_cvtph_ps?__m128i燼)
...
template <SIMDType simd>
void LimitFloatBitDepth(dng_host &host, const dng_image &srcImage, dng_image &dstImage, uint32 bitDepth, real32 scale) {
    ...
    dng_limit_float_depth_task<simd> task(srcImage, dstImage, bitDepth, scale);
...
template void LimitFloatBitDepth<Scalar>(dng_host &host, const dng_image &srcImage, dng_image &dstImage, uint32 bitDepth, real32 scale);

/*****************************************************************************/

#if qDNGIntelCompiler

template void LimitFloatBitDepth<AVX2>(dng_host &host, const dng_image &srcImage, dng_image &dstImage, uint32 bitDepth, real32 scale);
...
void LimitFloatBitDepth(dng_host &host, const dng_image &srcImage, dng_image &dstImage, uint32 bitDepth, real32 scale) {
    ...
    if (gDNGMaxSIMD >= AVX2) {
        LimitFloatBitDepth<AVX2>(host, srcImage, dstImage, bitDepth, scale);
    }
...

    {
        ...
        LimitFloatBitDepth<Scalar>(host, srcImage, dstImage, bitDepth, scale);
    }
}
