...
const dng_noise_function &dng_noise_profile::NoiseFunction(uint32 plane) const {
    ...
    DNG_REQUIRE(plane < NumFunctions(), "Bad plane index argument for NoiseFunction ().");
...
dng_metadata::dng_metadata(const dng_metadata &rhs, dng_memory_allocator &allocator)
...
dng_memory_block *dng_metadata::BuildExifBlock(dng_memory_allocator &allocator,
...

    {
        ...
        tag_iptc tagIPTC(IPTCData(), IPTCLength());
        ...
        exif_tag_set exifSet(mainIFD, *GetExif(), IsMakerNoteSafe(), MakerNoteData(), MakerNoteLength(), false);
        ...
        if (mainIFD.Size() != 0) {
            ...
            if (thumbnail) {
                ...
                stream.Put(thumbnail->fCompressedData->Buffer(), thumbnail->fCompressedData->LogicalSize());
            }
...
void dng_metadata::RebuildIPTC(dng_memory_allocator &allocator, bool padForTIFF) {
    ClearIPTC();
...
void dng_metadata::ResetXMPSidecarNewer(dng_xmp *newXMP, bool inSidecar, bool isNewer) {
    fXMP.Reset(newXMP);
...
bool dng_metadata::SetXMP(dng_host &host, const void *buffer, uint32 count, bool xmpInSidecar, bool xmpIsNewer) {
    bool result = false;
...
void dng_metadata::SetEmbeddedXMP(dng_host &host, const void *buffer, uint32 count) {
...
void dng_negative::GetCameraWhiteXY(dng_urational &x, dng_urational &y) const {
...
void dng_negative::AddProfile(AutoPtr<dng_camera_profile> &profile) {
    ...
    if (fCameraProfile.size()) {
        ...
        if (fCameraProfile[0]->NameIsEmbedded() && fCameraProfile[0]->EqualData(*profile.Get())) {
    ...
    for (uint32 index = 0; index < (uint32)fCameraProfile.size(); index++) {
        ...
        const bool equalColorAndSameName =
            (fCameraProfile[index]->EqualData(*profile.Get()) && fCameraProfile[index]->Name() == profile->Name());
...
void dng_negative::ClearProfiles(bool clearBuiltinMatrixProfiles, bool clearReadFromDisk) {
    // If neither flag is set, then there's nothing to do.

    if (!clearBuiltinMatrixProfiles && !clearReadFromDisk) {
        return;
    }
    ...
    for (; iter != fCameraProfile.end(); iter = next) {
        ...
        if (!profile || (clearBuiltinMatrixProfiles && profile->WasBuiltinMatrix()) || (clearReadFromDisk && profile->WasReadFromDisk())) {
            delete profile;
...
const dng_camera_profile &dng_negative::ProfileByIndex(uint32 index) const {
    DNG_ASSERT(index < ProfileCount(), "Invalid index for ProfileByIndex");
...
const dng_camera_profile *dng_negative::ProfileByID(const dng_camera_profile_id &id, bool useDefaultIfNoMatch) const {
    uint32 index;
    ...
    if (id.Name().NotEmpty() && id.Fingerprint().IsValid()) {
        for (index = 0; index < profileCount; index++) {
            ...
            if (id.Name() == profile.Name() && id.Fingerprint() == profile.Fingerprint()) {
                return &profile;
            }
        }
    }
    ...
    if (id.Name().NotEmpty()) {
        ...
        SplitCameraProfileName(id.Name(), baseName, version);
        ...
        for (index = 0; index < profileCount; index++) {
            ...
            if (profile.Name().StartsWith(baseName.Get())) {
                ...
                SplitCameraProfileName(profile.Name(), testBaseName, testVersion);
...
const dng_camera_profile *dng_negative::ComputeCameraProfileToEmbed(const dng_metadata & /* metadata */) const {
    uint32 index;
...
dng_fingerprint dng_negative::FindImageDigest(dng_host &host, const dng_image &image) {
    dng_md5_printer printer;

    dng_pixel_buffer buffer(image.Bounds(), 0, image.Planes(), image.PixelType(), pcInterleaved, NULL);
    ...
    if (!SafeUint32Mult(kBufferRows, buffer.fRowStep, &bufferBytes) || !SafeUint32Mult(bufferBytes, buffer.fPixelSize, &bufferBytes)) {
    ...
    dng_tile_iterator iter(dng_point(kBufferRows, image.Width()), image.Bounds());
    ...
    while (iter.GetOneTile(area)) {
        ...
        uint32 count = buffer.fArea.H() * buffer.fRowStep * buffer.fPixelSize;
...
public:
...

    {
        ...
        fUnitCell = dng_point(Min_int32(kTileSize, fImage.Bounds().H()), Min_int32(kTileSize, fImage.Bounds().W()));
    ...
    virtual void Start(uint32 threadCount,
        ...
        const uint32 bufferSize = ComputeBufferSize(fPixelType, tileSize, fImage.Planes(), padNone);
    ...
    virtual void Process(uint32 threadIndex, const dng_rect &tile, dng_abort_sniffer * /* sniffer */) {
        ...
        DNG_ASSERT(tile.l == fImage.Bounds().l + colIndex * fUnitCell.h && tile.t == fImage.Bounds().t + rowIndex * fUnitCell.v,
                   "Bad tile origin");

        uint32 tileIndex = rowIndex * fTilesAcross + colIndex;

        dng_pixel_buffer buffer(tile, 0, fImage.Planes(), fPixelType, pcPlanar, fBufferData[threadIndex]->Buffer());

        fImage.Get(buffer);

        uint32 count = buffer.fPlaneStep * buffer.fPlanes * buffer.fPixelSize;
...
void dng_negative::FindNewRawImageDigest(dng_host &host) const {
    if (fNewRawImageDigest.IsNull()) {
        ...
        if (RawTransparencyMask() != NULL) {
            ...
            {
                dng_find_new_raw_image_digest_task task(*RawTransparencyMask(), RawTransparencyMask()->PixelType());
...
void dng_negative::ValidateRawImageDigest(dng_host &host) {
    if (Stage1Image() && !IsPreview() && (fRawImageDigest.IsValid() || fNewRawImageDigest.IsValid())) {
        bool isNewDigest = fNewRawImageDigest.IsValid();

        dng_fingerprint &rawDigest = isNewDigest ? fNewRawImageDigest : fRawImageDigest;
        ...
        // Else we can compare the stored digest to the image in memory.
            ...
            if (oldDigest != rawDigest) {
                ...
                if (!isNewDigest) {
                    ...
                    if (oldDigest.data[0] == 0x08 && oldDigest.data[1] == 0x00 && oldDigest.data[2] == 0x00 && oldDigest.data[3] == 0x00) {
                        return;
                    }
                }
...
dng_fingerprint dng_negative::RawDataUniqueID() const {
    ...
    if (fRawDataUniqueID.IsValid() && fEnhanceParams.NotEmpty()) {
        ...
        printer.Process(fEnhanceParams.Get(), fEnhanceParams.Length());
...
void dng_negative::FindRawDataUniqueID(dng_host &host) const {
    if (RawDataUniqueID().IsNull()) {
        ...
        printer.Put(ModelName().Get(), ModelName().Length());
...
void dng_negative::FindOriginalRawFileDigest() const {
    if (fOriginalRawFileDigest.IsNull() && fOriginalRawFileData.Get()) {
        ...
        printer.Process(fOriginalRawFileData->Buffer(), fOriginalRawFileData->LogicalSize());
...
void dng_negative::SetMaskedAreas(uint32 count, const dng_rect *area) {
...
void dng_negative::SetBlackLevel(real64 black, int32 plane) {
    NeedLinearizationInfo();
...
void dng_negative::SetQuadBlacks(real64 black0, real64 black1, real64 black2, real64 black3, int32 plane) {
    NeedLinearizationInfo();
...
void dng_negative::Set6x6Blacks(real64 blacks6x6[36], int32 plane) {
    NeedLinearizationInfo();
...
void dng_negative::SetRowBlacks(const real64 *blacks, uint32 count) {
    if (count) {
        NeedLinearizationInfo();

        dng_linearization_info &info = *fLinearizationInfo.Get();

        dng_safe_uint32 byteCount = dng_safe_uint32(count) * (uint32)sizeof(real64);

        info.fBlackDeltaV.Reset(Allocator().Allocate(byteCount.Get()));

        DoCopyBytes(blacks, info.fBlackDeltaV->Buffer(), byteCount.Get());
...
void dng_negative::SetColumnBlacks(const real64 *blacks, uint32 count) {
    if (count) {
        NeedLinearizationInfo();
        ...
        DoCopyBytes(blacks, info.fBlackDeltaH->Buffer(), byteCount.Get());
...
void dng_negative::SetWhiteLevel(uint32 white, int32 plane) {
    NeedLinearizationInfo();
...
void dng_negative::SetColorKeys(ColorKeyCode color0, ColorKeyCode color1, ColorKeyCode color2, ColorKeyCode color3) {
    NeedMosaicInfo();
...
void dng_negative::SetFujiMosaic6x6(uint32 phase) {
    ...
    DNG_REQUIRE(phase >= 0 && phase < patSize * patSize, "Bad phase in SetFujiMosaic6x6.");
...
void dng_negative::Parse(dng_host &host, dng_stream &stream, dng_info &info) {
    // Shared info.
    ...
    SetDefaultCropSize(rawIFD.fDefaultCropSizeH, rawIFD.fDefaultCropSizeV);

    SetDefaultCropOrigin(rawIFD.fDefaultCropOriginH, rawIFD.fDefaultCropOriginV);

    // Default user crop rectangle.

    SetDefaultUserCrop(rawIFD.fDefaultUserCropT, rawIFD.fDefaultUserCropL, rawIFD.fDefaultUserCropB, rawIFD.fDefaultUserCropR);

    // Default scale.

    SetDefaultScale(rawIFD.fDefaultScaleH, rawIFD.fDefaultScaleV);
    ...
    if (shared.fCameraCalibration1.NotEmpty() || shared.fCameraCalibration2.NotEmpty()) {
    ...
    fLinearizationInfo.Get()->Parse(host, stream, info);
    ...
    if (rawIFD.fPhotometricInterpretation == piCFA) {
        ...
        fMosaicInfo.Get()->Parse(host, stream, info);
    }

    // Fill in original sizes.

    if (shared.fOriginalDefaultFinalSize.h > 0 && shared.fOriginalDefaultFinalSize.v > 0) {
    ...
    if (shared.fOriginalBestQualityFinalSize.h > 0 && shared.fOriginalBestQualityFinalSize.v > 0) {
        SetOriginalBestQualityFinalSize(shared.fOriginalBestQualityFinalSize);
    }

    if (shared.fOriginalDefaultCropSizeH.As_real64() >= 1.0 && shared.fOriginalDefaultCropSizeV.As_real64() >= 1.0) {
        SetOriginalDefaultCropSize(shared.fOriginalDefaultCropSizeH, shared.fOriginalDefaultCropSizeV);
    }
...
void dng_negative::SetDefaultOriginalSizes() {
    ...
    if (OriginalDefaultFinalSize() == dng_point()) {
        SetOriginalDefaultFinalSize(dng_point(DefaultFinalHeight(), DefaultFinalWidth()));
    }

    if (OriginalBestQualityFinalSize() == dng_point()) {
        SetOriginalBestQualityFinalSize(dng_point(BestQualityFinalHeight(), BestQualityFinalWidth()));
    }

    if (OriginalDefaultCropSizeH().NotValid() || OriginalDefaultCropSizeV().NotValid()) {
...
void dng_negative::SetOriginalSizes(const dng_point &size) {
    ...
    SetOriginalDefaultCropSize(dng_urational(size.h, 1), dng_urational(size.v, 1));
}

/*****************************************************************************/

void dng_negative::PostParse(dng_host &host, dng_stream &stream, dng_info &info) {
    // Shared info.
    ...
    if (host.NeedsMeta()) {
        ...
        if (shared.fXMPCount) {
            ...
            Metadata().SetEmbeddedXMP(host, block->Buffer(), block->LogicalSize());
...
const dng_image &dng_negative::RawImage() const {
    ...
    DNG_REQUIRE(fStage3Image.Get(), "dng_negative::RawImage with no raw image");
...
void dng_negative::ReadOpcodeLists(dng_host &host, dng_stream &stream, dng_info &info) {
    dng_ifd &rawIFD = *info.fIFD[info.fMainIndex];
    ...
    if (rawIFD.fOpcodeList1Count) {
        ...
        fOpcodeList1.Parse(host, stream, rawIFD.fOpcodeList1Count, rawIFD.fOpcodeList1Offset);
    }
    ...
    if (rawIFD.fOpcodeList2Count) {
        ...
        fOpcodeList2.Parse(host, stream, rawIFD.fOpcodeList2Count, rawIFD.fOpcodeList2Offset);
    }
    ...
    if (rawIFD.fOpcodeList3Count) {
        ...
        fOpcodeList3.Parse(host, stream, rawIFD.fOpcodeList3Count, rawIFD.fOpcodeList3Offset);
    }
}

/*****************************************************************************/

void dng_negative::ReadStage1Image(dng_host &host, dng_stream &stream, dng_info &info) {
    // Allocate image we are reading.

    dng_ifd &rawIFD = *info.fIFD[info.fMainIndex];

    fStage1Image.Reset(host.Make_dng_image(rawIFD.Bounds(), rawIFD.fSamplesPerPixel, rawIFD.PixelType()));

    // See if we should grab the compressed JPEG data.

    AutoPtr<dng_jpeg_image> jpegImage;

    if (host.SaveDNGVersion() >= dngVersion_1_4_0_0 && !host.PreferredSize() && !host.ForPreview() && rawIFD.fCompression == ccLossyJPEG) {
    ...
    bool needJPEGDigest =
        (RawImageDigest().IsValid() || NewRawImageDigest().IsValid()) && rawIFD.fCompression == ccLossyJPEG && jpegImage.Get() == NULL;

    dng_fingerprint jpegDigest;

    // Read the image.

    rawIFD.ReadImage(host, stream, *fStage1Image.Get(), jpegImage.Get(), needJPEGDigest ? &jpegDigest : NULL);
    ...
    ReadOpcodeLists(host, stream, info);
}

/*****************************************************************************/

void dng_negative::ReadEnhancedImage(dng_host &host, dng_stream &stream, dng_info &info) {
    // Allocate image we are reading.

    dng_ifd &enhancedIFD = *info.fIFD[info.fEnhancedIndex];

    fStage3Image.Reset(host.Make_dng_image(enhancedIFD.Bounds(), enhancedIFD.fSamplesPerPixel, enhancedIFD.PixelType()));

    // Read the image.

    enhancedIFD.ReadImage(host, stream, *fStage3Image.Get());
    ...
    bool needRawImage = host.SaveDNGVersion() != 0 && !host.SaveLinearDNG(*this);
    ...
    if (needRawImage) {
        ...
        fRawImage.Reset(host.Make_dng_image(mainIFD.Bounds(), mainIFD.fSamplesPerPixel, mainIFD.PixelType()));

        mainIFD.ReadImage(host, stream, *fRawImage.Get());
    }
    ...
    if (fLinearizationInfo.Get()) {
        if (fLinearizationInfo->fActiveArea.W()) {
            fRawToFullScaleH = (real64)fStage3Image->Bounds().W() / (real64)fLinearizationInfo->fActiveArea.W();
        }

        if (fLinearizationInfo->fActiveArea.H()) {
            fRawToFullScaleV = (real64)fStage3Image->Bounds().H() / (real64)fLinearizationInfo->fActiveArea.H();
        }
    }
...
void dng_negative::SetStage3Image(AutoPtr<dng_image> &image) {
    ...
    SetFloatingPoint(fStage3Image.Get() && (fStage3Image->PixelType() == ttFloat));
}
...
void dng_negative::DoBuildStage2(dng_host &host) {
    ...
    if (stage1.PixelType() == ttLong || stage1.PixelType() == ttFloat) {
        pixelType = ttFloat;
    }

    fStage2Image.Reset(host.Make_dng_image(info.fActiveArea.Size(), stage1.Planes(), pixelType));

    info.Linearize(host, *this, stage1, *fStage2Image.Get());
}
...
bool dng_negative::NeedDefloatStage2(dng_host &host) {
    if (fStage2Image->PixelType() == ttFloat) {
        if (fRawImageStage >= rawImageStagePostOpcode2 && host.SaveDNGVersion() != dngVersion_None
...
void dng_negative::BuildStage2Image(dng_host &host) {
    ...
    if (host.SaveDNGVersion() != dngVersion_None) {
        ...
        else if (fOpcodeList3.MinVersion(false) > host.SaveDNGVersion() || fOpcodeList3.AlwaysApply()) {
            fRawImageStage = rawImageStagePostOpcode3;
        }
        ...
        else if (fOpcodeList2.MinVersion(false) > host.SaveDNGVersion() || fOpcodeList2.AlwaysApply()) {
            fRawImageStage = rawImageStagePostOpcode2;
        }

        else if (fOpcodeList1.MinVersion(false) > host.SaveDNGVersion() || fOpcodeList1.AlwaysApply()) {
        ...
        if (host.ForFastSaveToDNG() && (fRawImageStage > rawImageStagePreOpcode1)) {
...
void dng_negative::DoInterpolateStage3(dng_host &host, int32 srcPlane, dng_matrix *scaleTransforms) {
    ...
    if (fastSaveToDNG && (fastSaveSize > 0)) {
        downScale = info.DownScale(host.MinimumSize(), host.FastSaveToDNGSize(), host.CropFactor());
    }

    else {
        downScale = info.DownScale(host.MinimumSize(), host.PreferredSize(), host.CropFactor());
    }
    ...
    fStage3Image.Reset(host.Make_dng_image(dng_rect(dstSize), info.fColorPlanes, stage2.PixelType()));

    if (srcPlane < 0 || srcPlane >= (int32)stage2.Planes()) {
        srcPlane = 0;
    }

    info.Interpolate(host, *this, stage2, *fStage3Image.Get(), downScale, srcPlane, scaleTransforms);
}
...
void dng_negative::DoMergeStage3(dng_host &host, dng_matrix *scaleTransforms) {
...
void dng_negative::DoBuildStage3(dng_host &host, int32 srcPlane, dng_matrix *scaleTransforms) {
    ...
    if (!info || !info->IsColorFilterArray()) {
        ...
        if ((fStage2Image->Planes() > 1) && (srcPlane < 0)) {
            DoMergeStage3(host, scaleTransforms);
        }

        // Else do a single channel interpolation.

        else {
            DoInterpolateStage3(host, srcPlane, scaleTransforms);
        }
...
void dng_negative::BuildStage3Image(dng_host &host, int32 srcPlane) {
...
static inline real64 SceneProxyCurve(real64 x) {
    ...
    DNG_ASSERT(Abs_real64(x - (kSceneProxyCurveSlope * y + (1.0 - kSceneProxyCurveSlope) * y * y * y)) < 0.0000001,
...
static inline real64 OutputProxyCurve(real64 x) {
    DNG_ASSERT(kOutputProxyCurveSlope == 1.0 / 16.0, "OutputProxyCurve unexpected slope");

    real64 y = (sqrt(960.0 * x + 1.0) - 1.0) / 30.0;

    DNG_ASSERT(Abs_real64(x - (kOutputProxyCurveSlope * y + (1.0 - kOutputProxyCurveSlope) * y * y)) < 0.0000001,
    ...
    virtual real64 Evaluate(real64 x) const {
        ...
        if (fIsSceneReferred) {
            if (fLower < fStage3BlackLevel) {
                x = Pin_real64(-1.0, (x - fStage3BlackLevel) / (fUpper - fStage3BlackLevel), 1.0);
...
class dng_encode_proxy_task : public dng_area_task, private dng_uncopyable {
private:
...
public:
    ...
    virtual dng_rect RepeatingTile1() const { return fSrcImage.RepeatingTile(); }

    virtual dng_rect RepeatingTile2() const { return fDstImage.RepeatingTile(); }
...

{
    for (uint32 plane = 0; plane < fSrcImage.Planes(); plane++) {
        ...
        dng_gamma_encode_proxy gamma(lower[plane], upper[plane], isSceneReferred, stage3BlackLevel, blackLevel[plane]);
...
void dng_encode_proxy_task::Process(uint32 /* threadIndex */, const dng_rect &tile, dng_abort_sniffer * /* sniffer */) {
...
dng_image *dng_negative::EncodeRawProxy(dng_host &host, const dng_image &srcImage, dng_opcode_list &opcodeList, real64 *blackLevel) const {
...

    {
        ...
        uint64 pixels = (uint64)srcImage.Bounds().H() * (uint64)srcImage.Bounds().W();
        ...
        for (uint32 plane = 0; plane < srcImage.Planes(); plane++) {
            HistogramArea(host, srcImage, srcImage.Bounds(), hist, 65535, plane);
    ...
    if (isSceneReferred && stage3BlackLevel > 0.0) {
        for (uint32 plane = 0; plane < srcImage.Planes(); plane++) {
            if (lower[plane] < stage3BlackLevel) {
                upper[plane] = Max_real64(
                    upper[plane], stage3BlackLevel + (stage3BlackLevel - lower[plane]) * (1.0 / kMaxStage3BlackLevelNormalized - 1.0));

                upper[plane] = Min_real64(upper[plane], 1.0);

                real64 negRange = SceneProxyCurve((stage3BlackLevel - lower[plane]) / (upper[plane] - stage3BlackLevel));

                real64 outBlack = negRange / (1.0 + negRange);

                blackLevel[plane] = Min_real64(kMaxStage3BlackLevelNormalized * 255.0, ceil(outBlack * 255.0));
            }
        }
    }
    ...
    AutoPtr<dng_image> dstImage(host.Make_dng_image(srcImage.Bounds(), srcImage.Planes(), ttByte));

    {
        dng_encode_proxy_task task(host, srcImage, *dstImage, lower, upper, isSceneReferred, stage3BlackLevel, blackLevel);

        host.PerformAreaTask(task, srcImage.Bounds());
    }
    ...
    {
        for (uint32 plane = 0; plane < srcImage.Planes(); plane++) {
            dng_area_spec areaSpec(srcImage.Bounds(), plane);
            ...
            AutoPtr<dng_opcode> opcode(new dng_opcode_MapPolynomial(areaSpec, isSceneReferred ? 3 : 2, coefficient));
...
void dng_negative::ConvertToProxy(dng_host &host, dng_image_writer &writer, uint32 proxySize, uint64 proxyCount) {
    if (!proxySize) {
    ...
    if (proxySize < kMaxImageSide || proxyCount < kMaxImageSide * kMaxImageSide) {
        ClearMakerNote();
    ...
    if (fRawImage.Get() && fRawImage->PixelType() == ttByte && fRawImage->Bounds() == DefaultCropArea()
        && fRawImage->Bounds().H() <= proxySize && fRawImage->Bounds().W() <= proxySize
        && (uint64)fRawImage->Bounds().H() * (uint64)fRawImage->Bounds().W() <= proxyCount && fRawToFullScaleH == 1.0
        && fRawToFullScaleV == 1.0 && (!GetMosaicInfo() || !GetMosaicInfo()->IsColorFilterArray()) && fRawJPEGImage.Get()
        && (!RawTransparencyMask() || RawTransparencyMask()->PixelType() == ttByte)) {
        return;
    }

    if (fRawImage.Get() && fRawImage->PixelType() == ttFloat && fRawImage->Bounds().H() <= proxySize && fRawImage->Bounds().W() <= proxySize
        && (uint64)fRawImage->Bounds().H() * (uint64)fRawImage->Bounds().W() <= proxyCount && fRawToFullScaleH == 1.0
        && fRawToFullScaleV == 1.0 && RawFloatBitDepth() == 16
        && (!RawTransparencyMask() || RawTransparencyMask()->PixelType() == ttByte)) {
        return;
    }
    ...
    if ((uint64)newSize.v * (uint64)newSize.h > proxyCount) {
        if (aspectRatio >= 1.0) {
    ...
    if ((uint64)newSize.v * (uint64)newSize.h < (uint64)oldSize.v * (uint64)oldSize.h || pixelAspect < 0.99 || pixelAspect > 1.01) {
        const dng_image &srcImage(*Stage3Image());

        AutoPtr<dng_image> dstImage(host.Make_dng_image(newSize, srcImage.Planes(), srcImage.PixelType()));

        host.ResampleImage(srcImage, *dstImage);
    ...
    else if (fRawToFullScaleH != 1.0 || fRawToFullScaleV != 1.0) {
    ...
    if (Stage3Image()->PixelType() == ttFloat) {
        fRawImage.Reset(host.Make_dng_image(Stage3Image()->Bounds(), Stage3Image()->Planes(), ttFloat));

        fRawImageBlackLevel = 0;

        LimitFloatBitDepth(host, *Stage3Image(), *fRawImage, 16, 32768.0f);
        ...
        fRawImage.Reset(EncodeRawProxy(host, *Stage3Image(), fOpcodeList2, blackLevel));
        ...
        if (fRawImage.Get()) {
            ...
            if (fRawImage->PixelType() == ttByte && host.SaveDNGVersion() >= dngVersion_1_4_0_0) {
                AutoPtr<dng_jpeg_image> jpegImage(new dng_jpeg_image);

                jpegImage->Encode(host, *this, writer, *fRawImage);
    ...
    if (DepthMap()) {
        ...
        if (fRawDepthMap.Get()) {
            if (fRawDepthMap->Bounds().W() > fDepthMap->Bounds().W() || fRawDepthMap->Bounds().H() > fDepthMap->Bounds().H()) {
                fRawDepthMap.Reset();
            }
        }
    }
...
bool dng_negative::IsProxy() const {
    return (DefaultCropSizeH() != OriginalDefaultCropSizeH()) && (DefaultCropSizeV() != OriginalDefaultCropSizeV());
}
...
void dng_negative::SetTransparencyMask(AutoPtr<dng_image> &image, uint32 bitDepth) {
...
void dng_negative::ReadTransparencyMask(dng_host &host, dng_stream &stream, dng_info &info) {
    if (info.fMaskIndex != -1) {
        // Allocate image we are reading.

        dng_ifd &maskIFD = *info.fIFD[info.fMaskIndex];

        fTransparencyMask.Reset(host.Make_dng_image(maskIFD.Bounds(), 1, maskIFD.PixelType()));

        // Read the image.

        maskIFD.ReadImage(host, stream, *fTransparencyMask.Get());
...
void dng_negative::ResizeTransparencyToMatchStage3(dng_host &host, bool convertTo8Bit) {
    if (TransparencyMask()) {
        if ((TransparencyMask()->Bounds() != fStage3Image->Bounds()) || (TransparencyMask()->PixelType() != ttByte && convertTo8Bit)) {
            AutoPtr<dng_image> newMask(
                host.Make_dng_image(fStage3Image->Bounds(), 1, convertTo8Bit ? ttByte : TransparencyMask()->PixelType()));

            host.ResampleImage(*TransparencyMask(), *newMask);
...
void dng_negative::ReadDepthMap(dng_host &host, dng_stream &stream, dng_info &info) {
    if (info.fDepthIndex != -1) {
        // Allocate image we are reading.

        dng_ifd &depthIFD = *info.fIFD[info.fDepthIndex];

        fDepthMap.Reset(host.Make_dng_image(depthIFD.Bounds(), 1, depthIFD.PixelType()));

        // Read the image.

        depthIFD.ReadImage(host, stream, *fDepthMap.Get());
...
void dng_negative::ResizeDepthToMatchStage3(dng_host &host) {
    if (DepthMap()) {
        if (DepthMap()->Bounds() != fStage3Image->Bounds()) {
            ...
            if (!fRawDepthMap.Get()) {
                uint64 imagePixels = fStage3Image->Bounds().H() * (uint64)fStage3Image->Bounds().W();

                uint64 depthPixels = DepthMap()->Bounds().H() * (uint64)DepthMap()->Bounds().W();
            ...
            AutoPtr<dng_image> newMap(host.Make_dng_image(fStage3Image->Bounds(), 1, DepthMap()->PixelType()));

            host.ResampleImage(*DepthMap(), *newMap);
