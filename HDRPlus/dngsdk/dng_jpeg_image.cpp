...
class dng_jpeg_image_encode_task : public dng_area_task, private dng_uncopyable {
private:
    dng_host &fHost;
...
public:
    ...
    void Process(uint32 /* threadIndex */, const dng_rect & /* tile */, dng_abort_sniffer *sniffer) {
        ...
        uint32 uncompressedSize = SafeUint32Mult(fIFD.fTileLength, fIFD.fTileWidth, fIFD.fSamplesPerPixel);
        ...
        while (true) {
            ...
            fWriter.WriteTile(
                fHost, fIFD, stream, fImage, tileArea, 1, compressedBuffer, uncompressedBuffer, subTileBlockBuffer, tempBuffer, true);
...
void dng_jpeg_image::Encode(dng_host &host, const dng_negative &negative, dng_image_writer &writer, const dng_image &image) {
#if qDNGValidate
    ...
    bool useHigherQuality =
        (uint64)ifd.fImageWidth * (uint64)ifd.fImageLength > 5000000 || image.Bounds().Size() == negative.OriginalDefaultFinalSize();
    ...
    uint32 threadCount = Min_uint32(tileCount, host.PerformAreaTaskThreads());

    dng_jpeg_image_encode_task task(host, writer, image, *this, tileCount, ifd);
...
class dng_jpeg_image_find_digest_task : public dng_area_task, private dng_uncopyable {
private:
...
public:
    ...
    void Process(uint32 /* threadIndex */, const dng_rect & /* tile */, dng_abort_sniffer *sniffer) {
        while (true) {
            ...
            printer.Process(fJPEGImage.fJPEGData[tileIndex]->Buffer(), fJPEGImage.fJPEGData[tileIndex]->LogicalSize());
...
dng_fingerprint dng_jpeg_image::FindDigest(dng_host &host) const {
...

    {
        ...
        dng_jpeg_image_find_digest_task task(*this, tileCount, digests.Get());
...

    {
        ...
        for (uint32 k = 0; k < arrayCount; k++) {
            printer.Process(digests[k].data, dng_fingerprint::kDNGFingerprintSize);
        }
