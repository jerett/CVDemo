...
public:
    ...
    ~InnermostMutexHolder() { pthread_key_delete(fInnermostMutexKey); }
...
void dng_mutex::Lock() {
    ...
    if (innermostMutex != NULL) {
        ...
        if (!lockOrderPreserved) {
            ...
            sprintf(msg, "Lock order violation: This mutex: %s v Innermost mutex: %s", this->MutexName(), innermostMutex->MutexName());
