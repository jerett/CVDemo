...
public:
    ...
    const int32 *Coords(int32 index) const { return fCoords->Buffer_int32() + (index - fOrigin); }

    int32 Pixel(int32 index) const { return Coords(index)[0] >> kResampleSubsampleBits; }
};
...
public:
    ...
    uint32 Radius() const { return fRadius; }

    uint32 Width() const { return fRadius * 2; }

    int32 Offset() const { return 1 - (int32)fRadius; }

    uint32 Step() const { return fWeightStep; }
...
public:
    ...
    uint32 RowStep() const { return fRowStep; }

    uint32 ColStep() const { return fColStep; }
