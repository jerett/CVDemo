...
dng_memory_stream::dng_memory_stream(dng_memory_allocator &allocator, dng_abort_sniffer *sniffer, uint32 pageSize)

    : dng_stream(sniffer, kDefaultBufferSize, kDNGStreamInvalidOffset)
...
void dng_memory_stream::DoRead(void *data, uint32 count, uint64 offset) {
    ...
    while (count) {
        ...
        const uint8 *sPtr = fPageList[pageIndex]->Buffer_uint8() + pageOffset;
...
void dng_memory_stream::DoSetLength(uint64 length) {
    if (fLengthLimit && length > fLengthLimit) {
        Throw_dng_error(dng_error_end_of_file, "dng_memory_stream::fLengthLimit", NULL, true);
    }
    ...
    while (length > fPageCount * (uint64)fPageSize) {
        if (fPageCount == fPagesAllocated) {
            ...
            if (!SafeUint32Add(fPagesAllocated, 32u, &newSizeTemp1) || !SafeUint32Mult(fPagesAllocated, 2u, &newSizeTemp2)) {
            ...
            if (!SafeUint32Mult(newSize, sizeof(dng_memory_block *), &numBytes)) {
                ThrowOverflow("Arithmetic overflow in DoSetLength");
            }
            ...
            if (fPageCount) {
                ...
                DoCopyBytes(fPageList, list, fPageCount * (uint32)sizeof(dng_memory_block *));
            }
...
void dng_memory_stream::DoWrite(const void *data, uint32 count, uint64 offset) {
    DoSetLength(Max_uint64(fMemoryStreamLength, offset + count));
    ...
    while (count) {
        ...
        uint8 *dPtr = fPageList[pageIndex]->Buffer_uint8() + pageOffset;
...
void dng_memory_stream::CopyToStream(dng_stream &dstStream, uint64 count) {
