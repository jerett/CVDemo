...
struct ScopedLock {
    ...
    ScopedLock(dng_pthread_mutex_impl *arg) : mutex(arg) { mutex->Lock(); }
    ScopedLock(dng_pthread_mutex_impl &arg) : mutex(&arg) { mutex->Lock(); }
...
int dng_pthread_rwlock_destroy(dng_pthread_rwlock_t *rwlock) {
...

    {
        ...
        if (real_rwlock.head_waiter != NULL || real_rwlock.readers_active != 0 || real_rwlock.writers_waiting != 0
...
int dng_pthread_rwlock_trywrlock(dng_pthread_rwlock_t *rwlock) {
    ...
    if (real_rwlock.readers_active == 0 && real_rwlock.writers_waiting == 0 && !real_rwlock.writer_active) {
...
int dng_pthread_rwlock_wrlock(dng_pthread_rwlock_t *rwlock) {
...

    {
        ...
        if (real_rwlock.readers_active || real_rwlock.writers_waiting || real_rwlock.writer_active) {
            semaphore = GetThreadSemaphore();
