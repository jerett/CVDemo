...
void dng_camera_profile::NormalizeForwardMatrix(dng_matrix &m) {
    if (m.NotEmpty()) {
        ...
        m = PCStoXYZ().AsDiagonal() * Invert(xyz.AsDiagonal()) * m;
    }
}
...
bool dng_camera_profile::HasColorMatrix1() const {
    return fColorMatrix1.Cols() == 3 && fColorMatrix1.Rows() > 1;
}

/*****************************************************************************/

bool dng_camera_profile::HasColorMatrix2() const {
    return fColorMatrix2.Cols() == 3 && fColorMatrix2.Rows() == fColorMatrix1.Rows();
}
...
static void FingerprintMatrix(dng_md5_printer_stream &printer, const dng_matrix &matrix) {
...
static void FingerprintHueSatMap(dng_md5_printer_stream &printer, const dng_hue_sat_map &map) {
    if (map.IsNull())
        return;
...
void dng_camera_profile::CalculateFingerprint() const {
    ...
    if (HasColorMatrix1()) {
        ...
        if (fForwardMatrix1.Rows() == fColorMatrix1.Cols() && fForwardMatrix1.Cols() == fColorMatrix1.Rows()) {
            FingerprintMatrix(printer, fForwardMatrix1);
        }

        if (colorChannels > 3 && fReductionMatrix1.Rows() * fReductionMatrix1.Cols() == colorChannels * 3) {
        ...
        if (HasColorMatrix2()) {
            ...
            if (fForwardMatrix2.Rows() == fColorMatrix2.Cols() && fForwardMatrix2.Cols() == fColorMatrix2.Rows()) {
                FingerprintMatrix(printer, fForwardMatrix2);
            }

            if (colorChannels > 3 && fReductionMatrix2.Rows() * fReductionMatrix2.Cols() == colorChannels * 3) {
                FingerprintMatrix(printer, fReductionMatrix2);
            }
        }

        printer.Put(fName.Get(), fName.Length());

        printer.Put(fProfileCalibrationSignature.Get(), fProfileCalibrationSignature.Length());

        printer.Put_uint32(fEmbedPolicy);

        printer.Put(fCopyright.Get(), fCopyright.Length());

        bool haveHueSat1 = HueSatDeltas1().IsValid();

        bool haveHueSat2 = HueSatDeltas2().IsValid() && HasColorMatrix2();
...
dng_fingerprint dng_camera_profile::UniqueID() const {
    ...
    printer.Put(fFingerprint.data, (uint32)sizeof(fFingerprint.data));

    // Also include the UniqueCameraModelRestriction tag.

    printer.Put(fUniqueCameraModelRestriction.Get(), fUniqueCameraModelRestriction.Length());
...
bool dng_camera_profile::ValidForwardMatrix(const dng_matrix &m) {
    ...
    if (m.NotEmpty()) {
        ...
        if (Abs_real64(xyz[0] - pcs[0]) > kThreshold || Abs_real64(xyz[1] - pcs[1]) > kThreshold
...
bool dng_camera_profile::IsValid(uint32 channels) const {
    ...
    if (fColorMatrix1.Cols() != 3 || fColorMatrix1.Rows() != channels) {
#if qDNGValidate
    ...
    if (fColorMatrix2.Cols() != 0 || fColorMatrix2.Rows() != 0) {
        if (fColorMatrix2.Cols() != 3 || fColorMatrix2.Rows() != channels) {
#if qDNGValidate
    ...
    if (fForwardMatrix1.Cols() != 0 || fForwardMatrix1.Rows() != 0) {
        if (fForwardMatrix1.Rows() != 3 || fForwardMatrix1.Cols() != channels) {
#if qDNGValidate
    ...
    if (fForwardMatrix2.Cols() != 0 || fForwardMatrix2.Rows() != 0) {
        if (fForwardMatrix2.Rows() != 3 || fForwardMatrix2.Cols() != channels) {
#if qDNGValidate
    ...
    if (fReductionMatrix1.Cols() != 0 || fReductionMatrix1.Rows() != 0) {
        if (fReductionMatrix1.Cols() != channels || fReductionMatrix1.Rows() != 3) {
#if qDNGValidate
    ...
    if (fReductionMatrix2.Cols() != 0 || fReductionMatrix2.Rows() != 0) {
        if (fReductionMatrix2.Cols() != channels || fReductionMatrix2.Rows() != 3) {
#if qDNGValidate
    ...
    try {
        if (fReductionMatrix1.NotEmpty()) {
            (void)Invert(fColorMatrix1, fReductionMatrix1);
        }
    ...
    if (fColorMatrix2.NotEmpty()) {
        try {
            if (fReductionMatrix2.NotEmpty()) {
                (void)Invert(fColorMatrix2, fReductionMatrix2);
            }
...
bool dng_camera_profile::EqualData(const dng_camera_profile &profile) const {
    return fCalibrationIlluminant1 == profile.fCalibrationIlluminant1 && fCalibrationIlluminant2 == profile.fCalibrationIlluminant2
           && fColorMatrix1 == profile.fColorMatrix1 && fColorMatrix2 == profile.fColorMatrix2 && fForwardMatrix1 == profile.fForwardMatrix1
           && fForwardMatrix2 == profile.fForwardMatrix2 && fReductionMatrix1 == profile.fReductionMatrix1
           && fReductionMatrix2 == profile.fReductionMatrix2 && fHueSatDeltas1 == profile.fHueSatDeltas1
           && fHueSatDeltas2 == profile.fHueSatDeltas2 && fHueSatMapEncoding == profile.fHueSatMapEncoding
           && fLookTable == profile.fLookTable && fLookTableEncoding == profile.fLookTableEncoding
           && fDefaultBlackRender == profile.fDefaultBlackRender && fToneCurve == profile.fToneCurve
...
void dng_camera_profile::Parse(dng_stream &stream, dng_camera_profile_info &profileInfo) {
    ...
    if (profileInfo.fHueSatDeltas1Offset != 0 && profileInfo.fHueSatDeltas1Count != 0) {
        TempBigEndian setEndianness(stream, profileInfo.fBigEndian);

        stream.SetReadPosition(profileInfo.fHueSatDeltas1Offset);

        bool skipSat0 =
            (profileInfo.fHueSatDeltas1Count == profileInfo.fProfileHues * (profileInfo.fProfileSats - 1) * profileInfo.fProfileVals * 3);

        ReadHueSatMap(stream, fHueSatDeltas1, profileInfo.fProfileHues, profileInfo.fProfileSats, profileInfo.fProfileVals, skipSat0);
    }

    if (profileInfo.fHueSatDeltas2Offset != 0 && profileInfo.fHueSatDeltas2Count != 0) {
        TempBigEndian setEndianness(stream, profileInfo.fBigEndian);

        stream.SetReadPosition(profileInfo.fHueSatDeltas2Offset);

        bool skipSat0 =
            (profileInfo.fHueSatDeltas2Count == profileInfo.fProfileHues * (profileInfo.fProfileSats - 1) * profileInfo.fProfileVals * 3);

        ReadHueSatMap(stream, fHueSatDeltas2, profileInfo.fProfileHues, profileInfo.fProfileSats, profileInfo.fProfileVals, skipSat0);
    }

    if (profileInfo.fLookTableOffset != 0 && profileInfo.fLookTableCount != 0) {
        TempBigEndian setEndianness(stream, profileInfo.fBigEndian);

        stream.SetReadPosition(profileInfo.fLookTableOffset);

        bool skipSat0 =
            (profileInfo.fLookTableCount == profileInfo.fLookTableHues * (profileInfo.fLookTableSats - 1) * profileInfo.fLookTableVals * 3);

        ReadHueSatMap(stream, fLookTable, profileInfo.fLookTableHues, profileInfo.fLookTableSats, profileInfo.fLookTableVals, skipSat0);
    }
...
dng_hue_sat_map *dng_camera_profile::HueSatMapForWhite(const dng_xy_coord &white) const {
    if (fHueSatDeltas1.IsValid()) {
        ...
        return dng_hue_sat_map::Interpolate(HueSatDeltas1(), HueSatDeltas2(), g);
    }
...
void SplitCameraProfileName(const dng_string &name, dng_string &baseName, int32 &version) {
    ...
    if (len == 7 && baseName.StartsWith("ACR ", true)) {
        if (name.Get()[len - 3] >= '0' && name.Get()[len - 3] <= '9' && name.Get()[len - 2] == '.' && name.Get()[len - 1] >= '0'
            && name.Get()[len - 1] <= '9')

            baseName.Truncate(3);

        version = ((int32)(name.Get()[len - 3] - '0')) * 10 + ((int32)(name.Get()[len - 1] - '0'));
...
void BuildHueSatMapEncodingTable(dng_memory_allocator &allocator,
    ...
    switch (encoding) {
        ...
        case encoding_sRGB: {
            ...
            encodeTable->Initialize(allocator, curve, subSample);

            const dng_1d_inverse inverse(curve);

            decodeTable->Initialize(allocator, inverse, subSample);
